...
def main():
    ...
    if (names != ['T8-control12-repeat-r9', 'T11-sve8x16-repeat-r9', 'T12-forwardbarrier']
            or order != [names[i:] + names[:i] for i in range(3)]):
        raise RuntimeError('The predeclared r9 protocol requires formal A/B/C, B/C/A, C/A/B rounds')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r9-one-official-warmup-v1'
    ...
    with (ROOT / 'preflight-wide.log').open('xb') as log:
        code = stream(['/bin/bash', str(ROOT / 'preflight-wide/run.sh'),
                       str(ROOT / 'T11-sve8x16-repeat-r9/source'),
                       str(ROOT / 'preflight-wide-results/T11-sve8x16-repeat-r9')], ROOT, env, [log])
        if code:
            raise RuntimeError('Wide-kernel and large-path supplemental preflight failed')
    completion = (ROOT / 'preflight-wide-results/T11-sve8x16-repeat-r9/completion.txt').read_text()
