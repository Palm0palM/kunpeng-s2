...
def prepare():
    ...
    if [item['run_name'] for item in plan['members']] != ['T8-control12-repeat-r9', 'T11-sve8x16-repeat-r9', 'T12-forwardbarrier']:
        raise RuntimeError('The predeclared r9 protocol requires exactly the planned A/B/C members')
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r9-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r9 measurements.',
