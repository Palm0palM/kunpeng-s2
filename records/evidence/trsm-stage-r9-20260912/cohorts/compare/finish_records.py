#!/usr/bin/env python3
"""Register collected r9 logs without running task code or computing digests."""
...
names = ['T8-control12-repeat-r9', 'T11-sve8x16-repeat-r9', 'T12-forwardbarrier']
...
wide = (DEST / 'preflight-wide-results/T11-sve8x16-repeat-r9/completion.txt').read_text().strip()
...
comparisons = {}
for candidate in ('T11-sve8x16', 'T12-forwardbarrier'):
...
medians = {name: json.loads((ROOT / 'records/experiments/trsm' / (name + '.json')).read_text())['total_median_ms']
           for name in ('T8-control12', 'T11-sve8x16', 'T12-forwardbarrier')}
