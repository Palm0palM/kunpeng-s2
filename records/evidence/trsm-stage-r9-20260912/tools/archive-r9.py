#!/usr/bin/env python3
"""Create one public, redacted r9 evidence archive; never run tests or digests.
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r9-20260912'
COHORTS = {
    'compare': RUNS / 'optimization-20260912-r9-compare',
}
VERSION_BY_MEMBER = {
    'T8-control12-repeat-r9': 'T8-control12',
    'T11-sve8x16-repeat-r9': 'T11-sve8x16',
    'T12-forwardbarrier': 'T12-forwardbarrier',
}
MEMBERS = tuple(VERSION_BY_MEMBER)
REPEAT_ORIGINS = {
    'T8-control12-repeat-r9': 'T8-control12-repeat-r8',
    'T11-sve8x16-repeat-r9': 'T11-sve8x16',
}
...
def main():
    ...
    for member in MEMBERS:
        ...
        for name in sorted(MEMBER_FILES):
            add(folder / name, prefix / name,
                required=member == 'T12-forwardbarrier' and name == 'PREPARATION.md')
        for name in SOURCE_FILES:
        ...
        for path in sorted(folder.glob('*NOTES*.md')):
            add(path, prefix / path.name)
        assembly_reviews(folder, prefix)
        if member == 'T11-sve8x16-repeat-r9':
            # A repeated run has new source/log snapshots, but need not have
            # newly authored candidate documents. Keep the original provenance.
            for name in ('PREPARATION.md', 'STATIC-REVIEW.md', 'VALIDATION-PLAN.md'):
                preparation = folder / name
                destination = prefix / name
                if not preparation.is_file() or preparation.is_symlink():
                    preparation = RUNS / 'T11-sve8x16' / name
    ...
    add(Path(__file__).resolve(), 'tools/archive-r9.py', required=True)

    # Both repeats keep separate pre-r9 records. Associate each with its own
    # public r8 record and run, using the known r8 job ID (no byte identity claim).
    history_archive = ROOT / 'records/evidence/trsm-stage-r8-20260912'
    expected_history_job = '1579444'
    ...
    for member, prior_run in REPEAT_ORIGINS.items():
        ...
        prior_record_origins.append({
            ...
            'prior_run': prior_run,
            'prior_public_archive': '../trsm-stage-r8-20260912/',
            'prior_public_record': '../trsm-stage-r8-20260912/records/latest/' + version + '.json',
            'prior_run_cluster': '../trsm-stage-r8-20260912/members/' + prior_run + '/cluster.json',
            ...
            'note': 'Job ID/version association only. The r8 archive retains both original records and their run evidence; older T8 history is referenced there. Original T8 baseline evidence is retained in r6.',
        })
        if not job_id_match or not version_match:
            missing.append(member + ' prior-record.json matching its public r8 record and job 1579444; inspect the referenced r8 archive')
...
File owner/group identifiers redacted for publication.
    ...
    document = {
        'schema_version': 1, 'problem': 'trsm', 'stage': 'r9-20260912',
        ...
        'repeat_policy': 'The r9 T8 and T11 repeats each retain a separate prior record associated by version and job 1579444 with the corresponding public r8 record. Older T8 history is referenced by r8; original T8 baseline evidence is retained in r6. Samples from separate jobs are not merged.',
    ...
    (DEST / 'README.md').write_text('''# TRSM r9 公开实验归档
...
''')
    print('Created public TRSM r9 archive:', DEST.relative_to(ROOT), 'text_files=', len(inventory))
