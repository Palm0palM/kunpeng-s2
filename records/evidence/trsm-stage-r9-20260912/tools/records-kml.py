...
def validate_repeat_existing(folder, meta, previous):
    """Repeat an unchanged current baseline or its verified, unpromoted child."""
    ...
    if ('parent' not in meta or 'parent' not in previous
            or meta['parent'] != previous['parent']):
        raise ValueError('--repeat-existing requires the original explicit parent value')
    parent = previous['parent']
    for key in ('created_at', 'strategy', 'source_implementation_id'):
        if not isinstance(previous.get(key), str) or not previous[key].strip():
            raise ValueError('--repeat-existing requires the original identity field: ' + key)
    current_best = read_json(ROOT / 'records' / 'best.json').get('trsm')
    if parent is None:
        if not isinstance(previous.get('promoted_at'), str) or not previous['promoted_at'].strip():
            raise ValueError('--repeat-existing requires the original baseline field: promoted_at')
        if current_best != meta['version']:
            raise ValueError('--repeat-existing requires this baseline to remain the current TRSM best')
    else:
        version_id(parent)
        if parent == meta['version'] or current_best != parent:
            raise ValueError('--repeat-existing requires the candidate parent to remain the current TRSM best')
        if previous.get('promoted_at') is not None:
            raise ValueError('--repeat-existing candidate must not have been promoted')
        parent_record = load_record(parent)
        if not isinstance(parent_record.get('promoted_at'), str) or not parent_record['promoted_at'].strip():
            raise ValueError('--repeat-existing requires a promoted candidate parent')
        parent_reasons = record_reasons(parent_record)
        if parent_reasons:
            raise ValueError('--repeat-existing candidate parent is invalid: ' + '; '.join(parent_reasons))
        for name in SOURCES:
            if not same_bytes(ROOT / 'trsm' / name, measured_source(parent_record, name)):
                raise ValueError('Current trsm source differs from the promoted candidate parent: ' + name)
    ...
    for entry in history:
        ...
        if (not isinstance(historical_record, dict)
                ...
                or 'parent' not in historical_record or historical_record['parent'] != parent
    ...
    for name in SOURCES:
        ...
        if not same_bytes(candidate, measured_source(previous, name)):
            raise ValueError('A repeated source must match its prior measurement snapshot: ' + name)
        if parent is None and not same_bytes(candidate, ROOT / 'trsm' / name):
            raise ValueError('A repeated baseline source must match current trsm bytes: ' + name)


def preserve_repeat_identity(result, previous):
    # A new measurement must not redefine the original strategy or source lineage.
    keys = {'problem', 'version', 'created', 'created_at', 'strategy', 'parent', 'promoted_at',
            'promotion_validation_policy'}
    keys.update(key for key in result if key.startswith(('source_', 'promotion_', 'promoted_')))
    keys.update(key for key in previous if key.startswith(('source_', 'promotion_', 'promoted_')))
    for key in keys:
        if key in previous:
...
def report_repeat_audit_exception(folder, meta, previous, error):
    report = dict(problem='trsm', version=meta['version'], parent=previous['parent'],
...
def do_record(args):
    ...
    if args.repeat_existing:
        try:
            ...
            if any(not same_bytes(snapshot / 'source' / name, measured_source(previous, name))
                   or (previous['parent'] is None
                       and not same_bytes(snapshot / 'source' / name, ROOT / 'trsm' / name))
                   for name in SOURCES):
                raise ValueError('The repeat snapshot differs from the prior source or required current baseline')
    ...
    if args.repeat_existing:
        ...
        if verified:
            ...
            if 'promoted_at' in previous:
                prior_entry['promoted_at'] = previous['promoted_at']
            result['previous_runs'] = copy.deepcopy(previous.get('previous_runs', [])) + [prior_entry]
...
def main():
    ...
    p.add_argument('--repeat-existing', action='store_true',
                   help='Repeat the unchanged current parent=null baseline or its verified unpromoted candidate; preserve prior evidence')
