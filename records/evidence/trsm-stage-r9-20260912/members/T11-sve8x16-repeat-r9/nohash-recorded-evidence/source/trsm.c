...
/* Two eight-column panels share one L coefficient per row and k.
 * X0/X1 have independent panel origins but the same k stride: packed panels
 * are KB*RHS apart, while the allocation-failure path uses adjacent columns.
 * Every output keeps the original zero-based, increasing-k FMA chain and
 * subtracts that sum from C only after the complete dot product. */
__attribute__((target("arch=armv8-a+sve"), noinline))
static void update8x16_sve(int count,const double *L,int lda,const double *X0,const double *X1,int ldx,double *C,int ldc)
{
    svbool_t pg=svptrue_b64();
    ...
    svfloat64_t a4_8=svdup_n_f64(0);
    svfloat64_t a5_0=svdup_n_f64(0);
    svfloat64_t a5_8=svdup_n_f64(0);
    svfloat64_t a6_0=svdup_n_f64(0);
    svfloat64_t a6_8=svdup_n_f64(0);
    svfloat64_t a7_0=svdup_n_f64(0);
    svfloat64_t a7_8=svdup_n_f64(0);
    for(int k=0;k<count;++k) {
        ...
        { double a=L[(size_t)4*lda+k];
            a4_0=svmla_n_f64_x(pg,a4_0,x0,a);
            a4_8=svmla_n_f64_x(pg,a4_8,x1,a);
        }
        { double a=L[(size_t)5*lda+k];
            a5_0=svmla_n_f64_x(pg,a5_0,x0,a);
            a5_8=svmla_n_f64_x(pg,a5_8,x1,a);
        }
        { double a=L[(size_t)6*lda+k];
            a6_0=svmla_n_f64_x(pg,a6_0,x0,a);
            a6_8=svmla_n_f64_x(pg,a6_8,x1,a);
        }
        { double a=L[(size_t)7*lda+k];
            a7_0=svmla_n_f64_x(pg,a7_0,x0,a);
            a7_8=svmla_n_f64_x(pg,a7_8,x1,a);
        }
    }
    ...
    svst1_f64(pg,C+(size_t)4*ldc+0,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)4*ldc+0),a4_0));
    svst1_f64(pg,C+(size_t)4*ldc+8,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)4*ldc+8),a4_8));
    svst1_f64(pg,C+(size_t)5*ldc+0,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)5*ldc+0),a5_0));
    svst1_f64(pg,C+(size_t)5*ldc+8,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)5*ldc+8),a5_8));
    svst1_f64(pg,C+(size_t)6*ldc+0,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)6*ldc+0),a6_0));
    svst1_f64(pg,C+(size_t)6*ldc+8,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)6*ldc+8),a6_8));
    svst1_f64(pg,C+(size_t)7*ldc+0,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)7*ldc+0),a7_0));
    svst1_f64(pg,C+(size_t)7*ldc+8,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)7*ldc+8),a7_8));
}
            ...
            for(int ib=end;ib<m;ib+=CT)for(int jb=0;jb<n;jb+=CT) {
                ...
                if(use_sve) {
                    for(;ie-first_row>=16;first_row+=16) {
                        int j=jb;
                        for(;j+16<=je;j+=16) {
                            const double *x0=packed?packed+(size_t)(j/RHS)*KB*RHS:B+(size_t)kk*ldb+j;
                            const double *x1=packed?x0+(size_t)KB*RHS:x0+RHS;
                            int xstride=packed?RHS:ldb;
                            update8x16_sve(end-kk,L+(size_t)first_row*lda+kk,lda,x0,x1,xstride,B+(size_t)first_row*ldb+j,ldb);
                            update8x16_sve(end-kk,L+(size_t)(first_row+8)*lda+kk,lda,x0,x1,xstride,B+(size_t)(first_row+8)*ldb+j,ldb);
                        }
                        for(;j<je;j+=8) {
