...
def main():
    ...
    if (names != ['T8-control12-repeat-r13', 'T10-lhistbarrier-repeat-r13', 'T14-lhistcyclic']
            or order != [names[i:] + names[:i] for i in range(3)]):
        raise RuntimeError('The predeclared r13 protocol requires formal A/B/C, B/C/A, C/A/B rounds')
    for index, (name, version) in enumerate(zip(names, ('T8-control12', 'T10-lhistbarrier', 'T14-lhistcyclic'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index < 2)):
            raise RuntimeError('The r13 members must retain their declared versions and repeat flags')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r13-one-official-warmup-v1'
    ...
    with (ROOT / 'preflight.log').open('xb') as log:
        for name in names:
            ...
            source_text = (ROOT / name / 'source/trsm.c').read_text()
            ...
            if features != (True, name != names[0]):
                raise RuntimeError('Declared general preflight source features differ for ' + name)
            expected = ('TRSM_PREFLIGHT_COMPLETE=1 MICRO_CASES=28 WHOLE_CASES=486 '
                        'NOOP_CASES=36 OLD_MICRO_CASES=14 PACKED_MICRO_CASES=14 SHARED_FAIL_CASES=80'
                        if features[1] else
                        'TRSM_PREFLIGHT_COMPLETE=1 MICRO_CASES=14 WHOLE_CASES=406 '
                        'NOOP_CASES=28 OLD_MICRO_CASES=14 PACKED_MICRO_CASES=0 SHARED_FAIL_CASES=0')
            completion = (ROOT / 'preflight-results' / name / 'completion.txt').read_text().strip()
            if completion != expected:
                raise RuntimeError('General preflight completion differs from actual source features: ' + name)
    handles = {}
    try:
        member_details = {}
        for name in names:
