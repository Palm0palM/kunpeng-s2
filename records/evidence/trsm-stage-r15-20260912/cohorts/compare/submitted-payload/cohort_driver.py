...
def main():
    ...
    if (names != ['T8-control12-repeat-r15', 'T18-budgetwide-repeat-r15']
            or order != [names, names[::-1], names]):
        raise RuntimeError('The predeclared r15 protocol requires formal A/B, B/A, A/B rounds')
    for index, (name, version) in enumerate(zip(names, ('T8-control12', 'T18-budgetwide'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not True):
            raise RuntimeError('The r15 members must retain their declared versions and repeat flags')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r15-one-official-warmup-v1'
    ...
    with (ROOT / 'preflight-wide32.log').open('xb') as log:
        name = 'T18-budgetwide-repeat-r15'
