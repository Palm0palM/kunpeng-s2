...
PREPARE_REVIEW_PENDING = False  # Root completed independent review; see STATIC-CONTROLLER-REVIEW.md.
EXPECTED_NAMES = ['T8-control12-repeat-r15', 'T18-budgetwide-repeat-r15']
EXPECTED_VERSIONS = ['T8-control12', 'T18-budgetwide']
PRIOR_JOB_ID = '1579730'
...
def record_helpers():
    ...
    spec = importlib.util.spec_from_file_location('r15_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
def prepare():
    if PREPARE_REVIEW_PENDING:
        raise RuntimeError('r15 preparation is blocked pending root independent review')
    ...
    if ([item['run_name'] for item in plan['members']] != EXPECTED_NAMES
            ...
            or any(item.get('repeat_existing') is not True or item.get('prior_job_id') != PRIOR_JOB_ID
                   for item in plan['members'])):
        raise RuntimeError('The r15 protocol requires exactly the two unchanged r14 repeat members')
    declared = plan['measurement_protocol']
    if (declared['warmup_order'] != EXPECTED_NAMES or declared['warmup_suites_per_member'] != 1
            or declared['formal_round_order'] != [EXPECTED_NAMES, EXPECTED_NAMES[::-1], EXPECTED_NAMES]
        ...
        raise RuntimeError('The r15 predeclared warm-up/formal protocol changed')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing'):
            ...
            if prior.get('job_id') != PRIOR_JOB_ID:
                raise RuntimeError('The r15 repeat must follow the registered r14 job 1579730')
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r15 plan must preserve original implementation identity: ' + field)
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r15-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r15 measurements.',
    ...
    config['round_order'] = [names.copy(), names[::-1], names.copy()]
    exclusive_json(HERE / 'cohort-config.json', config)
    shutil.copy2(HERE / 'cohort-config.json', payload / 'cohort-config.json')
    for filename in ('cohort_driver.py', 'remote_job.sh', 'preflight_audit.py'):
        shutil.copy2(HERE / filename, payload / filename)
    for directory in ('preflight', 'preflight-wide32', 'reference'):
    ...
    for item in plan['members']:
        if helper.regular_bytes(ROOT / 'records/experiments/trsm' / (item['version'] + '.json')) != prior_bytes[item['version']]:
            raise RuntimeError('Prior record changed during preparation')
        helper.validate_repeat_existing((ROOT / '.runs/trsm' / item['run_name']).resolve(),
                                        cluster.read_json(ROOT / '.runs/trsm' / item['run_name'] / 'experiment.json'),
                                        priors[item['version']])
        for name in SOURCES:
            if ((payload / item['run_name'] / 'source' / name).read_bytes()
                    != (ROOT / '.runs/trsm' / item['source_from'] / 'source' / name).read_bytes()):
                raise RuntimeError('Frozen repeat source changed during preparation: ' + name)
