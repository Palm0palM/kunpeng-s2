...
#endif
#ifndef AX_OUTPUT_BEGIN
#define AX_OUTPUT_BEGIN(P,N) ((void)0)
#define AX_OUTPUT_END() 1
#endif
struct guarded {
    void *mapping;
...
static struct guarded allocate_guarded(size_t count, int pad, int leading) {
    ...
    g.mapping = mmap(NULL, g.mapping_bytes, PROT_NONE,
                     MAP_PRIVATE LOCAL_USER MAP_ANONYMOUS, -1, 0);
    if (g.mapping == MAP_FAILED) fail("mmap");
    g.usable = (unsigned char *)g.mapping + page;
    if (mprotect(g.usable, g.usable_bytes, PROT_READ LOCAL_USER PROT_WRITE)) fail("mprotect rw");
...
static int setup_config(int argc, char **argv) {
    if (argc != 2 LOCAL_USERLOCAL_USER !(getauxval(AT_HWCAP) & HWCAP_SVE)) {
    ...
    if (bad_workers LOCAL_USERLOCAL_USER (actual_threads != 1 && actual_threads != 4 && actual_threads != 38)) {
...
int main(int argc, char **argv) {
    ...
    int count = 0, core = 0, narrow = 0, small = 0, larger = 0, quad_boundary = 0;
    ...
    const int extra_heights[] = {1,7,13,28};
    for (int kh=6; kh<=8; ++kh)
    for (int kw=1; kw<=3; ++kw)
    ...
    const int small_widths[] = {1,3*lanes-1,3*lanes+1};
    ...
    const int larger_kernels[][2] = {{10,7},{9,8},{15,15},{81,81}};
    const int larger_heights[] = {7,13};
    ...
    for (int pad=0; pad<=1; ++pad)
    for (int leading=0; leading<=1; ++leading) {
        if (!one_case(3*lanes+1,larger_kernels[k][0],larger_kernels[k][1],larger_heights[h],pad,leading)) return 1;
        ++larger; ++count;
    }
    /* C54: actual four-column work, all u1 remainders, t and tile transitions. */
    ...
    for (int pad=0; pad<=1; ++pad)
    for (int leading=0; leading<=1; ++leading) {
        if (!one_case(widths[w],kh,kw,quad_heights[h],pad,leading)) return 1;
        ++quad_boundary; ++count;
    }
    if (core!=3888 LOCAL_USERLOCAL_USER narrow!=144 LOCAL_USERLOCAL_USER small!=720 LOCAL_USERLOCAL_USER larger!=32 LOCAL_USERLOCAL_USER quad_boundary!=960 LOCAL_USERLOCAL_USER count!=5744) return 6;
    printf("FULL_MATRIX_COUNTS core=%d narrow=%d small=%d larger=%d quad_boundary=%d\n",core,narrow,small,larger,quad_boundary);
