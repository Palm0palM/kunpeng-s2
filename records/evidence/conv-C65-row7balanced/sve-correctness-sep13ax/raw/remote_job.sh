...
finish() {
    ...
    trap - EXIT
    exec 1>&3 2>&4 3>&- 4>&-
    wait "$LOG_PID" LOCAL_USERLOCAL_USER log_code=$?
...
stage() {
    ...
    code=$?
    set -e
    printf 'STAGE=%s EXIT=%s\n' "$name" "$code" LOCAL_USER tee -a stage-exits.txt
    return "$code"
}
...
[[ "$CANDIDATE" == C65-row7balanced && "$EXPECTED_ACC" == 3 ]]
[[ "$EXPECTED_FULL" == 5744 && "$EXPECTED_DISPATCH" == 2724 && "$EXPECTED_DIRECT" == 432 && "$EXPECTED_TOTAL" == 80100 ]]
[[ "$EXPECTED_DISPATCH_ROWSEVEN_T1" == 4008 && "$EXPECTED_DISPATCH_ROWSEVEN_T4" == 6168 && "$EXPECTED_DISPATCH_ROWSEVEN_T38" == 24010 && "$EXPECTED_DIRECT_ROWSEVEN_ENTRIES" == 1080 ]]
printf 'CANDIDATE=%s\n' "$CANDIDATE"
date -u
hostname
compiler_check() {
    gcc --version LOCAL_USERLOCAL_USER return "$?"
    gcc -dumpfullversion -dumpversion > compiler-version.txt LOCAL_USERLOCAL_USER return "$?"
    [[ "$(cat compiler-version.txt)" == 10.3.1 ]]
}
stage compiler compiler_check
write_manifest() {
    sha256sum conv2d.c check_conv_guard.c check_sve_dispatch.c remote_job.sh candidate.env > source-sha256.txt LOCAL_USERLOCAL_USER return "$?"
    printf '%s  conv2d.c\n' a855c14b81c00f3d398ac36c5ece6726e15235f18ebf3fe4746570da25a7f874 LOCAL_USER sha256sum -c - LOCAL_USERLOCAL_USER return "$?"
...
run_guard() {
    ...
    OMP_NUM_THREADS="$threads" ./check_conv_guard "$bytes" LOCAL_USER tee "guard-vl${bytes}-t${threads}.log"
}
run_dispatch() {
    local bytes=$1 threads=$2
    OMP_NUM_THREADS="$threads" ./check_sve_dispatch "$bytes" LOCAL_USER tee "dispatch-vl${bytes}-t${threads}.log"
}
