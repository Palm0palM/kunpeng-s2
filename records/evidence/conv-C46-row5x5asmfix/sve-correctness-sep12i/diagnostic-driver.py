"""Prepared I diagnostic transport for C45/C46; explicit root --go only.
...
VERSIONS = ('C45-row4dup4', 'C46-row5x5asmfix')
SOURCE_NAMES = {'conv2d.c', 'check_conv_guard.c', 'check_sve_dispatch.c', 'candidate.env', 'remote_job.sh'}
SOURCE_PARENTS = {'C45-row4dup4': 'C26-row4loads', 'C46-row5x5asmfix': 'C42-row5x5asm'}
COUNTS = {
    'C45-row4dup4': dict(full_per_configuration=20164, dispatch_per_configuration=96,
        direct_per_configuration=0, configurations=6, total=121560),
    'C46-row5x5asmfix': dict(full_per_configuration=3776, dispatch_per_configuration=504,
        direct_per_configuration=288, configurations=6, total=27408),
}
...
# Neither current I wrapper builds a production object for separate transfer.
...
def serial_gate(cfg):
    """One bounded read-only query per actual job in this coordinator's sequence.

    Check J and any reserved H/I/K/package work; never scan teammate jobs or
    cancel/retry anything. An unknown reservation blocks instead of disappearing.
    This is not a global scheduler mutex; a single coordinator supplies --go.
    """
    identities = {'1579660': ['J performance']}
    unresolved = []
    ...
    # K confirmation or the later exact ZIP must not overlap I if already reserved.
    job_paths += [ROOT / '.runs/conv' / name / 'cluster.json'
        for name in ('C26-r18', 'C40-r2', 'C26-r19', 'C40-package')]
    ...
    for round_name in ('sep12h', 'sep12j', 'sep12k'):
        ...
        origin = str(path.relative_to(ROOT))
        if not re.fullmatch(r'[0-9]+', str(job_id or '')):
            unresolved.append(dict(path=origin, reason='Campaign exists without a reconciled performance job ID'))
            continue
        if round_name == 'sep12j' and str(job_id) != '1579660':
            unresolved.append(dict(path=origin, reason='J campaign differs from its known job1579660'))
        identities.setdefault(str(job_id), []).append(origin)
...
def frozen_source(base, version):
    ...
    # C45's original prepared schema has the simple map only; C46 also has bytes/SHA.
    ...
    if extra_manifest.exists():
        ...
        require(version == 'C45-row4dup4', 'C46 supplemental source-manifest.json is required')
    ...
    if version == 'C45-row4dup4':
        require(prepared.get('expected_acc') == 4 and prepared.get('check_rowtriple') == prepared.get('check_rowquad') == 1,
            'C45 helper instrumentation schema mismatch')
    else:
        require(prepared.get('expected_acc') == 5 and prepared.get('expected_checks') == counts,
            'C46 matrix schema mismatch')
        require(prepared.get('expected_rowquint_entries') == dict(dispatch_per_configuration=528, direct_per_configuration=720),
            'C46 quint entry expectations mismatch')
    return expected
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root I submission authorization')
    ...
    if args.action == 'submit':
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12i-' + args.version + '-' + created
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep12i-' + args.version)
    ...
    print('Fetched immutable I diagnostic text/assembly. Fetch is not acceptance or performance.')
