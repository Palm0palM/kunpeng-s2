"""Atomically freeze I evidence; local file copying/text inspection only.

Usage: python3 .runs/conv/sep12i-checks/freeze_returned.py VERSION [--failed]
...
EXPECTED={
    'C45-row4dup4':dict(full=20164,dispatch=96,direct=0,total=121560),
    'C46-row5x5asmfix':dict(full=3776,dispatch=504,direct=288,total=27408),
}
...
def validate_archive_input(source,version,failed):
    ...
    if failed:
        ...
        require(result.get('total_cases')==expected['total'] and result.get('runner_cases',0)==0
            ...
            'Accepted per-path/total counts differ from I plan')
...
def main():
    ...
    target=BASE.parent/args.version/'sve-correctness-sep12i'
    staging=target.with_name('.sve-correctness-sep12i.preparing')
