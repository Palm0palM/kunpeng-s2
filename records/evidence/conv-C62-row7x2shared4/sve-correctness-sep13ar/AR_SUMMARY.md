...
候选 C62-row7x2shared4，原作业 1590510。首次 status/fetch 均退出0，调度器 SUCCEEDED，job/system/wrapper = 0/0/0。
...
旧fallback计数仅按原AR checker要求五类均非零；以下为实际观测，不是预期值。没有沿用AP固定计数。
...
原始日志在 raw/；完整逐配置、source关联、资源及两次外层argv/UTC/输出路径见 AR_SUMMARY.json。用户已取消40%停止阈值；未使用reset。此子任务只读取日志，不执行本机题目或提交新作业。
