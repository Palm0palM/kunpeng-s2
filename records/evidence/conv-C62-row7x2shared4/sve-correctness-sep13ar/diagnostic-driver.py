"""Prepared AR diagnostic transport for C62; explicit root --go only.
...
VERSIONS = ('C62-row7x2shared4',)
SOURCE_SHA256 = 'e6cc9bdb4c14e9f6982f687123379ebf5dfb2013777f7bb076c04edd4827dc8a'
SOURCE_NAMES = {'conv2d.c', 'check_conv_guard.c', 'check_sve_dispatch.c', 'candidate.env', 'remote_job.sh'}
SOURCE_PARENTS = {'C62-row7x2shared4': 'C58-r1'}
AQ_JOB_ID = '1590254'
...
COUNTS = {'C62-row7x2shared4': dict(full_per_configuration=5744, dispatch_per_configuration=1212,
...
# The frozen AR wrapper builds no retained production object for separate transfer.
...
def completed_aq_evidence():
    path = ROOT / '.runs/conv/sep13aq-campaign.json'
    campaign = read_json(path)
    order = ['C58-r6', 'C61-row7shared2rowwise', 'C58-r7']
    require(campaign.get('performance_job') == AQ_JOB_ID and campaign.get('status') == 'complete'
        and campaign.get('measurement_order') == order and campaign.get('expected_benchmark_cases') == 36,
        'Original AQ must be completely recorded')
    ...
    require([r.get('version') for r in rows] == order
        ...
        'Original AQ all36 samples required')
    return dict(path=str(path.relative_to(ROOT)), job_id=AQ_JOB_ID, expected_samples=36, read_only=True)


def serial_gate(cfg):
    identities = {AQ_JOB_ID: ['.runs/conv/sep13aq-campaign.json']}
    unresolved = []
    try:
        completed_aq_evidence()
    ...
    if path.exists():
        ...
        if data.get('version') != VERSIONS[0] or not re.fullmatch(r'[0-9]+', job_id):
            unresolved.append('Original AR reservation unknown; reconcile without resubmission')
        else:
...
def frozen_source(base, version):
    ...
    require(set(expected) == SOURCE_NAMES and expected['conv2d.c'] == SOURCE_SHA256, 'Own C62 transport identity')
    source = base / 'source'
    ...
    require(record.get('version') == version and record.get('source_parent', record.get('parent')) == 'C58-r1'
        and record.get('status') == 'prepared' and record.get('verified') is False, 'Unmeasured C62 checkpoint')
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root AR submission authorization')
    ...
    if args.action == 'submit':
        ...
        aq_evidence = completed_aq_evidence()
        gate = serial_gate(cfg)
        save(base / ('gate-' + stamp() + '.json'), gate)
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep13ar-' + args.version + '-' + created
        data = dict(version=args.version, remote_dir=remote_dir, submitted=False,
            ...
            source_hashes=hashes, known_job_gate=gate, completed_aq_evidence=aq_evidence, explicit_go=True, no_automatic_continuation=True)
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep13ar-' + args.version)
    ...
    print('Fetched immutable AR diagnostic text/assembly. Fetch is not acceptance or performance.')
