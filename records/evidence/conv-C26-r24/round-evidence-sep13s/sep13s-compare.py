"""Judge only the independent S confirmation; keep all original36 samples.

No pooled R/S measurements, failed-confirmation retry, promotion or packaging.
"""
...
spec = importlib.util.spec_from_file_location('sep13s_record_checks', Path(__file__).with_name('sep13s-record-group.py'))
checks = importlib.util.module_from_spec(spec)
spec.loader.exec_module(checks)
prior_spec = importlib.util.spec_from_file_location('sep13s_r_comparison_definition', Path(__file__).with_name('sep13r-compare.py'))
...
def main():
    ...
    with e.locked():
        ...
        candidate['confirmation_samples_scope'] = 'Only36 original S samples; R initial qualification remains separate.'
        candidate['decision_note'] = ('Independent S confirmation passed both current S C6 gates. Return to root; no automatic promotion or ZIP.'
            if passed else 'Independent S confirmation failed at least one current S C6 gate. Preserve every original sample and C6; do not repeat failed confirmation or combine R/S samples.')
        ...
        # Only the three S records and S campaign are written. R is untouched.
        for name, record in records.items():
        ...
        plan.update(status='performance_complete', results=rows, confirmation_passed=passed,
            final_selection=None, confirmation_pending=[], prior_c40_decisions_unchanged=True,
    ...
    print(json.dumps(dict(round='sep13s', confirmation_passed=passed, results=rows,
        samples_mixed_with_initial_round=False, prior_c40_decisions_unchanged=True), ensure_ascii=False, indent=2))
