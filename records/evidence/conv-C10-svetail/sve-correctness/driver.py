...
import cluster
base=Path('.runs/conv/C10-svetail/sve-correctness')
cfg=cluster.load_config('config/conv-sep9.local.json')
cfg['scheduler']['walltime_seconds']=300
manifest=base/'job.json'
action=sys.argv[1]
if action=='submit':
    if manifest.exists(): raise SystemExit('Refusing repeat submission')
    remote_dir=cfg['remote_root']+'/diagnostics/conv-svetail-correctness-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command=cluster.submit_command(cfg,remote_dir,'conv-svetail-correctness')
