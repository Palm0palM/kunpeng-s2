...
/* Small-workspace full four-row solve: keep the historical increasing-k
 * FMA accumulators through the diagonal solve, avoiding a sums array.
 * Each in-block q contribution remains a separate multiply then add;
 * run.sh keeps -ffp-contract=off. Solved rows replace their accumulators
 * and are written to the same packed X before the next block. */
static inline void panel_solve4x8_neon(int start,int lda,const double *L,double *x)
{
    float64x2_t s0_0=vdupq_n_f64(0);
    float64x2_t s0_2=vdupq_n_f64(0);
    float64x2_t s0_4=vdupq_n_f64(0);
    ...
    {
        double *row=x+(size_t)(start+0)*RHS;
        float64x2_t diag=vdupq_n_f64(L[(size_t)0*lda+start+0]);
        s0_0=vdivq_f64(vsubq_f64(vld1q_f64(row+0),s0_0),diag);
        s0_2=vdivq_f64(vsubq_f64(vld1q_f64(row+2),s0_2),diag);
        s0_4=vdivq_f64(vsubq_f64(vld1q_f64(row+4),s0_4),diag);
        s0_6=vdivq_f64(vsubq_f64(vld1q_f64(row+6),s0_6),diag);
        vst1q_f64(row+0,s0_0);
        vst1q_f64(row+2,s0_2);
        vst1q_f64(row+4,s0_4);
        vst1q_f64(row+6,s0_6);
    }
    { double a=L[(size_t)1*lda+start+0];
        s1_0=vaddq_f64(s1_0,vmulq_n_f64(s0_0,a));
        s1_2=vaddq_f64(s1_2,vmulq_n_f64(s0_2,a));
        s1_4=vaddq_f64(s1_4,vmulq_n_f64(s0_4,a));
        s1_6=vaddq_f64(s1_6,vmulq_n_f64(s0_6,a));
    }
    {
        double *row=x+(size_t)(start+1)*RHS;
        float64x2_t diag=vdupq_n_f64(L[(size_t)1*lda+start+1]);
        s1_0=vdivq_f64(vsubq_f64(vld1q_f64(row+0),s1_0),diag);
        s1_2=vdivq_f64(vsubq_f64(vld1q_f64(row+2),s1_2),diag);
        s1_4=vdivq_f64(vsubq_f64(vld1q_f64(row+4),s1_4),diag);
        s1_6=vdivq_f64(vsubq_f64(vld1q_f64(row+6),s1_6),diag);
        vst1q_f64(row+0,s1_0);
        vst1q_f64(row+2,s1_2);
        vst1q_f64(row+4,s1_4);
        vst1q_f64(row+6,s1_6);
    }
    { double a=L[(size_t)2*lda+start+0];
        s2_0=vaddq_f64(s2_0,vmulq_n_f64(s0_0,a));
        s2_2=vaddq_f64(s2_2,vmulq_n_f64(s0_2,a));
        s2_4=vaddq_f64(s2_4,vmulq_n_f64(s0_4,a));
        s2_6=vaddq_f64(s2_6,vmulq_n_f64(s0_6,a));
    }
    { double a=L[(size_t)2*lda+start+1];
        s2_0=vaddq_f64(s2_0,vmulq_n_f64(s1_0,a));
        s2_2=vaddq_f64(s2_2,vmulq_n_f64(s1_2,a));
        s2_4=vaddq_f64(s2_4,vmulq_n_f64(s1_4,a));
        s2_6=vaddq_f64(s2_6,vmulq_n_f64(s1_6,a));
    }
    {
        double *row=x+(size_t)(start+2)*RHS;
        float64x2_t diag=vdupq_n_f64(L[(size_t)2*lda+start+2]);
        s2_0=vdivq_f64(vsubq_f64(vld1q_f64(row+0),s2_0),diag);
        s2_2=vdivq_f64(vsubq_f64(vld1q_f64(row+2),s2_2),diag);
        s2_4=vdivq_f64(vsubq_f64(vld1q_f64(row+4),s2_4),diag);
        s2_6=vdivq_f64(vsubq_f64(vld1q_f64(row+6),s2_6),diag);
        vst1q_f64(row+0,s2_0);
        vst1q_f64(row+2,s2_2);
        vst1q_f64(row+4,s2_4);
        vst1q_f64(row+6,s2_6);
    }
    { double a=L[(size_t)3*lda+start+0];
        s3_0=vaddq_f64(s3_0,vmulq_n_f64(s0_0,a));
        s3_2=vaddq_f64(s3_2,vmulq_n_f64(s0_2,a));
        s3_4=vaddq_f64(s3_4,vmulq_n_f64(s0_4,a));
        s3_6=vaddq_f64(s3_6,vmulq_n_f64(s0_6,a));
    }
    { double a=L[(size_t)3*lda+start+1];
        s3_0=vaddq_f64(s3_0,vmulq_n_f64(s1_0,a));
        s3_2=vaddq_f64(s3_2,vmulq_n_f64(s1_2,a));
        s3_4=vaddq_f64(s3_4,vmulq_n_f64(s1_4,a));
        s3_6=vaddq_f64(s3_6,vmulq_n_f64(s1_6,a));
    }
    { double a=L[(size_t)3*lda+start+2];
        s3_0=vaddq_f64(s3_0,vmulq_n_f64(s2_0,a));
        s3_2=vaddq_f64(s3_2,vmulq_n_f64(s2_2,a));
        s3_4=vaddq_f64(s3_4,vmulq_n_f64(s2_4,a));
        s3_6=vaddq_f64(s3_6,vmulq_n_f64(s2_6,a));
    }
    {
        double *row=x+(size_t)(start+3)*RHS;
        float64x2_t diag=vdupq_n_f64(L[(size_t)3*lda+start+3]);
        s3_0=vdivq_f64(vsubq_f64(vld1q_f64(row+0),s3_0),diag);
        s3_2=vdivq_f64(vsubq_f64(vld1q_f64(row+2),s3_2),diag);
        s3_4=vdivq_f64(vsubq_f64(vld1q_f64(row+4),s3_4),diag);
        s3_6=vdivq_f64(vsubq_f64(vld1q_f64(row+6),s3_6),diag);
        vst1q_f64(row+0,s3_0);
        vst1q_f64(row+2,s3_2);
        vst1q_f64(row+4,s3_4);
        vst1q_f64(row+6,s3_6);
    }
}
#endif
...
#if defined(__aarch64__)
                if(h==ROWS) {
                    panel_solve4x8_neon(ii,lda,L+(size_t)ii*lda,x);
                    continue;
                }
#endif
