...
NUMERIC_CANDIDATE = 'T9-neondirect'
NUMERIC_REFERENCE = 'T7-diagpanel'
...
NUMERIC_PREFLIGHT_FILES = ('guard.py', 'check-t7-t9.c', 'run.sh', 'README.md')
...
def prepare():
    ...
    numerical_candidates = [item for item in plan['members'] if item.get('version') == NUMERIC_CANDIDATE]
    if len(numerical_candidates) != 1:
        raise RuntimeError('This comparison requires exactly one T9-neondirect numerical preflight candidate')
    numeric_original = ROOT / '.runs/trsm' / NUMERIC_REFERENCE / 'source'
    for base, files in ((numeric_original, SOURCES), (HERE / 'preflight-t9', NUMERIC_PREFLIGHT_FILES)):
        for name in files:
            path = base / name
            if not path.is_file() or path.is_symlink():
                raise RuntimeError('Required numerical preflight input is missing or symlinked: ' + str(path))
    ...
    payload.mkdir()
    config = dict(cohort_id=group_id, members={}, numerical_preflight=dict(
        candidate_version=NUMERIC_CANDIDATE,
        candidate_run=numerical_candidates[0]['run_name'],
        reference_implementation=NUMERIC_REFERENCE,
        reference_source='numeric-reference/T7/source',
        reference_snapshot_files=list(SOURCES),
        runner='preflight-t9/run.sh',
        output_dir='preflight-t9-results/' + NUMERIC_CANDIDATE,
        completion_marker=NUMERIC_COMPLETE,
        purpose='T7 numerical reference only; performance comparison uses current promoted baseline'))
    # The historical T7 snapshot is only a numeric input, never a benchmark member.
    for name in SOURCES:
        destination = payload / 'numeric-reference/T7/source' / name
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(numeric_original / name, destination)
    for item in plan['members']:
        folder = ROOT / '.runs/trsm' / item['run_name']
    ...
    config['round_order'] = [names[i % len(names):] + names[:i % len(names)] for i in range(3)]
    ...
    for directory in ('preflight', 'preflight-t9', 'reference'):
...
def collect():
    ...
    for name, member in config['members'].items():
        ...
        for artifact in ('benchmark.log', 'environment.log', 'exit-code.txt', 'wrapper.stdout.log', 'linkage.log', 'runtime-settings.json'):
            ...
            (folder / artifact).write_bytes(result.stdout)
    ...
    script = 'import pathlib,tarfile,sys; root=pathlib.Path(sys.argv[1]); a=tarfile.open(fileobj=sys.stdout.buffer,mode="w|gz"); '\
             '[(a.add(p,arcname=str(p.relative_to(root)),recursive=False)) for p in sorted(root.rglob("*")) if p.is_file() and not p.is_symlink() and p.suffix in (".log",".txt",".tsv",".json",".s")]; a.close()'
