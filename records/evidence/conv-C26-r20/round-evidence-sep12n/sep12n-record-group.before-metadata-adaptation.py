"""Record only the saved N group; preserve all 96 original measurements.
...
spec = importlib.util.spec_from_file_location('sep12n_submission_contract', Path(__file__).with_name('sep12n-submit-performance.py'))
...
def verify_plan(plan):
    assert plan['round'] == 'sep12n' and plan['status'] in ('performance_running','performance_complete')
    ...
    assert plan['rowsix_reference'] == 'C40-r3'
    assert plan['suites_per_member'] == 3 and plan['expected_benchmark_cases'] == 96
    ...
    assert plan['auxiliary_comparisons'] == {'C47-row6x4u1':'C40-r3','C48-row8x2u1':'C40-r3','C49-row4dup4fence':'C45-row4dup4'}
    assert plan['prior_job'] == '1579748' and plan['prior_c40_decisions'] == contract.old_c40_decisions()
...
def verify_source_association(rec, plan):
    ...
    # Same frozen-source diagnostic association is checked for every candidate,
    # including C49's final explicit O contract; no new test or SSH occurs here.
...
def main():
    ...
    for name in names:
        ...
        assert metadata['source_hashes'] == e.source_files(run/'source')
        verify_source_association(metadata, plan)
