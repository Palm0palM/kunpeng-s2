...
```text
python3 .runs/diagnostics/conv-profile-sep12-r2/driver.py config/conv-sep12.local.json submit --go
python3 .runs/diagnostics/conv-profile-sep12-r2/driver.py config/conv-sep12.local.json status
python3 .runs/diagnostics/conv-profile-sep12-r2/driver.py config/conv-sep12.local.json fetch
```
...
Retry of job 1579436. Only remote allocation parsing now skips empty cpulist entries, as proven diagnostics already do. Original failed evidence remains in ../conv-profile-sep12. No build or operator ran in that failed attempt.

## Task-only revision r2: prepared, not submitted

Only change from r1 runtime: remove -C from perf record and its recorded command. Existing 38-core allocation, single NUMA, numactl and OMP38/close/cores binding remain. No -a, other-process attachment, sysctl or capability changes. C38 job1579469 task-only /bin/true probe exited0 (5 samples); this only supports trying task-only recording, not any operator metric.

Original1579436 failed in allocation parsing before compilation; r1 job1579456 compiled but its CPU-selected record was denied (exit255). Both original evidence trees remain unchanged. This r2 has no job.json and must wait for explicit root GO after F performance.
