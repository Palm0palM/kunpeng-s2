...
for kind in panel update; do
    ...
    gcc "${flags[@]}" -Dposix_memalign=trsm_test_alloc_fail -c "../T3-sve$kind-r3/source/trsm.c" -o "trsm-$kind-fail.o"
    gcc "${flags[@]}" check-trsm.c fail-alloc.c "trsm-$kind-fail.o" -lm -o "check-$kind-fail"
    OMP_NUM_THREADS=4 "./check-$kind-fail"
    gcc "${flags[@]}" -S "../T3-sve$kind-r3/source/trsm.c" -o "trsm-$kind.s"
done
