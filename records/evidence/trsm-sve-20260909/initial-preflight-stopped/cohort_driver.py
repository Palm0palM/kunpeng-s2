...
def main():
    ...
    probe = subprocess.run(['/bin/bash', './run-preflight.sh'], cwd=str(ROOT / 'preflight'),
                           env=probe_env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    (ROOT / 'preflight.log').write_bytes(probe.stdout)
    sys.stdout.buffer.write(probe.stdout); sys.stdout.buffer.flush()
    if probe.returncode:
