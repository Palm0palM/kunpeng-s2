...
set -uo pipefail
cd "$(dirname "$0")"
exec > >(tee environment-probe.log) 2>&1
date -u +%FT%TZ
hostname
uname -a
...
for folder in /opt/apps /usr/local /home/HPC; do
    if [[ -d "$folder" ]]; then
        find "$folder" -maxdepth 8 \( -iname '*kml*' -o -name 'libkblas*' -o -name 'kblas.h' \) -print 2>/dev/null
    fi
done
