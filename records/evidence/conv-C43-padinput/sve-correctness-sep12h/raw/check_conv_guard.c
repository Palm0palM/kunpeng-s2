...
int main(int argc, char **argv) {
    ...
    const int copy_success = argc == 3 && strcmp(argv[2], "copy-success") == 0;
    const int copy_failure = argc == 3 && strcmp(argv[2], "copy-failure") == 0;
    const int copy_only = copy_success || copy_failure;
#ifdef COPY_PATH_PROBE
    if (!copy_only) return 3;
    copy_probe_set_failure(copy_failure);
#else
    if (copy_only) return 3;
#endif
    int count = 0;
    if (smoke) {
        ...
        const int pair_kernels[][2] = {{1,1},{1,2},{1,3},{2,1},{2,2},{2,3},
                                       {3,2},{3,3},{4,7},{7,4},{7,8},{8,7},{4,4},{7,7}};
    ...
    /* Gate matrix is expressed in actual INPUT width, independently of ow. */
    if (!smoke) {
        const int input_widths[] = {63,64,65,79,80,81,95,96,97,127,128,129};
        const int gate_khs[] = {3,4,5};
        const int gate_ohs[] = {3,4,5,7,8,9};
        const int gate_kws[] = {1,2,3,7,16,31,63};
        for (unsigned w = 0; w < sizeof(input_widths)/sizeof(*input_widths); ++w)
        for (unsigned k = 0; k < sizeof(gate_khs)/sizeof(*gate_khs); ++k)
        for (unsigned h = 0; h < sizeof(gate_ohs)/sizeof(*gate_ohs); ++h)
        for (unsigned q = 0; q < sizeof(gate_kws)/sizeof(*gate_kws); ++q)
        for (int pad = 0; pad <= 1; ++pad)
        for (int leading = 0; leading <= 1; ++leading) {
            const int ow = input_widths[w] - gate_kws[q] + 1;
            if (!one_case(ow, gate_khs[k], gate_kws[q], gate_ohs[h], pad, leading)) return 1;
            ++count;
        }
    }
#ifdef COPY_PATH_PROBE
    if (!copy_probe_report(6048, 2800, copy_failure ? 0 : 28280)) return 7;
#endif
    ...
    return count == (smoke ? 96 : (copy_only ? 6048 : 24100)) ? 0 : 6;
}
