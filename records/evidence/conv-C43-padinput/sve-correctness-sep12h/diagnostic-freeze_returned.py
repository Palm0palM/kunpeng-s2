"""Atomically freeze H evidence; local file copying/text inspection only.

Usage: python3 .runs/conv/sep12h-checks/freeze_returned.py VERSION [--failed]
...
ATTACHMENTS={
    'diagnostic-driver.py':'diagnostic-driver.py',
...
def validate_archive_input(source,version,failed):
    ...
    require(prepared.get('total_cases_planned')==217176 and prepared.get('runner_cases_planned')==0,
        'Frozen prepared plan mismatch')
    require(isinstance(hashes,dict) and len(hashes)==10 and job.get('source_hashes')==hashes,
        'Original prepared/job source identity missing or inconsistent')
    # Check the actual saved scheduler text, without connecting to any server.
    ...
    if failed:
        ...
        # Missing .s is valid for compilation/allocation failure; never synthesize it.
        # A parse-only rejection is not silently converted to an executed-job failure.
        nonzero_stage=False
        stage_path=source/'raw/stage-exits.txt'
        if stage_path.exists():
            nonzero_stage=any(int(value)!=0 for value in re.findall(r'^STAGE=\S+ EXIT=(-?\d+)$',stage_path.read_text(),re.M))
        require(scheduler['status']!='SUCCEEDED' or scheduler['jobExitCode']!=0
            ...
            or nonzero_stage,'No actual scheduler/wrapper/stage failure; do not turn acceptance errors into fake failed runs')
    else:
        ...
        require(scheduler['status']=='SUCCEEDED' and scheduler['jobExitCode']==0
            and scheduler['systemExitCode']==0 and wrapper_exit==0,'PASS needs both scheduler exits and wrapper exit zero')
        require(result.get('total_cases')==217176 and result.get('runner_cases')==0
            and result.get('production_guard_cases')==144600
            and result.get('copy_success_cases')==36288 and result.get('copy_failure_cases')==36288,
            'Accepted per-path/total counts differ from H plan')
        configurations=result.get('configurations') or []
        require(len(configurations)==6 and
            [(row.get('sve_bytes'),row.get('threads')) for row in configurations]==
            [(vl,t) for vl in (16,32,64) for t in (1,4)],'Expected six real VL/thread configurations')
        require(all(row.get('passed') is True and row.get('production_guard_cases')==24100
            and row.get('copy_success_cases')==6048 and row.get('copy_failure_cases')==6048
            for row in configurations),'Missing accepted per-configuration counts')
        require(result.get('source_hashes_verified') is True and
            result.get('source_manifest_remote_matches') is True and result.get('source_hashes')==hashes,
            'Accepted frozen-source verification missing')
        require((source/'raw/conv2d-sve.s').is_file(),'PASS archive requires actual production assembly')
    return result
...
def main():
    ...
    target=BASE.parent/args.version/'sve-correctness-sep12h'
    staging=target.with_name('.sve-correctness-sep12h.preparing')
