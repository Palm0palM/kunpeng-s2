                             ...
                             float *restrict dst2, float *restrict dst3, int ow)
{
    ...
    for (; ow - i >= block; i += block) {
        ...
        svfloat32_t d5 = svdup_n_f32(0.0f);
        /* Input row 0 starts output 0. */
        {
            ...
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ak = svdup_n_f32(ka[ik]);
                /* One source-level input lifetime at a time; GCC may reschedule. */
                {
                ...
                {
                    const svfloat32_t v = svld1(pg, row + ik + 5 * lanes);
                    a5 = svadd_f32_x(pg, a5, svmul_f32_x(pg, v, ak));
                }
            }
        }
        /* Input row 1 advances output 0 and starts output 1. */
        {
            ...
            for (int ik = 0; ik < kw; ++ik) {
                ...
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 5 * lanes);
                    a5 = svadd_f32_x(pg, a5, svmul_f32_x(pg, v, ak));
                    b5 = svadd_f32_x(pg, b5, svmul_f32_x(pg, v, bk));
                }
            }
        }
        /* Input row 2 advances outputs 0/1 and starts output 2. */
        {
            ...
            for (int ik = 0; ik < kw; ++ik) {
                ...
                }
                {
                    ...
                    a5 = svadd_f32_x(pg, a5, svmul_f32_x(pg, v, ak));
                    b5 = svadd_f32_x(pg, b5, svmul_f32_x(pg, v, bk));
                    c5 = svadd_f32_x(pg, c5, svmul_f32_x(pg, v, ck));
                }
            }
        }
        /* Each middle input row advances all four outputs in strict kernel-column order. */
        for (int t = 3; t < kh; ++t) {
            const float *row = base + (size_t)t * stride + i;
            const float *ka = kernel + ((size_t)t) * kw;
            const float *kb = kernel + ((size_t)(t - 1)) * kw;
            ...
            for (int ik = 0; ik < kw; ++ik) {
                ...
                }
                {
                    ...
                    b5 = svadd_f32_x(pg, b5, svmul_f32_x(pg, v, bk));
                    c5 = svadd_f32_x(pg, c5, svmul_f32_x(pg, v, ck));
                    d5 = svadd_f32_x(pg, d5, svmul_f32_x(pg, v, dk));
                }
            }
        }
        ...
        /* Input row kh+1 finishes output 2 and advances output 3; widen before addition. */
        {
            const float *row = base + ((size_t)kh + 1) * stride + i;
            const float *kc = kernel + ((size_t)(kh - 1)) * kw;
        ...
        /* Input row kh+2 finishes output 3; widen before addition. */
        {
