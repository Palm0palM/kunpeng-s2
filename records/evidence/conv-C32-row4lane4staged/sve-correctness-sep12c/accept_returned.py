...
base = Path(__file__).resolve().parent / 'C32-row4lane4staged'
raw = base / 'raw'
...
plan = dict(full_per_configuration=20164, smoke_per_configuration=96, configurations=6, total=121560)
...
for sve_bytes in (16, 32, 64):
    for threads in (1, 4):
        ...
        texts = []
        for kind, count in (('guard', 20164), ('dispatch', 96)):
        ...
        configs.append(dict(sve_bytes=sve_bytes, threads=threads, lanes=lanes,
                            block_outputs_per_row=lanes*4, full_cases=20164,
                            smoke_cases=96, helper_entries=entries, passed=True))
...
result = dict(status='passed', complete=True, candidate=job['version'], job_id=job['job_id'],
    ...
    configurations=configs, full_cases=120984, smoke_cases=576, total_cases=121560,
    coverage_extension=dict(kernel_shapes_added=[[4,5],[4,6]], purpose='Four-column core followed separately by remainder1/remainder2; remainder0/remainder3 coverage retained.'),
...
print(json.dumps(dict(candidate=job['version'], job_id=job['job_id'], complete=True, total_cases=121560, configurations=configs), indent=2))
