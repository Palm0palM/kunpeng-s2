...
if len(sys.argv) != 4 or sys.argv[1] not in ('C32-row4lane4staged',) or sys.argv[3] not in ('submit', 'status', 'fetch'):
    raise SystemExit('Usage: python3 .runs/conv/sep12c-checks/driver.py VERSION VERIFIED_CONFIG submit|status|fetch')
version, config, action = sys.argv[1:]
expected_full_cases = {'C32-row4lane4staged': 20164}[version]
...
if action == 'submit':
    ...
    remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12c-' + version + '-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command = cluster.submit_command(cfg, remote_dir, 'conv-sep12c-' + version)
