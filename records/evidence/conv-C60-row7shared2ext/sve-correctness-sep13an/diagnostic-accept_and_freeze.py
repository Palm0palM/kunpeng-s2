"""AN returned-log acceptance/archive only; no operator or network execution."""
from pathlib import Path
import argparse,hashlib,json,re,shlex,shutil,datetime
BASE=Path(__file__).resolve().parent
ROOT=BASE.parents[2]
VERSION='C60-row7shared2ext'
SOURCE_SHA256='10df62b8d43c59a6164de7d23c1b59ab954bbaaebc330d57662e8f5d6b7f2168'
B=BASE/VERSION
RAW=B/'raw'
FROZEN=ROOT/'.runs/conv'/VERSION/'sve-correctness-sep13an'
def read(p):return json.loads(p.read_text())
def need(ok,why):
    if not ok:raise ValueError(why)
def main():
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--job-id',required=True,help='Original AN ID actually saved after root one-shot submission')
    job_id=parser.parse_args().job_id
    need(re.fullmatch(r'[0-9]+',job_id) and job_id != '1589364','Own original AN ID required')
    need(not FROZEN.exists() and not (B/'validation.json').exists(),'Already accepted/frozen; do not overwrite')
    s=read(B/'AN_SUMMARY.json');j=read(B/'job.json');prepared=read(B/'prepared.json')
    ...
    need(hashes.get('conv2d.c')==SOURCE_SHA256,'Own C60 reviewed source identity')
    ...
    print('AN'+job_id+' accepted/frozen44328; performance unmeasured.')
