"""Prepared AN diagnostic transport for C60; explicit root --go only.
...
VERSIONS = ('C60-row7shared2ext',)
SOURCE_SHA256 = '10df62b8d43c59a6164de7d23c1b59ab954bbaaebc330d57662e8f5d6b7f2168'
SOURCE_NAMES = {'conv2d.c', 'check_conv_guard.c', 'check_sve_dispatch.c', 'candidate.env', 'remote_job.sh'}
SOURCE_PARENTS = {'C60-row7shared2ext': 'C58-r1'}
AM_JOB_ID = '1589364'
...
COUNTS = {'C60-row7shared2ext': dict(full_per_configuration=5744, dispatch_per_configuration=1212,
...
# The frozen AN wrapper builds no retained production object for separate transfer.
...
def completed_am_evidence():
    path = ROOT / '.runs/conv/sep13am-campaign.json'
    campaign = read_json(path)
    order = ['C58-r2', 'C59-row7boundaryu2nofive', 'C58-r3']
    require(campaign.get('performance_job') == AM_JOB_ID and campaign.get('status') == 'complete'
        and campaign.get('measurement_order') == order and campaign.get('expected_benchmark_cases') == 36,
        'Original AM must be completely recorded')
    ...
    require([r.get('version') for r in rows] == order
        ...
        'Original AM all36 samples required')
    return dict(path=str(path.relative_to(ROOT)), job_id=AM_JOB_ID, expected_samples=36, read_only=True)


def serial_gate(cfg):
    identities = {AM_JOB_ID: ['.runs/conv/sep13am-campaign.json']}
    unresolved = []
    try:
        completed_am_evidence()
    ...
    if path.exists():
        ...
        if data.get('version') != VERSIONS[0] or not re.fullmatch(r'[0-9]+', job_id):
            unresolved.append('Original AN reservation unknown; reconcile without resubmission')
        else:
...
def frozen_source(base, version):
    ...
    require(set(expected) == SOURCE_NAMES and expected['conv2d.c'] == SOURCE_SHA256, 'Own C60 transport identity')
    source = base / 'source'
    ...
    require(record.get('version') == version and record.get('source_parent', record.get('parent')) == 'C58-r1'
        and record.get('status') == 'prepared' and record.get('verified') is False, 'Unmeasured C60 checkpoint')
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root AN submission authorization')
    ...
    if args.action == 'submit':
        ...
        am_evidence = completed_am_evidence()
        gate = serial_gate(cfg)
        save(base / ('gate-' + stamp() + '.json'), gate)
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep13an-' + args.version + '-' + created
        data = dict(version=args.version, remote_dir=remote_dir, submitted=False,
            ...
            source_hashes=hashes, known_job_gate=gate, completed_am_evidence=am_evidence, explicit_go=True, no_automatic_continuation=True)
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep13an-' + args.version)
    ...
    print('Fetched immutable AN diagnostic text/assembly. Fetch is not acceptance or performance.')
