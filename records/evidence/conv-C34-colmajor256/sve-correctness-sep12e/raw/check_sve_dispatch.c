...
#undef main
#include "conv2d.c"
static unsigned long prefix_entries, tail_entries, rowpair_entries, worker_entries[4];
#if CHECK_ROWTRIPLE
...
void __cyg_profile_func_enter(void *fn, void *caller) {
    (void)caller;
    if (fn == (void *)conv_sve_prefix || fn == (void *)conv_sve_tail ||
        fn == (void *)conv_sve_rowpair || fn == (void *)conv_sve_rowtriple ||
        fn == (void *)conv_sve_rowquad) {
        const int tid = omp_get_thread_num();
        if (tid >= 0 && tid < 4) __atomic_fetch_add(&worker_entries[tid], 1, __ATOMIC_RELAXED);
    }
...
int main(int argc, char **argv) {
    ...
    int workers_ok = workers == 1 || workers == 4;
    for (int i = 0; i < workers && i < 4; ++i) {
        const unsigned long calls = __atomic_load_n(&worker_entries[i], __ATOMIC_RELAXED);
        printf("SVE_WORKER_%d_ACTUAL_ENTRIES=%lu\n", i, calls);
        workers_ok = workers_ok && calls != 0;
    }
    return rc ? rc : (pair && triple_ok && quad_ok && workers_ok ? 0 : 4);
}
