#!/usr/bin/env python3
"""Register collected r17 baseline logs without running task code or computing digests."""
...
with (HERE / 'finish-attempt.json').open('x') as handle:
    ...
    handle.write('\n')
spec = importlib.util.spec_from_file_location('r17_baseline_finish_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
names = ['T19-control13']
versions = ['T19-control13']
if (list(config['members']) != names or config['round_order'] != [names.copy() for _ in range(3)]
        or config['warmup_order'] != names or config['warmup_suites_per_member'] != 1
        or config['warmup_protocol'].get('protocol_id') != 'trsm-r17-baseline-one-official-warmup-v1'
    ...
    raise SystemExit('r17 baseline protocol identity changed')
for index, (name, version) in enumerate(zip(names, versions)):
    if config['members'][name].get('version') != version or config['members'][name].get('repeat_existing') is not False:
        raise SystemExit('baseline requires a new parent-null T19 measurement')
...
for name, version in zip(names, versions):
    ...
    if (prepared_bytes[version] != helper.regular_bytes(run / 'experiment.json')
            or meta.get('version') != version or meta.get('parent') is not None
            or meta.get('source_from') != 'T19-panel8x16budget'
            or meta.get('source_implementation_id') != 'T19-panel8x16budget'
            or meta.get('status') != 'prepared' or meta.get('verified') is not False
            or meta.get('recorded_at') is not None or meta.get('repeat_existing', False)):
        raise SystemExit('New baseline prepared identity changed')
    if json.loads((ROOT / 'records/best.json').read_text()).get('trsm') != 'T19-panel8x16budget':
        raise SystemExit('Origin is no longer current best')
    if helper.regular_bytes(ROOT / 'trsm/trsm.c') != helper.regular_bytes(run / 'source/trsm.c'):
        raise SystemExit('Current implementation changed before baseline registration')
...
if audit['measured_official_pass_count'] != 9 or audit['warmup_official_pass_count'] != 3:
    raise SystemExit('Expected 9 measured and 3 warm-up official case passes')
...
for index, (name, version) in enumerate(zip(names, versions)):
    expected_record = prepared_bytes[version]
...
record = json.loads((ROOT / 'records/experiments/trsm/T19-control13.json').read_text())
summary = dict(status='recorded_baseline_pending_root_review', job_id=GROUP['job_id'],
               version='T19-control13', source_implementation_id='T19-panel8x16budget',
               parent=None, formal_pass_count=9, warmup_pass_count=3,
               fresh_preflight_executed=False, historical_preflight_job='1579861',
               cases=record['cases'], total_median_ms=record['total_median_ms'],
...
with (HERE / 'finish-complete.json').open('x') as handle:
    json.dump(dict(job_id=GROUP['job_id'], status='recorded_baseline_pending_root_review',
                   version='T19-control13', formal_pass_count=9, warmup_pass_count=3,
                   promoted=False), handle, indent=2)
    handle.write('\n')
print('Recorded unchanged T19 baseline; promotion requires explicit root review.')
