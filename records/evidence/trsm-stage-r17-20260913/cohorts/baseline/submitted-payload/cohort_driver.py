...
def main():
    ...
    if (names != ['T19-control13']
            or order != [names.copy() for i in range(3)]):
        raise RuntimeError('The baseline protocol requires three independent unchanged-source suites')
    for index, (name, version) in enumerate(zip(names, ('T19-control13',))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not False):
            raise RuntimeError('The baseline must retain its new parent-null measurement identity')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r17-baseline-one-official-warmup-v1'
    ...
    # No task source was changed: fresh baseline timing follows the actual KML probe.
    handles = {}
    try:
        member_details = {}
        for name in names:
