...
def main():
    ...
    if (names != ['T19-control13-repeat-r17', 'T20-wideunroll2']
            or order != [names, names[::-1], names]):
        raise RuntimeError('The r17 protocol requires A/B, B/A, A/B formal suites')
    for index, (name, version) in enumerate(zip(names, ('T19-control13', 'T20-wideunroll2'))):
        if (config['members'][name].get('version') != version
            ...
            raise RuntimeError('The r17 pair must retain its repeat/new identities')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r17-one-official-warmup-v1'
