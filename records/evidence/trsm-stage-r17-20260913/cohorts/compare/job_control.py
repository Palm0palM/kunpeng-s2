...
EXPECTED_NAMES = ['T19-control13-repeat-r17', 'T20-wideunroll2']
EXPECTED_VERSIONS = ['T19-control13', 'T20-wideunroll2']
PRIOR_JOB_ID = '1581760'
...
def record_helpers():
    ...
    spec = importlib.util.spec_from_file_location('r17_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
def prepare():
    if PREPARE_REVIEW_PENDING:
        raise RuntimeError('r17 preparation is blocked pending root independent review')
    ...
    if ([item['run_name'] for item in plan['members']] != EXPECTED_NAMES
        ...
        raise RuntimeError('The r17 protocol requires one unchanged baseline repeat and the new T20 member')
    ...
    if (declared['warmup_order'] != EXPECTED_NAMES or declared['warmup_suites_per_member'] != 1
        ...
        raise RuntimeError('The r17 predeclared warm-up/formal protocol changed')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing'):
            ...
            if prior.get('job_id') != PRIOR_JOB_ID:
                raise RuntimeError('The r17 repeat must follow the registered r17 baseline job 1581760')
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r17 plan must preserve original implementation identity: ' + field)
        ...
        if item.get('repeat_existing'):
            ...
            if item['source_from'] != item['version'] or item['run_name'] != item['version']:
                raise RuntimeError('The new T20 member must use its prepared source directory')
        original = ROOT / '.runs/trsm' / item['source_from'] / 'source'
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r17-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after full CT64 wide32 preflight, to apply the '
                'startup-warming protocol declared before viewing r17 measurements.',
