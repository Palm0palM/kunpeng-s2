#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Archive existing r17 evidence once; never execute or synthesize tests."""
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r17-20260913'
COHORTS = {
    'baseline': ('optimization-20260913-r17-baseline', ['T19-control13']),
    'compare': ('optimization-20260913-r17-compare', ['T19-control13-repeat-r17', 'T20-wideunroll2']),
}
...
def main():
    ...
    base = RUNS / COHORTS['baseline'][0]
    compare = RUNS / COHORTS['compare'][0]
    for name in ('T19-control13', 'T20-wideunroll2'):
        add(ROOT / 'records/experiments/trsm' / (name + '.json'),
            Path('records/latest') / (name + '.json'), required=True)
    add(base / 'baseline-record.json', Path('records/baseline-initial/T19-control13.json'), required=True)
    prior_path = RUNS / 'T19-control13-repeat-r17/prior-record.json'
    add(prior_path, Path('members/T19-control13-repeat-r17/prior-record.json'), required=True)
    prior, initial = obj(prior_path), obj(base / 'baseline-record.json')
    associated = (prior['version'] == initial['version'] == 'T19-control13'
                  and prior['job_id'] == initial['job_id'] == '1581760'
                  and states['baseline']['submission']['job_id'] == '1581760')
    if not associated:
        raise RuntimeError('The repeat prior is not associated with its fresh baseline')
    for name in ('PREPARATION.md', 'VALIDATION-PLAN.md', 'STATIC-REVIEW.md'):
        add(RUNS / 'T20-wideunroll2' / name, Path('members/T20-wideunroll2') / name, required=True)
    ...
    add(Path(__file__), Path('tools/archive-r17.py'), required=True)
    if missing:
    ...
    manifest = dict(schema_version=1, stage='r17-20260913', problem='trsm',
                    created_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                    states=states, repeat_prior_association_valid=associated,
                    baseline_initial_record='records/baseline-initial/T19-control13.json',
    ...
    (DEST / 'README.md').write_text('''# TRSM r17 evidence

This redacted public derivative preserves the fresh T19-control13 baseline and
the subsequent same-allocation T19/T20 comparison. Each cohort retains its own
actual job, source snapshots, commands, official/warmup logs and results.
Current preparation and submitted-payload snapshots are stored separately.

The repeat's full prior refers to baseline job 1581760 and the initial promoted
record under records/baseline-initial. Historical samples are never pooled with
the comparison. T20 changes only the SVE 4x32 update k loop; unchanged small-path
validation is explicitly historical (job 1579861), not a fresh preflight claim.
Both comparison members run the complete CT64 wide32 direct/whole/argument tests.

The archive only copies existing evidence; its creation does not validate tests,
promote a candidate or submit a contest entry. Results and failures remain as
observed. No local task compilation/execution or hash operations. Actual reference
is KML25.1/GCC12, not the specified KML25.2 revalidation. Private originals remain
available locally, and this destination must never be reused in place.
''')
    print('Archived', len(inventory), 'r17 text files; prior association valid, required missing=0')
