    ...
    for (; ow - i >= block; i += block) {
        ...
        for (int jk = 0; jk < kh; ++jk) {
            ...
            for (; ik < kw - 1; ik += 2) {
                ...
                /* Finish each output vector's ordered pair before loading the
                 * next neighbor. vN is dead after ext(vN, vN+1), allowing a
                 * shorter input lifetime without changing any lane's sums. */
                ...
                /* A full v8 load could cross the last valid input column. */
