...
version = sys.argv[1]
full_count, smoke_count, probe_count = {'C37-row4u1': (18052,96,0)}[version]
...
plan = dict(full_per_configuration=full_count, smoke_per_configuration=smoke_count, prefetch_probe_per_configuration=probe_count, configurations=6, total=(full_count+smoke_count+probe_count)*6, runner_cases=0)
...
result = dict(status='passed', complete=True, candidate=job['version'], job_id=job['job_id'],
    ...
    coverage_extension=dict(purpose='C37: unchanged standard matrix, shared single-column loop and original paired first/trailing phases.'),
    executed_locally=False, executed_remotely=True, sanitizer='not_run', issues=[],
    assembly=assembly, evidence_directory='raw')
pmu_file = raw / 'pmu-metadata.log'
if pmu_file.exists():
    pmu = pmu_file.read_text()
    statuses = {name: int(value) for name,value in re.findall(r'^PERF_(COMMAND|PARANOID|STAT)_EXIT=(\d+)$',pmu,re.M)}
    events = {event: int(value) for value,event in re.findall(r'^(\d+),,(cycles(?::u)?|instructions(?::u)?),',pmu,re.M)}
    paranoid = re.search(r'\+ cat /proc/sys/kernel/perf_event_paranoid\n(-?\d+)\n',pmu)
    result['pmu_metadata'] = dict(command='/bin/true', status_codes=statuses, perf_event_paranoid=int(paranoid[1]) if paranoid else None, events=events, available_for_probe=statuses.get('STAT') == 0 and len(events) == 2, correctness_gate=False, note='Probe only establishes current allocated-node user-event availability; counts are for /bin/true, not the operator.')
