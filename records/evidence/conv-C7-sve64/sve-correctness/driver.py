...
import cluster
base=Path('.runs/conv/C7-sve64/sve-correctness')
...
if action=='submit':
    ...
    remote_dir=cfg['remote_root']+'/diagnostics/conv-sve-correctness-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command=cluster.submit_command(cfg,remote_dir,'conv-sve-correctness')
    ...
    match=re.search(cfg['scheduler']['job_id_pattern'],out)
    if not match: raise SystemExit('Submission uncertain; inspect submit.log, do not repeat')
    data.update(job_id=match.group(1),submitted=True)
    manifest.write_text(json.dumps(data,indent=2)+'\n')
