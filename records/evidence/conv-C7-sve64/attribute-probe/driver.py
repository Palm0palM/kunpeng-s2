...
import cluster
base=Path('.runs/conv/C7-sve64/attribute-probe')
...
if action=='submit':
    ...
    remote_dir=cfg['remote_root']+'/diagnostics/conv-sve-attribute-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command=cluster.submit_command(cfg,remote_dir,'conv-sve-attribute')
