...
/* Twenty-four independent rows reuse each X vector. The k chain for every
 * lane and the final C-sum subtraction match the existing SVE kernels. */
__attribute__((target("arch=armv8-a+sve"), noinline))
static void update24x8_sve(int count,const double *L,int lda,const double *X,int ldx,double *C,int ldc)
{
    svbool_t pg=svptrue_b64();
    svfloat64_t a0=svdup_n_f64(0);
    svfloat64_t a1=svdup_n_f64(0);
    ...
    svfloat64_t a14=svdup_n_f64(0);
    svfloat64_t a15=svdup_n_f64(0);
    svfloat64_t a16=svdup_n_f64(0);
    svfloat64_t a17=svdup_n_f64(0);
    svfloat64_t a18=svdup_n_f64(0);
    svfloat64_t a19=svdup_n_f64(0);
    svfloat64_t a20=svdup_n_f64(0);
    svfloat64_t a21=svdup_n_f64(0);
    svfloat64_t a22=svdup_n_f64(0);
    svfloat64_t a23=svdup_n_f64(0);
    for(int k=0;k<count;++k) {
        svfloat64_t x=svld1_f64(pg,X+(size_t)k*ldx);
        a0=svmla_n_f64_x(pg,a0,x,L[(size_t)0*lda+k]);
        a1=svmla_n_f64_x(pg,a1,x,L[(size_t)1*lda+k]);
        ...
        a14=svmla_n_f64_x(pg,a14,x,L[(size_t)14*lda+k]);
        a15=svmla_n_f64_x(pg,a15,x,L[(size_t)15*lda+k]);
        a16=svmla_n_f64_x(pg,a16,x,L[(size_t)16*lda+k]);
        a17=svmla_n_f64_x(pg,a17,x,L[(size_t)17*lda+k]);
        a18=svmla_n_f64_x(pg,a18,x,L[(size_t)18*lda+k]);
        a19=svmla_n_f64_x(pg,a19,x,L[(size_t)19*lda+k]);
        a20=svmla_n_f64_x(pg,a20,x,L[(size_t)20*lda+k]);
        a21=svmla_n_f64_x(pg,a21,x,L[(size_t)21*lda+k]);
        a22=svmla_n_f64_x(pg,a22,x,L[(size_t)22*lda+k]);
        a23=svmla_n_f64_x(pg,a23,x,L[(size_t)23*lda+k]);
    }
    svst1_f64(pg,C+(size_t)0*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)0*ldc),a0));
    svst1_f64(pg,C+(size_t)1*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)1*ldc),a1));
    svst1_f64(pg,C+(size_t)2*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)2*ldc),a2));
    ...
    svst1_f64(pg,C+(size_t)14*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)14*ldc),a14));
    svst1_f64(pg,C+(size_t)15*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)15*ldc),a15));
    svst1_f64(pg,C+(size_t)16*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)16*ldc),a16));
    svst1_f64(pg,C+(size_t)17*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)17*ldc),a17));
    svst1_f64(pg,C+(size_t)18*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)18*ldc),a18));
    svst1_f64(pg,C+(size_t)19*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)19*ldc),a19));
    svst1_f64(pg,C+(size_t)20*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)20*ldc),a20));
    svst1_f64(pg,C+(size_t)21*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)21*ldc),a21));
    svst1_f64(pg,C+(size_t)22*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)22*ldc),a22));
    svst1_f64(pg,C+(size_t)23*ldc,svsub_f64_x(pg,svld1_f64(pg,C+(size_t)23*ldc),a23));
}
#endif
...
#if TRSM_CAN_DISPATCH_SVE
                if(use_sve) {
                    for(;ie-first_row>=24;first_row+=24) {
                        for(int j=jb;j<je;j+=8) {
                            int cols=je-j<8?je-j:8;
                            const double *xp=packed?packed+(size_t)(j/RHS)*KB*RHS:B+(size_t)kk*ldb+j;
                            int xstride=packed?RHS:ldb;
                            if(cols==8) {
                                update24x8_sve(end-kk,L+(size_t)first_row*lda+kk,lda,xp,xstride,B+(size_t)first_row*ldb+j,ldb);
                            } else {
                                for(int r=0;r<24;++r) {
                                    double sum[8]={0};
                                    for(int k=kk;k<end;++k) {
