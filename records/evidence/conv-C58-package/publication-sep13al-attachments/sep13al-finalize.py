"""Root C7 finalization after real AI/AK and exact original ZIP PASS. No network."""
import argparse,importlib.util,json,shutil,zipfile,io,hashlib
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]
spec=importlib.util.spec_from_file_location('al_contract',Path(__file__).with_name('sep13al-submit-package.py'))
...
def verified():
    ...
    contract.need(str(rec['job_id']) not in ['1583350','1583408',str(confirm['performance_job'])],'Independent package allocation')
...
def main():
    ...
    rec=e.get_record('conv',contract.CONFIRMATION);base=e.get_record('conv','C26-r39');opening=e.get_record('conv','C26-r38')
    lineage=e.read_json(ROOT/'records/conv-lineage.json');contract.need(lineage['current_label']=='C6' and not any(x['label']=='C7' for x in lineage['versions']),'C7 not yet finalized')
    gain=rec['comparison']['total_gain_pct'];summary=dict(mode='apply' if args.apply else 'read-only',candidate=contract.CONFIRMATION,new_label='C7',confirmation_job=confirm['performance_job'],package_job=pack['job_id'],total_median_ms=rec['total_median_ms'],baseline_ms=base['total_median_ms'],gain_pct=gain)
    print(json.dumps(summary,indent=2))
    if not args.apply:return
    with e.locked():
        contract.need(verified()==(initial,confirm,source,pack,package,archive),'Evidence changed before promotion')
        backup=ROOT/'.runs/conv/sep13al-before-promotion';backup.mkdir()
        ...
        e.promote(argparse.Namespace(problem='conv',version='C26-r39'))
        ...
        lineage['versions'].append(dict(label='C7',experiment=contract.CONFIRMATION,source_experiment=contract.SOURCE,parent_label='C6',comparison_baseline=base['version'],kernel_sha256=source['conv2d.c'],total_median_ms=rec['total_median_ms'],time_reduction_pct=gain,strategy='Seven rows x three SVE vectors; shared columns in ordered pairs, twelve boundary loops unroll2, strict separate multiply/add',promoted_at=rec['promoted_at'],official_score=None,repeat_measurements=[dict(experiment=contract.SOURCE,job_id='1583408',total_median_ms=e.get_record('conv',contract.SOURCE)['total_median_ms'])],package_verification=dict(zipcheck,sha256=package['zip_sha256'])))
        lineage.update(current_label='C7',current_measurement=contract.CONFIRMATION,current_total_median_ms=rec['total_median_ms']);e.write_json(ROOT/'records/conv-lineage.json',lineage)
    ...
    print('Raw C7 finalized; public GitHub copy remains a separate step.')
