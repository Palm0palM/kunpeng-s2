...
#undef main
#include "conv2d.c"
static unsigned long prefix_entries, tail_entries, rowpair_entries, rowquint_entries;
#if CHECK_ROWTRIPLE
...
void __cyg_profile_func_enter(void *fn, void *caller) {
    (void)caller;
    if (fn == (void *)conv_sve_rowquint) __atomic_fetch_add(&rowquint_entries, 1, __ATOMIC_RELAXED);
...
int main(int argc, char **argv) {
    ...
    const int quad_ok = 1;
#endif
    const unsigned long quint = __atomic_load_n(&rowquint_entries, __ATOMIC_RELAXED);
    printf("SVE_ROWQUINT_ACTUAL_ENTRIES=%lu\n", quint);
    return rc ? rc : (pair && triple_ok && quad_ok && quint ? 0 : 4);
}
