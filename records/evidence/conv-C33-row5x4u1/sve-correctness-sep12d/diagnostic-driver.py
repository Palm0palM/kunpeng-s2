...
if len(sys.argv) != 4 or sys.argv[1] not in ('C33-row5x4u1',) or sys.argv[3] not in ('submit', 'status', 'fetch'):
    raise SystemExit('Usage: python3 .runs/conv/sep12d-checks/driver.py VERSION VERIFIED_CONFIG submit|status|fetch')
version, config, action = sys.argv[1:]
expected_full_cases = {'C33-row5x4u1': 40804}[version]
expected_checks = {'full_per_configuration': expected_full_cases, 'smoke_per_configuration': 864, 'configurations': 6, 'total': (expected_full_cases + 864) * 6}
...
if action == 'submit':
    ...
    remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12d-' + version + '-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command = cluster.submit_command(cfg, remote_dir, 'conv-sep12d-' + version)
