"""AT returned-log acceptance/archive only; no operator or network execution."""
from pathlib import Path
import argparse,hashlib,json,re,shlex,shutil,datetime
BASE=Path(__file__).resolve().parent
ROOT=BASE.parents[2]
VERSION='C63-row7x2shared4rowwise'
SOURCE_SHA256='d78a863eb49c85d415420819e4b6ef8379260a34ad9f118da5bab1bdca7e94a8'
B=BASE/VERSION
RAW=B/'raw'
FROZEN=ROOT/'.runs/conv'/VERSION/'sve-correctness-sep13at'
def read(p):return json.loads(p.read_text())
def need(ok,why):
    if not ok:raise ValueError(why)
def main():
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--job-id',required=True,help='Original AT ID actually saved after root one-shot submission')
    job_id=parser.parse_args().job_id
    need(re.fullmatch(r'[0-9]+',job_id) and job_id != '1590550','Own original AT ID required')
    need(not FROZEN.exists() and not (B/'validation.json').exists(),'Already accepted/frozen; do not overwrite')
    s=read(B/'AT_SUMMARY.json');j=read(B/'job.json');prepared=read(B/'prepared.json')
    ...
    for v in (16,32,64):
        for t in (1,4):
            ...
            header=f'SVE_BYTES={v} SVE_LANES={v//4} BLOCK_OUTPUTS={2*v//4} THREADS={t}'
    ...
    need(hashes.get('conv2d.c')==SOURCE_SHA256,'Own C63 reviewed source identity')
    ...
    result=dict(status='passed',complete=True,candidate=VERSION,job_id=job_id,scheduler=status,exit_code=0,total_cases=44328,full_cases=34464,dispatch_cases=7272,direct_cases=2592,runner_cases=0,configurations=configs,source_hashes=hashes,source_hashes_verified=True,compiler_version='10.3.1',build_commands=expected,stage_exits=s['stages'],allocation=s['scheduler_resources'],assembly=dict(fused_instructions=0,review='TARGETED_ASSEMBLY_REVIEW.md'),performance_measured=False,executed_locally=False,executed_remotely=True,sanitizer='NOT_RUN',issues=[],note='Original AT own numerical evidence; root-read actual 7-row 2VL shared4 rowwise load/broadcast schedule, spill and frame review. No speed claim or automatic promotion.')
    ...
    print('AT'+job_id+' accepted/frozen44328; performance unmeasured.')
