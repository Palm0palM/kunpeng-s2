"""Prepared AT diagnostic transport for C63; explicit root --go only.
...
VERSIONS = ('C63-row7x2shared4rowwise',)
SOURCE_SHA256 = 'd78a863eb49c85d415420819e4b6ef8379260a34ad9f118da5bab1bdca7e94a8'
SOURCE_NAMES = {'conv2d.c', 'check_conv_guard.c', 'check_sve_dispatch.c', 'candidate.env', 'remote_job.sh'}
SOURCE_PARENTS = {'C63-row7x2shared4rowwise': 'C58-r1'}
AS_JOB_ID = '1590550'
...
COUNTS = {'C63-row7x2shared4rowwise': dict(full_per_configuration=5744, dispatch_per_configuration=1212,
...
# The frozen AT wrapper builds no retained production object for separate transfer.
...
def completed_as_evidence():
    path = ROOT / '.runs/conv/sep13as-campaign.json'
    campaign = read_json(path)
    order = ['C58-r8', 'C62-row7x2shared4', 'C58-r9']
    require(campaign.get('performance_job') == AS_JOB_ID and campaign.get('status') == 'complete'
        and campaign.get('measurement_order') == order and campaign.get('expected_benchmark_cases') == 36,
        'Original AS must be completely recorded')
    ...
    require([r.get('version') for r in rows] == order
        ...
        'Original AS all36 samples required')
    return dict(path=str(path.relative_to(ROOT)), job_id=AS_JOB_ID, expected_samples=36, read_only=True)


def serial_gate(cfg):
    identities = {AS_JOB_ID: ['.runs/conv/sep13as-campaign.json']}
    unresolved = []
    try:
        completed_as_evidence()
    ...
    if path.exists():
        ...
        if data.get('version') != VERSIONS[0] or not re.fullmatch(r'[0-9]+', job_id):
            unresolved.append('Original AT reservation unknown; reconcile without resubmission')
        else:
...
def frozen_source(base, version):
    ...
    require(set(expected) == SOURCE_NAMES and expected['conv2d.c'] == SOURCE_SHA256, 'Own C63 transport identity')
    source = base / 'source'
    ...
    require(record.get('version') == version and record.get('source_parent', record.get('parent')) == 'C58-r1'
        and record.get('status') == 'prepared' and record.get('verified') is False, 'Unmeasured C63 checkpoint')
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root AT submission authorization')
    ...
    if args.action == 'submit':
        ...
        as_evidence = completed_as_evidence()
        gate = serial_gate(cfg)
        save(base / ('gate-' + stamp() + '.json'), gate)
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep13at-' + args.version + '-' + created
        data = dict(version=args.version, remote_dir=remote_dir, submitted=False,
            ...
            source_hashes=hashes, known_job_gate=gate, completed_as_evidence=as_evidence, explicit_go=True, no_automatic_continuation=True)
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep13at-' + args.version)
    ...
    print('Fetched immutable AT diagnostic text/assembly. Fetch is not acceptance or performance.')
