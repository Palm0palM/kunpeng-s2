"""Read original AT logs and write a lifecycle summary; no network/operator import."""
...
check(j['job_id']=='1590603' and j['version']=='C63-row7x2shared4rowwise','Original AT identity')
...
for v in (16,32,64):
    for t in (1,4):
        ...
        check(header==dh==(v,v//4,2*v//4,t),f'Actual VL/thread {v}/{t}')
...
flags=['gcc','-O3','-std=c11','-D_DEFAULT_SOURCE','-Wall','-Wextra','-fno-fast-math','-ffp-contract=off','-mcpu=generic','-fopenmp','-DEXPECTED_ACC=2']
...
outer=[]
for label in ('sep13at-lifecycle-status-001','sep13at-lifecycle-fetch-001'):
...
lines += ['', '旧fallback计数仅按原AT checker要求五类均非零；以下为实际观测，不是预期值。没有沿用AP或AR固定计数。', '|SVE字节LOCAL_USER线程|PREFIX|TAIL|ROWPAIR|ROWTRIPLE|ROWQUAD|', 'LOCAL_USER---:LOCAL_USER---:LOCAL_USER---:LOCAL_USER---:LOCAL_USER---:LOCAL_USER---:LOCAL_USER---:LOCAL_USER']
...
lines += ['',f'实际 GCC {compiler}，三条完整 gcc argv 来自 probe.log 的 xtrace，strict/generic/关闭FP融合；38CPU、24576MiB、单packed NUMA、1800秒，允许CPU {cpus[0]}–{cpus[-1]}，{numa}。六配置实际线程1/4，close/cores绑定。','', '五份运输源的返回字节、远端manifest、原件、prepared/job与本地清单一致。未运行sanitizer或官方runner；汇编原件已返回，目标spill分析由独立审查者处理。此摘要不执行接受/归档，不填性能或提速。','', '原始日志在 raw/；完整逐配置、source关联、资源及两次外层argv/UTC/输出路径见 AT_SUMMARY.json。用户已取消40%停止阈值；未使用reset。此子任务只读取日志，不执行本机题目或提交新作业。']
with (B/'AT_SUMMARY.md').open('x') as f:f.write('\n'.join(lines)+'\n')
