#!/usr/bin/env python3
"""Create one public, redacted r7 evidence archive; never run tests or digests.
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r7-20260912'
COHORTS = {
    'compare': RUNS / 'optimization-20260912-r7-compare',
}
MEMBERS = ('T8-control12-repeat-r7', 'T10-lhistbarrier')
...
def main():
    ...
    for member in MEMBERS:
        ...
        for path in sorted(folder.glob('*NOTES*.md')):
            add(path, prefix / path.name)
        assembly_reviews(folder, prefix)
    prior = RUNS / 'T8-control12-repeat-r7/prior-record.json'
    add(prior, 'members/T8-control12-repeat-r7/prior-record.json', required=True)
    for version in ('T8-control12', 'T10-lhistbarrier'):
    ...
    add(Path(__file__).resolve(), 'tools/archive-r7.py', required=True)

    # The r7 prior record belongs to the r6 repeat, not the initial T8 run.
    # Older runs remain in the existing public r6 archive; do not create an
    # artificial baseline-only cohort or require their live .runs directories.
    previous = read_object(prior)
    history_archive = ROOT / 'records/evidence/trsm-stage-r6-20260912'
    prior_run = 'T8-control12-repeat-r6'
    ...
    prior_record_origin = {
        'record': 'members/T8-control12-repeat-r7/prior-record.json',
        'prior_run': prior_run,
        'prior_public_archive': '../trsm-stage-r6-20260912/',
        'prior_run_cluster': '../trsm-stage-r6-20260912/members/' + prior_run + '/cluster.json',
        ...
        'note': 'Job ID association only; the r6 archive retains the original baseline and older history.',
    }
    if not prior_record_origin['job_id_match']:
        missing.append('r7 prior-record.json matching the r6 repeat job; inspect the referenced public r6 archive')
...
File owner/group identifiers redacted for publication.
    ...
    document = {
        'schema_version': 1, 'problem': 'trsm', 'stage': 'r7-20260912',
        ...
        'repeat_policy': 'The r7 T8 repeat and its prior record are separate objects; the prior record is associated with the r6 repeat by job ID. Initial baseline and older history are referenced in the existing r6 public archive.',
    ...
    (DEST / 'README.md').write_text('''# TRSM r7 公开实验归档
...
''')
    print('Created public TRSM r7 archive:', DEST.relative_to(ROOT), 'text_files=', len(inventory))
