...
def prepare():
    ...
    if [item['run_name'] for item in plan['members']] != ['T8-control12-repeat-r7', 'T10-lhistbarrier']:
        raise RuntimeError('The predeclared r7 protocol requires exactly the planned A/B members')
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r7-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r7 measurements.',
