...
def main():
    ...
    if (names != ['T8-control12-repeat-r7', 'T10-lhistbarrier']
            or order != [names, list(reversed(names)), names]):
        raise RuntimeError('The predeclared r7 protocol requires formal A/B, B/A, A/B rounds')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r7-one-official-warmup-v1'
