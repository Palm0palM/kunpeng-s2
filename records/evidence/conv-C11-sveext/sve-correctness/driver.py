...
import cluster
base=Path('.runs/conv/C11-sveext/sve-correctness')
...
if action=='submit':
    ...
    remote_dir=cfg['remote_root']+'/diagnostics/conv-sveext-correctness-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command=cluster.submit_command(cfg,remote_dir,'conv-sveext-correctness')
