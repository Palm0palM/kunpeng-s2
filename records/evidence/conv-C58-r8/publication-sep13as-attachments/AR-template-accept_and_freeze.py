"""AR returned-log acceptance/archive only; no operator or network execution."""
from pathlib import Path
import argparse,hashlib,json,re,shlex,shutil,datetime
BASE=Path(__file__).resolve().parent
ROOT=BASE.parents[2]
VERSION='C62-row7x2shared4'
SOURCE_SHA256='e6cc9bdb4c14e9f6982f687123379ebf5dfb2013777f7bb076c04edd4827dc8a'
B=BASE/VERSION
RAW=B/'raw'
FROZEN=ROOT/'.runs/conv'/VERSION/'sve-correctness-sep13ar'
def read(p):return json.loads(p.read_text())
def need(ok,why):
    if not ok:raise ValueError(why)
def main():
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--job-id',required=True,help='Original AR ID actually saved after root one-shot submission')
    job_id=parser.parse_args().job_id
    need(re.fullmatch(r'[0-9]+',job_id) and job_id != '1590254','Own original AR ID required')
    need(not FROZEN.exists() and not (B/'validation.json').exists(),'Already accepted/frozen; do not overwrite')
    s=read(B/'AR_SUMMARY.json');j=read(B/'job.json');prepared=read(B/'prepared.json')
    ...
    need(hashes.get('conv2d.c')==SOURCE_SHA256,'Own C62 reviewed source identity')
    ...
    print('AR'+job_id+' accepted/frozen44328; performance unmeasured.')
