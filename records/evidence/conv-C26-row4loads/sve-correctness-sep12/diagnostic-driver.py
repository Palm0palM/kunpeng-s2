...
if len(sys.argv) != 4 or sys.argv[1] not in ('C26-row4loads', 'C27-row3staged') or sys.argv[3] not in ('submit', 'status', 'fetch'):
    raise SystemExit('Usage: python3 .runs/conv/sep12-checks/driver.py VERSION VERIFIED_CONFIG submit|status|fetch')
...
if action == 'submit':
    ...
    remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12-' + version + '-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command = cluster.submit_command(cfg, remote_dir, 'conv-sep12-' + version)
