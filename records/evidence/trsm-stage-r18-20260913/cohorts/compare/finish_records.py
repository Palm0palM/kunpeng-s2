#!/usr/bin/env python3
"""Register collected r18 logs without running task code or computing digests."""
...
with (HERE / 'finish-attempt.json').open('x') as handle:
    ...
    handle.write('\n')
spec = importlib.util.spec_from_file_location('r18_finish_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
names = ['T19-control13-repeat-r18', 'T20-wideunroll2-repeat-r18']
...
if (list(config['members']) != names or config['round_order'] != [names[i:] + names[:i] for i in range(3)]
        ...
        or config['warmup_protocol'].get('protocol_id') != 'trsm-r18-one-official-warmup-v1'
    ...
    raise SystemExit('r18 protocol identity changed')
for index, (name, version) in enumerate(zip(names, versions)):
    if config['members'][name].get('version') != version or config['members'][name].get('repeat_existing') is not True:
        raise SystemExit('r18 requires both unchanged T19/T20 repeats')
...
for index, (name, version) in enumerate(zip(names, versions)):
    ...
    if prior.get('job_id') != '1581862' or meta.get('prior_job_id') != '1581862':
        raise SystemExit('r18 must preserve its declared r17 prior records')
...
for index, (name, version) in enumerate(zip(names, versions)):
    expected_record = prior_bytes[version]
...
with (HERE / 'finish-complete.json').open('x') as handle:
    json.dump(dict(job_id=GROUP['job_id'], status='compared_pending_root_review', formal_pass_count=18,
                   warmup_pass_count=6, repeat_versions=versions, new_versions=[], promoted=False), handle, indent=2)
    handle.write('\n')
