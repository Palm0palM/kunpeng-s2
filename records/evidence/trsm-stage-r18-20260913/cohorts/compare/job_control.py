...
EXPECTED_NAMES = ['T19-control13-repeat-r18', 'T20-wideunroll2-repeat-r18']
EXPECTED_VERSIONS = ['T19-control13', 'T20-wideunroll2']
PRIOR_JOB_ID = '1581862'
...
def record_helpers():
    ...
    spec = importlib.util.spec_from_file_location('r18_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
def prepare():
    if PREPARE_REVIEW_PENDING:
        raise RuntimeError('r18 preparation is blocked pending root independent review')
    ...
    if ([item['run_name'] for item in plan['members']] != EXPECTED_NAMES
            ...
            or any(item.get('repeat_existing', False) is not True
                   or item.get('prior_job_id') != PRIOR_JOB_ID
                   for index, item in enumerate(plan['members']))):
        raise RuntimeError('The r18 protocol requires two unchanged repeats following r17 job 1581862')
    ...
    if (declared['warmup_order'] != EXPECTED_NAMES or declared['warmup_suites_per_member'] != 1
        ...
        raise RuntimeError('The r18 predeclared warm-up/formal protocol changed')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing'):
            ...
            if prior.get('job_id') != PRIOR_JOB_ID:
                raise RuntimeError('The r18 repeat must follow the registered r17 comparison job 1581862')
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r18 plan must preserve original implementation identity: ' + field)
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r18-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after full CT64 wide32 preflight, to apply the '
                'startup-warming protocol declared before viewing r18 measurements.',
