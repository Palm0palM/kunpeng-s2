...
def main():
    ...
    if (names != ['T19-control13-repeat-r18', 'T20-wideunroll2-repeat-r18']
            or order != [names, names[::-1], names]):
        raise RuntimeError('The r18 protocol requires A/B, B/A, A/B formal suites')
    for index, (name, version) in enumerate(zip(names, ('T19-control13', 'T20-wideunroll2'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not True):
            raise RuntimeError('The r18 pair must retain its original repeat identities')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r18-one-official-warmup-v1'
