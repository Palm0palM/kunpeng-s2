#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Archive existing r18 evidence once; never execute or synthesize tests."""
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r18-20260913'
COHORTS = {
    'compare': ('optimization-20260913-r18-compare', ['T19-control13-repeat-r18', 'T20-wideunroll2-repeat-r18']),
}
...
def main():
    ...
    associations = {}
    previous = ROOT / 'records/evidence/trsm-stage-r17-20260913/records/latest'
    for version, member in zip(('T19-control13', 'T20-wideunroll2'), COHORTS['compare'][1]):
        ...
        valid = (prior['version'] == version and prior['job_id'] == '1581862'
                 and json.loads(redact(json.dumps(prior))) == previous_public)
        if not valid:
            raise RuntimeError('Repeat prior differs from archived r17 record: ' + version)
        associations[version] = dict(valid=True, prior_job_id='1581862',
    ...
    add(Path(__file__), Path('tools/archive-r18.py'), required=True)
    if missing:
    ...
    manifest = dict(schema_version=1, stage='r18-20260913', problem='trsm',
    ...
    (DEST / 'README.md').write_text("""# TRSM r18 evidence

This redacted public derivative preserves a fresh same-allocation comparison of
unchanged T19-control13 and T20-wideunroll2. The actual submitted inputs, sources,
commands, environment, formal/warmup results, target assembly and full prior
records are retained. Each prior record is associated with its archived r17
record from job 1581862. Historical samples are never pooled with this run.

Both members execute complete CT64 wide32 direct/whole/argument checks. The T19
small-path validation remains historical (job 1579861), not a fresh preflight
claim. Original T20 preparation documents are explicitly stored as historical.

Archiving only copies existing evidence; it does not itself validate tests or
promote the candidate. No local task execution, hash operations or contest
submission. Actual reference is KML25.1/GCC12, not specified KML25.2. Private
originals remain local. Do not reuse this destination in place.
""")
    print('Archived', len(inventory), 'r18 text files; prior association valid, required missing=0')
