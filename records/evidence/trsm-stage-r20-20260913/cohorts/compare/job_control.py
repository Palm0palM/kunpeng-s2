...
EXPECTED_NAMES = ['T19-control13-repeat-r20', 'T20-wideunroll2-repeat-r20', 'T22-unrollreg']
EXPECTED_VERSIONS = ['T19-control13', 'T20-wideunroll2', 'T22-unrollreg']
PRIOR_JOB_ID = '1582129'
...
def record_helpers():
    ...
    spec = importlib.util.spec_from_file_location('r20_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
def prepare():
    if PREPARE_REVIEW_PENDING:
        raise RuntimeError('r20 preparation is blocked pending root independent review')
    ...
    if ([item['run_name'] for item in plan['members']] != EXPECTED_NAMES
        ...
        raise RuntimeError('The r20 protocol requires two unchanged repeats and the new T22 member')
    ...
    if (declared['warmup_order'] != EXPECTED_NAMES or declared['warmup_suites_per_member'] != 1
        ...
        raise RuntimeError('The r20 predeclared warm-up/formal protocol changed')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing'):
            ...
            if prior.get('job_id') != PRIOR_JOB_ID:
                raise RuntimeError('The r20 repeat must follow the registered r19 comparison job 1582129')
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r20 plan must preserve original implementation identity: ' + field)
        ...
        if item.get('repeat_existing'):
            ...
            if item['source_from'] != item['version'] or item['run_name'] != item['version']:
                raise RuntimeError('The new T22 member must use its prepared source directory')
        original = ROOT / '.runs/trsm' / item['source_from'] / 'source'
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r20-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after full CT64 wide32 preflight, to apply the '
                'startup-warming protocol declared before viewing r20 measurements.',
