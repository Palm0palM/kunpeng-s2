...
def main():
    ...
    if (names != ['T19-control13-repeat-r20', 'T20-wideunroll2-repeat-r20', 'T22-unrollreg']
            or order != [names[i:] + names[:i] for i in range(3)]):
        raise RuntimeError('The r20 protocol requires ABC/BCA/CAB formal suites')
    for index, (name, version) in enumerate(zip(names, ('T19-control13', 'T20-wideunroll2', 'T22-unrollreg'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index < 2)):
            raise RuntimeError('The r20 three-member cohort must retain its repeat/new identities')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r20-one-official-warmup-v1'
