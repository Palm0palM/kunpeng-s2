...
def range_review(item,lines,outer,prior,work,require_arithmetic=True):
    a,z=item['line_start'],item['line_end']
    require(type(a) is int and type(z) is int and outer[0]<=a<=z<outer[1],'Invalid actual arithmetic-region range')
    require(all(z<x or a>y for x,y in prior),'Overlapping distinct phase/loop ranges')
    prior.append((a,z))
    require(item.get('kernel_columns_per_iteration')==work,'Incorrect region work normalization')
    ...
    require(not require_arithmetic or (actual['fmul']>0 and actual['fadd']>0),'Reviewed loop lacks separate arithmetic')
...
def shared_review(stage,lines,outer,ranges):
    ...
    for block in blocks:
        ...
        for key in ('vector_spill_loads','vector_spill_stores'):
            require(type(block.get(key)) is int and block[key]>=0,'Invalid shared block spill observation')
        actual_ranges=block.get('ranges') or []
        require(isinstance(actual_ranges,list) and actual_ranges,'Missing actual block intervals')
        totals=collections.Counter()
        for part in actual_ranges:
            item=dict(block,line_start=part['line_start'],line_end=part['line_end'])
            range_review(item,lines,outer,ranges,work,require_arithmetic=False)
            totals.update(item['derived_counts'])
        require(totals['fmul']==totals['fadd']==21*work,
    ...
    for path in paths:
        require(type(path['remainder_columns']) is int and bool(path.get('path_condition'))
            and bool(path.get('execution_order_review')),'Missing actual remainder path condition/order')
        sequence=path.get('sequence')
        require(isinstance(sequence,list),'Remainder path must give an ordered sequence, including empty zero path')
        work=0
    ...
    require(used=={b['block_id'] for b in blocks if b['role']=='u1_remainder'},'Unmapped actual remainder block')
    stage['actual_arithmetic_region_count']=sum(len(b['ranges']) for b in blocks)
    ...
    for helper in helpers:
        ...
        for stage in stages:
            if stage['stage']!='shared':
                ...
                continue
            shared_review(stage,lines,outer,ranges)
        helper['actual_arithmetic_region_count']=len(ranges)
    ...
    for vl in (16,32,64):
        for threads in (1,4):
            ...
            for phase,n in (('dispatch',1236),('direct',1080)):
                values=tuple(map(int,one('^'+phase.upper()+r'_ROWSEVEN_ACTUAL_ENTRIES=(\d+) EXPECTED=(\d+) WORKER_MASK=(\d+) EXPECTED_MASK=(\d+)$',dispatch,'Missing rowseven entry/mask')))
                require(values==(n,n,mask,mask),'Rowseven per-phase entry/mask mismatch')
            ...
            configurations.append(dict(sve_bytes=vl,threads=threads,lanes=vl//4,block_outputs_per_row=p['vectors']*(vl//4),
                full_cases=p['full'],dispatch_cases=p['dispatch'],direct_cases=p['direct'],helper_entries=entries,passed=True,
                full_matrix_counts=dict(core=3888,narrow=144,small=720,larger=32,quad_boundary=960),
                dispatch_matrix_counts=dict(core=972,quad_boundary=240),**extra))
