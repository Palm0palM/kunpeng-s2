...
CHECK_ALLOCATION
# Optional PMU metadata on the allocated node only; never a correctness gate.
(
    set +e
    command -v perf
    command_exit=$?
    printf 'PERF_COMMAND_EXIT=%s\n' "$command_exit"
    cat /proc/sys/kernel/perf_event_paranoid
    paranoid_exit=$?
    printf 'PERF_PARANOID_EXIT=%s\n' "$paranoid_exit"
    perf stat -x, -e cycles,instructions -- /bin/true
    stat_exit=$?
    printf 'PERF_STAT_EXIT=%s\n' "$stat_exit"
) > pmu-metadata.log 2>&1 || true
source candidate.env
[[ "$EXPECTED_ACC" == 4 || "$EXPECTED_ACC" == 6 ]]
[[ "$CHECK_ROWTRIPLE" == 0 || "$CHECK_ROWTRIPLE" == 1 ]]
[[ "$CHECK_ROWQUAD" == 0 || "$CHECK_ROWQUAD" == 1 ]]
date -u
hostname
gcc --version
