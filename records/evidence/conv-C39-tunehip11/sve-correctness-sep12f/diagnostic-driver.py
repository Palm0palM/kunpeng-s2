...
if len(sys.argv) != 4 or sys.argv[1] not in ('C37-row4u1','C38-splitbuild','C39-tunehip11') or sys.argv[3] not in ('submit', 'status', 'fetch'):
    raise SystemExit('Usage: python3 .runs/conv/sep12f-checks/driver.py VERSION VERIFIED_CONFIG submit|status|fetch')
version, config, action = sys.argv[1:]
expected_full_cases, expected_smoke_cases, expected_probe_cases = {'C37-row4u1': (18052,96,0),'C38-splitbuild': (0,96,0),'C39-tunehip11': (7108,96,0)}[version]
expected_checks = {'full_per_configuration': expected_full_cases, 'smoke_per_configuration': expected_smoke_cases, 'prefetch_probe_per_configuration': expected_probe_cases, 'configurations': 6, 'total': (expected_full_cases + expected_smoke_cases + expected_probe_cases) * 6, 'runner_cases': 0 if version == 'C37-row4u1' else 4}
...
if action == 'submit':
    ...
    remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12f-' + version + '-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command = cluster.submit_command(cfg, remote_dir, 'conv-sep12f-' + version)
