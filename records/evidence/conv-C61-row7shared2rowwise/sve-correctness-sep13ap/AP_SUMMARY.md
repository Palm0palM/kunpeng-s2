...
候选 C61-row7shared2rowwise，原作业 1590209。首次 status/fetch 均退出0，调度器 SUCCEEDED，job/system/wrapper = 0/0/0。
...
原始日志在 raw/；完整逐配置、source关联、资源及两次外层argv/UTC/输出路径见 AP_SUMMARY.json。用户已取消40%停止阈值；未使用reset。此子任务只读取日志，不执行本机题目或提交新作业。
