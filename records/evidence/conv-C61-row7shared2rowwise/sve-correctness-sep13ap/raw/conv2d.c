...
#if defined(__GNUC__) LOCAL_USERLOCAL_USER defined(__clang__)
    ...
    for (; ow - i >= block; i += block) {
        ...
        for (int t = 6; t < kh; ++t) {
            ...
            for (; kw - ik >= 2; ik += 2) {
                {
                    const int column = ik;
                    const svfloat32_t v0 = svld1(pg, row + column + 0 * lanes);
                    const svfloat32_t v1 = svld1(pg, row + column + 1 * lanes);
                    const svfloat32_t v2 = svld1(pg, row + column + 2 * lanes);
                    {
                        const svfloat32_t weight = svdup_n_f32(ka[column]);
                        a0 = svadd_f32_x(pg, a0, svmul_f32_x(pg, v0, weight));
                        a1 = svadd_f32_x(pg, a1, svmul_f32_x(pg, v1, weight));
                        a2 = svadd_f32_x(pg, a2, svmul_f32_x(pg, v2, weight));
                    }
                    {
                        const svfloat32_t weight = svdup_n_f32(kb[column]);
                        b0 = svadd_f32_x(pg, b0, svmul_f32_x(pg, v0, weight));
                        b1 = svadd_f32_x(pg, b1, svmul_f32_x(pg, v1, weight));
                        b2 = svadd_f32_x(pg, b2, svmul_f32_x(pg, v2, weight));
                    }
                    {
                        const svfloat32_t weight = svdup_n_f32(kc[column]);
                        c0 = svadd_f32_x(pg, c0, svmul_f32_x(pg, v0, weight));
                        c1 = svadd_f32_x(pg, c1, svmul_f32_x(pg, v1, weight));
                        c2 = svadd_f32_x(pg, c2, svmul_f32_x(pg, v2, weight));
                    }
                    {
                        const svfloat32_t weight = svdup_n_f32(kd[column]);
                        d0 = svadd_f32_x(pg, d0, svmul_f32_x(pg, v0, weight));
                        d1 = svadd_f32_x(pg, d1, svmul_f32_x(pg, v1, weight));
                        d2 = svadd_f32_x(pg, d2, svmul_f32_x(pg, v2, weight));
                    }
                    {
                        const svfloat32_t weight = svdup_n_f32(ke[column]);
                        e0 = svadd_f32_x(pg, e0, svmul_f32_x(pg, v0, weight));
                        e1 = svadd_f32_x(pg, e1, svmul_f32_x(pg, v1, weight));
                        e2 = svadd_f32_x(pg, e2, svmul_f32_x(pg, v2, weight));
                    }
                    {
                        const svfloat32_t weight = svdup_n_f32(kf[column]);
                        f0 = svadd_f32_x(pg, f0, svmul_f32_x(pg, v0, weight));
                        f1 = svadd_f32_x(pg, f1, svmul_f32_x(pg, v1, weight));
                        f2 = svadd_f32_x(pg, f2, svmul_f32_x(pg, v2, weight));
                    }
                    {
                        const svfloat32_t weight = svdup_n_f32(kg[column]);
                        g0 = svadd_f32_x(pg, g0, svmul_f32_x(pg, v0, weight));
                        g1 = svadd_f32_x(pg, g1, svmul_f32_x(pg, v1, weight));
                        g2 = svadd_f32_x(pg, g2, svmul_f32_x(pg, v2, weight));
                    }
                }
                {
            ...
            CONVFLOAT *output)
{
     ...
     */
    if (kernelHeight <= 0 LOCAL_USERLOCAL_USER kernelWidth <= 0 LOCAL_USERLOCAL_USER
        inputHeight < kernelHeight LOCAL_USERLOCAL_USER inputWidth < kernelWidth) {
        return;
    }
...
#pragma omp parallel for schedule(static)
        for (size_t group = 0; group < groups; ++group) {
            ...
            const float *base = input + first_row * stride;
            float *dst = output + first_row * output_stride;
            if (remaining >= 7) {
                ...
                conv_sve_prefix(base + (size_t)4 * stride, stride, kernel,
                                kernelHeight, kernelWidth, dst + 4 * output_stride, ow);
            } else if (remaining == 4) {
                conv_sve_rowquad(base, stride, kernel, kernelHeight, kernelWidth,
                                 dst, dst + output_stride, dst + 2 * output_stride,
                                 dst + 3 * output_stride, ow);
