"""Read original AP logs and write a lifecycle summary; no network/operator import."""
...
check(j['job_id']=='1590209' and j['version']=='C61-row7shared2rowwise','Original AP identity')
...
outer=[]
for label in ('sep13ap-lifecycle-status-001','sep13ap-lifecycle-fetch-001'):
...
lines += ['',f'实际 GCC {compiler}，三条完整 gcc argv 来自 probe.log 的 xtrace，strict/generic/关闭FP融合；38CPU、24576MiB、单packed NUMA、1800秒，允许CPU {cpus[0]}–{cpus[-1]}，{numa}。六配置实际线程1/4，close/cores绑定。','', '五份运输源的返回字节、远端manifest、原件、prepared/job与本地清单一致。未运行sanitizer或官方runner；汇编原件已返回，目标spill分析由独立审查者处理。此摘要不执行接受/归档，不填性能或提速。','', '原始日志在 raw/；完整逐配置、source关联、资源及两次外层argv/UTC/输出路径见 AP_SUMMARY.json。用户已取消40%停止阈值；未使用reset。此子任务只读取日志，不执行本机题目或提交新作业。']
with (B/'AP_SUMMARY.md').open('x') as f:f.write('\n'.join(lines)+'\n')
