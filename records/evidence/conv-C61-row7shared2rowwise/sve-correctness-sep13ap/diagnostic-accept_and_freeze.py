"""AP returned-log acceptance/archive only; no operator or network execution."""
from pathlib import Path
import argparse,hashlib,json,re,shlex,shutil,datetime
BASE=Path(__file__).resolve().parent
ROOT=BASE.parents[2]
VERSION='C61-row7shared2rowwise'
SOURCE_SHA256='40a3184f346983864593de73b3e3ade7973b1af9d2d273d566432e882a6904fa'
B=BASE/VERSION
RAW=B/'raw'
FROZEN=ROOT/'.runs/conv'/VERSION/'sve-correctness-sep13ap'
def read(p):return json.loads(p.read_text())
def need(ok,why):
    if not ok:raise ValueError(why)
def main():
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--job-id',required=True,help='Original AP ID actually saved after root one-shot submission')
    job_id=parser.parse_args().job_id
    need(re.fullmatch(r'[0-9]+',job_id) and job_id != '1589611','Own original AP ID required')
    need(not FROZEN.exists() and not (B/'validation.json').exists(),'Already accepted/frozen; do not overwrite')
    s=read(B/'AP_SUMMARY.json');j=read(B/'job.json');prepared=read(B/'prepared.json')
    ...
    need(hashes.get('conv2d.c')==SOURCE_SHA256,'Own C61 reviewed source identity')
    ...
    print('AP'+job_id+' accepted/frozen44328; performance unmeasured.')
