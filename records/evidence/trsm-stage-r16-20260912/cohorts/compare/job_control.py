...
EXPECTED_NAMES = ['T8-control12-repeat-r16', 'T18-budgetwide-repeat-r16', 'T19-panel8x16budget']
EXPECTED_VERSIONS = ['T8-control12', 'T18-budgetwide', 'T19-panel8x16budget']
PRIOR_JOB_ID = '1579788'
...
def record_helpers():
    ...
    spec = importlib.util.spec_from_file_location('r16_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
def prepare():
    if PREPARE_REVIEW_PENDING:
        raise RuntimeError('r16 preparation is blocked pending root independent review')
    ...
    if ([item['run_name'] for item in plan['members']] != EXPECTED_NAMES
            ...
            or [item.get('parent') for item in plan['members']] != [None, 'T8-control12', 'T8-control12']
        ...
        raise RuntimeError('The r16 protocol requires the two unchanged r15 repeats and new T19 member')
    ...
    if (declared['warmup_order'] != EXPECTED_NAMES or declared['warmup_suites_per_member'] != 1
        ...
        raise RuntimeError('The r16 predeclared warm-up/formal protocol changed')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing'):
            ...
            if prior.get('job_id') != PRIOR_JOB_ID:
                raise RuntimeError('The r16 repeat must follow the registered r15 job 1579788')
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r16 plan must preserve original implementation identity: ' + field)
        ...
        if item.get('repeat_existing'):
            ...
            if item['source_from'] != item['version'] or item['run_name'] != item['version']:
                raise RuntimeError('The new T19 member must use its prepared source directory')
        original = ROOT / '.runs/trsm' / item['source_from'] / 'source'
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r16-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r16 measurements.',
    ...
    for directory in ('preflight', 'preflight-panel8x16', 'preflight-wide32', 'reference'):
