...
本目录仅复制 r14 的四个干净输入文件，保留全部直接微核、一般非 dyadic 整算子、no-op、4095/4097 分派边界、分配失败检查和 4 MiB 预算边界的实际分配观察；不复制任何运行产物。本轮仅运行 T8-control12-repeat-r15（version=T8-control12）与 T18-budgetwide-repeat-r15（version=T18-budgetwide），两者均为前序 job 1579730 的重复实验。仅在调度分配的 Linux aarch64、38 CPU、单 NUMA 计算节点执行：
...
父 driver/collector 调用 `preflight_audit.audit_general(output_dir, source_trsm_c, member)`；member 只接受本轮两个完整 run 名。成功返回 `completion`、`summary`、`budget_summary`，异常即失败。输出 schema 保持 `trsm-r14-preflight-v1` 和 `trsm-packed-history-budget-v1`，表示复用不变的 r14 检查契约，不表示 r15 已执行。下文 T10/T17 特征说明保留为共用 runner 的规则说明，不将它们加入本轮成员。
...
本轮 T8 使用第一行，T18 使用第三行；其 general 进程数分别为14/18，summary.tsv 步骤数分别为22/27。仅 T18 另外运行 wide32 模块，显式传 expected_CT=64。
