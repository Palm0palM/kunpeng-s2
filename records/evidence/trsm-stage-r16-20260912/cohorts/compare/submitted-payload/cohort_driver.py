...
sys.path.insert(0, str(ROOT / 'preflight-panel8x16'))
...
def main():
    ...
    if (names != ['T8-control12-repeat-r16', 'T18-budgetwide-repeat-r16', 'T19-panel8x16budget']
            or order != [names[i:] + names[:i] for i in range(3)]):
        raise RuntimeError('The predeclared r16 protocol requires formal A/B/C, B/C/A, C/A/B rounds')
    for index, (name, version) in enumerate(zip(names, ('T8-control12', 'T18-budgetwide', 'T19-panel8x16budget'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index < 2)):
            raise RuntimeError('The r16 members must retain their declared versions and repeat flags')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r16-one-official-warmup-v1'
    ...
    with (ROOT / 'preflight.log').open('xb') as log:
        for name in names[:2]:
            ...
            audit_general(ROOT / 'preflight-results' / name, ROOT / name / 'source/trsm.c', name)
    with (ROOT / 'preflight-panel8x16.log').open('xb') as log:
        name = names[2]
        code = stream(['/bin/bash', str(ROOT / 'preflight-panel8x16/run.sh'), str(ROOT / name / 'source'),
                       str(ROOT / 'preflight-panel8x16-results' / name)], ROOT, env, [log])
        if code:
            raise RuntimeError('T19 panel8x16 preflight failed')
        panel_check = audit_panel8x16(ROOT / 'preflight-panel8x16-results' / name, ROOT / name / 'source/trsm.c')
        if panel_check['guard']['scheduler_job_id'] != job:
            raise RuntimeError('T19 dedicated preflight guard job differs from the confirmed cohort job')
    with (ROOT / 'preflight-wide32.log').open('xb') as log:
        for name in names[1:]:
            code = stream(['/bin/bash', str(ROOT / 'preflight-wide32/run.sh'), str(ROOT / name / 'source'),
                           str(ROOT / 'preflight-wide32-results' / name), '64'], ROOT, env, [log])
            if code:
                raise RuntimeError('CT64 wide32 preflight failed for ' + name)
            audit_wide(ROOT / 'preflight-wide32-results' / name)
    handles = {}
    try:
        member_details = {}
        for name in names:
