"""Atomically freeze L evidence; local file copying/text inspection only.

Usage: python3 .runs/conv/sep12l-checks/freeze_returned.py VERSION [--failed]
...
EXPECTED={'C47-row6x4u1':dict(full=3560,dispatch=432,direct=360,total=26112)}
...
def validate_archive_input(source,version,failed):
    ...
    # Preserve the mandatory original L byte-count manifest; compare existing
    ...
    if failed:
        ...
        require(result.get('total_cases')==expected['total'] and result.get('runner_cases',0)==0
            ...
            'Accepted per-path/total counts differ from L plan')
        ...
        require(assembly.get('review_complete') is True and assembly.get('production_uninstrumented') is True
            and assembly.get('stage_count')==11 and assembly.get('fma_count')==0
            ...
            and assembly.get('shape')==dict(rows_per_group=6,vectors_per_row=4,accumulators=24),
            'Actual eleven-stage rowsix and complete transition/dispatch review missing')
        require(all(row.get('dispatch_entries')==432 and row.get('direct_entries')==900
            and row.get('dispatch_worker_mask')==row.get('direct_worker_mask')==(1<<row['threads'])-1
            and row.get('block_outputs_per_row')==row['sve_bytes']
            for row in configurations), 'Accepted rowsix entries/masks/block widths mismatch')
...
def main():
    ...
    target=BASE.parent/args.version/'sve-correctness-sep12l'
    staging=target.with_name('.sve-correctness-sep12l.preparing')
