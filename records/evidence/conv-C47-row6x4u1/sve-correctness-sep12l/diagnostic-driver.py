"""Prepared L diagnostic transport for C47; explicit root --go only.
...
VERSIONS = ('C47-row6x4u1',)
SOURCE_NAMES = {'conv2d.c', 'check_conv_guard.c', 'check_sve_dispatch.c', 'candidate.env', 'remote_job.sh'}
SOURCE_PARENTS = {'C47-row6x4u1': 'C40-row6x3u1'}
COUNTS = {'C47-row6x4u1': dict(full_per_configuration=3560, dispatch_per_configuration=432,
    direct_per_configuration=360, configurations=6, total=26112)}
...
# The frozen L wrapper builds no retained production object for separate transfer.
...
def serial_gate(cfg):
    """Bounded live read-only queries for known J/K, package, H/I/L work.
    ...
    known = {'sep12j': '1579660', 'sep12k': '1579677'}
    for round_name in ('sep12h', 'sep12i', 'sep12j', 'sep12k', 'sep12l'):
        ...
        if round_name in known and str(job_id) != known[round_name]:
            unresolved.append(dict(path=origin, reason='Campaign differs from its known J/K job'))
        identities.setdefault(str(job_id), []).append(origin)
...
def frozen_source(base, version):
    ...
    require(extra_manifest.is_file() and not extra_manifest.is_symlink(),
        'Frozen L bytes/SHA source-manifest.json is required')
    ...
    require(prepared.get('expected_acc') == 4 and prepared.get('expected_checks') == counts,
        'L matrix schema mismatch')
    require(prepared.get('rows_per_group') == 6 and prepared.get('vectors_per_row') == 4
        and prepared.get('accumulators') == 24, 'C47 six-row/four-vector shape mismatch')
    require(prepared.get('expected_rowsix_entries') == dict(dispatch_per_configuration=432, direct_per_configuration=900),
        'Rowsix entry expectations mismatch')
    require(prepared.get('full_matrix_counts') == dict(core=2808, narrow=144, small=576, larger=32)
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root L submission authorization')
    ...
    if args.action == 'submit':
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12l-' + args.version + '-' + created
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep12l-' + args.version)
    ...
    print('Fetched immutable L diagnostic text/assembly. Fetch is not acceptance or performance.')
