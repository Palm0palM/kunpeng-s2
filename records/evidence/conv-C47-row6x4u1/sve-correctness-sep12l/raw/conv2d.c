...
#if CONV_CAN_DISPATCH_SVE
/* Six outputs share input row t; output r consumes kernel row t-r.
 * Each output retains the original kernel-row/column order and separate mul/add. */
__attribute__((target("arch=armv8-a+sve"), noinline))
static void conv_sve_rowsix(const float *restrict base, size_t stride,
                             const float *restrict kernel, int kh, int kw,
                             float *restrict dst0, float *restrict dst1,
                             float *restrict dst2, float *restrict dst3,
                             float *restrict dst4, float *restrict dst5, int ow)
{
    if (kh < 6) {
        conv_sve_rowquad(base, stride, kernel, kh, kw, dst0, dst1, dst2, dst3, ow);
        conv_sve_rowpair(base + (size_t)4 * stride, stride, kernel, kh, kw, dst4, dst5, ow);
        return;
    }
    const int lanes = (int)svcntw();
    const int block = 4 * lanes;
    ...
    for (; ow - i >= block; i += block) {
        ...
        svfloat32_t f3 = svdup_n_f32(0.0f);
        ...
        /* Shared input rows advance all six outputs in strict kernel-column order. */
        for (int t = 5; t < kh; ++t) {
            const float *row = base + (size_t)t * stride + i;
            ...
            for (int ik = 0; ik < kw; ++ik) {
                ...
                }
                {
                    ...
                    d3 = svadd_f32_x(pg, d3, svmul_f32_x(pg, v, dk));
                    e3 = svadd_f32_x(pg, e3, svmul_f32_x(pg, v, ek));
                    f3 = svadd_f32_x(pg, f3, svmul_f32_x(pg, v, fk));
                }
            }
        }
        ...
        svst1(pg, dst5 + i + 3 * lanes, f3);
    }
    if (i < ow) {
        conv_sve_rowquad(base + i, stride, kernel, kh, kw,
                         dst0 + i, dst1 + i, dst2 + i, dst3 + i, ow - i);
        conv_sve_rowpair(base + (size_t)4 * stride + i, stride, kernel, kh, kw,
                         dst4 + i, dst5 + i, ow - i);
    }
}
#endif
            ...
            CONVFLOAT *output)
{
    ...
    if (use_sve && kernelHeight >= 6 && oh >= 6) {
        const size_t output_rows = (size_t)oh;
        const size_t output_stride = (size_t)ow;
        const size_t groups = output_rows / 6 + (output_rows % 6 != 0);
#pragma omp parallel for schedule(static)
        for (size_t group = 0; group < groups; ++group) {
            const size_t first_row = group * 6;
            ...
            if (remaining >= 6) {
                conv_sve_rowsix(base, stride, kernel, kernelHeight, kernelWidth,
