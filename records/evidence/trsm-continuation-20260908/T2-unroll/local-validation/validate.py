...
run('build-check-final-fail-alloc', flags + omp + link + [str(LOCAL / 'check-final.c'),
    str(ROOT / 'zgemm/zgemm.c'), str(LOCAL / 'fail-alloc.c'),
    str(LOCAL / 'trsm-fail-alloc.o'), '-o', str(LOCAL / 'check-final-fail-alloc')])
run('build-check-update-ubsan', flags + omp + link + [str(LOCAL / 'check-update.c'),
    '-o', str(LOCAL / 'check-update-ubsan')])
for binary, threads, name in [
    ...
    ('check-final-fail-alloc', 4, 'check-final-fail-alloc-t4'),
    ('check-update-ubsan', 4, 'check-update-t4')]:
...
print('All seven local UBSan runs passed with exit code 0.', flush=True)
