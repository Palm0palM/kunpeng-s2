...
#ifndef EXPECTED_ACC
#error "Pass EXPECTED_ACC=4 (per row; 8 total accumulators) for the selected source."
#endif
struct guarded {
    void *mapping;
...
int main(int argc, char **argv) {
    ...
    if (smoke) {
        ...
        for (int leading = 0; leading <= 1; ++leading) {
            if (!one_case(smoke_outs[a], 3, kw, 5, pad, leading)) return 1;
            ++count;
        }
    } else {
        ...
        /* Row-pair coverage: isolated odd row, even pairs, and enough pairs for
         * all four workers. Include 4*VL/8*VL boundaries at every requested VL. */
        const int pair_outs[] = {15,16,17,31,32,33,63,64,65,127,128,129};
        const int pair_kernels[][2] = {{1,1},{1,2},{1,3},{2,1},{2,2},{2,3},
                                       {3,2},{3,3},{4,7},{7,4},{7,8},{8,7}};
        const int pair_heights[] = {1,2,8};
    ...
    return count == (smoke ? 24 : 4996) ? 0 : 6;
}
