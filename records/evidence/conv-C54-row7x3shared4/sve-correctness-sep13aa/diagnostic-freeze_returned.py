"""Atomically freeze AA evidence; local file copying/text inspection only.

Usage: python3 .runs/conv/sep13aa-checks/freeze_returned.py VERSION [--failed]
...
EXPECTED={'C54-row7x3shared4':dict(full=5744,dispatch=1212,direct=432,total=44328)}
...
def validate_archive_input(source,version,failed):
    ...
    require(hashes['conv2d.c']==SOURCE_SHA256 and prepared.get('source_parent')=='C52-row7x3shared2',
        'AA must retain the original independent C54 source identity')
    # Preserve the mandatory original AA byte-count manifest; compare existing
    ...
    if failed:
        ...
        require(result.get('total_cases')==expected['total'] and result.get('runner_cases',0)==0
            ...
            'Accepted per-path/total counts differ from AA plan')
        ...
        require(assembly.get('candidate')==version and assembly.get('job_id')==job_id
            ...
            and assembly.get('schema')=='aa-shared4-paths-v1'
            and assembly.get('shared_quad_and_remainder_reviewed') is True
            and assembly.get('shared_unroll')==dict(main_columns=4,remainder_source_columns=1,remainder_max_columns=3,other_stage_columns=1),
            'Accepted independent shared quad/u1-remainder identity or review missing')
        require(prepared.get('assembly_review_schema')=='aa-shared4-paths-v1'
            ...
            'Initial AA shared contract mismatch')
        ...
        for helper in helpers:
            ...
            for stage in stages:
                ...
                require({block.get('role') for block in blocks}=={'quad_main','u1_remainder'},
                    'Accepted shared quad/u1 evidence incomplete')
                ...
                for block in blocks:
                    ...
                    require(type(work) is int and (work==4 if block['role']=='quad_main' else 1<=work<=3)
                ...
                require(any(b['role']=='quad_main' and b['region_kind']=='loop' for b in blocks),'Accepted quad backedge missing')
                paths=stage.get('remainder_paths') or []
                require({p.get('remainder_columns') for p in paths}=={0,1,2,3},'Accepted remainder coverage incomplete')
                ...
                for path in paths:
                    ...
                    for step in sequence:
                        ...
                        require(block is not None and block['role']=='u1_remainder'
                            and type(repeat) is int and 1<=repeat<=3
...
def main():
    ...
    target=BASE.parent/args.version/'sve-correctness-sep13aa'
    staging=target.with_name('.sve-correctness-sep13aa.preparing')
