"""Prepared AA diagnostic transport for C54; explicit root --go only.
...
SOURCE_PARENTS = {'C54-row7x3shared4': 'C52-row7x3shared2'}
COUNTS = {'C54-row7x3shared4': dict(full_per_configuration=5744, dispatch_per_configuration=1212,
...
# The frozen AA wrapper builds no retained production object for separate transfer.
...
def serial_gate(cfg):
    """Bounded fixed Y/W/S/T/X and present P/AA jobs; no teammate scan.

    Prepared P/AA packages without job.json do not represent active jobs.
    ...
    job_paths = [ROOT / '.runs/conv/sep13p-checks/C50-row4dupfencenomem/job.json',
                 BASE / 'C54-row7x3shared4/job.json']
    ...
    required = [('.runs/conv/sep13y-campaign.json', 'performance_job', '1582410'),
                ('.runs/conv/sep13w-campaign.json', 'performance_job', '1582256'),
                ('.runs/conv/sep13s-campaign.json', 'performance_job', '1582067'),
    ...
    for origin, field, expected_id in required:
        path = ROOT / origin
        if not path.exists():
            unresolved.append(dict(path=origin, reason='Required fixed Y/W/S/T/X evidence is missing'))
            continue
...
def frozen_source(base, version):
    ...
    require(expected['conv2d.c'] == SOURCE_SHA256, 'AA requires its own root-reviewed C54 source')
    source = base / 'source'
    ...
    require(extra_manifest.is_file() and not extra_manifest.is_symlink(),
        'Frozen AA bytes/SHA source-manifest.json is required')
    ...
    require(prepared.get('expected_acc') == 3 and prepared.get('expected_checks') == counts,
        'AA matrix schema mismatch')
    require(prepared.get('rows_per_group') == 7 and prepared.get('vectors_per_row') == 3
        and prepared.get('accumulators') == 21, 'C54 seven-row/three-vector shape mismatch')
    ...
    require(prepared.get('expected_semantic_stage_count') == 13
        ...
        'C54 shared quad/u1-remainder expectations mismatch')
    return expected
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root AA submission authorization')
    ...
    if args.action == 'submit':
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep13aa-' + args.version + '-' + created
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep13aa-' + args.version)
    ...
    print('Fetched immutable AA diagnostic text/assembly. Fetch is not acceptance or performance.')
