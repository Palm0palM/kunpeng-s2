"""Accept AA returned text only. No SSH, compilation, tests or operator execution.
...
ROOT=BASE.parents[2]
PLANS={'C54-row7x3shared4':dict(full=5744,dispatch=1212,direct=432,total=44328,vectors=3,rows=7,helper='conv_sve_rowseven',parent='C52-row7x3shared2')}
SOURCE_SHA256='fa0f5b43fc7901e9853dbe189385653ea4ae8bf697fa3b67af7f288063a4fb88'
...
def source_identity(base,job,version,require_returned=True):
    ...
    require(hashes['conv2d.c']==SOURCE_SHA256,'AA requires its own fixed C54 production source')
    ...
    require(prepared.get('source_parent')==PLANS[version]['parent'] and prepared.get('rows_per_group')==7
        and prepared.get('vectors_per_row')==3 and prepared.get('accumulators')==21,'Prepared AA source/shape mismatch')
    require(prepared.get('expected_semantic_stage_count')==13
        and prepared.get('shared_kernel_columns_per_iteration')==4
        and prepared.get('shared_remainder_source_columns_per_iteration')==1
        and prepared.get('shared_remainder_max_columns')==3
        and prepared.get('assembly_review_schema')=='aa-shared4-paths-v1'
        and prepared.get('other_stage_kernel_columns_per_iteration')==1,'Prepared shared quad/u1-remainder contract mismatch')
    require(prepared.get('full_matrix_counts')==dict(core=3888,narrow=144,small=720,larger=32,quad_boundary=960)
        and prepared.get('dispatch_matrix_counts')==dict(core=972,quad_boundary=240), 'Prepared AA family counts mismatch')
...
def shared_review(stage,lines,outer,ranges):
    ...
    for key in ('source_mapping','control_flow_notes','accumulator_order_review',
                'quad_to_remainder_review','transition_spill_review','t_and_tile_reset_review'):
    ...
    require({b.get('role') for b in blocks}=={'quad_main','u1_remainder'},'Shared quad and u1 evidence both required')
    for block in blocks:
        ...
        require(type(work) is int and (work==4 if role=='quad_main' else 1<=work<=3),
            'Actual shared work is outside quad4/direct-u1 remainder contract')
    ...
    require(any(b['role']=='quad_main' and b['region_kind']=='loop' for b in blocks),
        'No actual quad main backedge identified; changed lowering requires root review')
    ...
    require(isinstance(paths,list) and {p.get('remainder_columns') for p in paths}=={0,1,2,3},
        'Actual remainder paths must cover zero through three columns')
    ...
    for path in paths:
        ...
        for step in sequence:
            ...
            require(block is not None and block['role']=='u1_remainder'
                and type(repeat) is int and 1<=repeat<=3,'Invalid remainder block invocation')
...
def assembly_review(base,job,p,source_hash):
    ...
    require(review.get('schema')=='aa-shared4-paths-v1','AA requires its own actual multiple-region review schema')
    ...
    require(review.get('shape')==dict(rows_per_group=7,vectors_per_row=3,accumulators=21),
        'Missing actual review association with C54 seven-row/three-vector source shape')
    require(review.get('whole_machine_code_identical_claimed') is False,
        'T/C52 is source background, not current C54 machine-code proof')
    require(review.get('shared_quad_and_remainder_reviewed') is True
        and review.get('shared_unroll')==dict(main_columns=4,remainder_source_columns=1,remainder_max_columns=3,other_stage_columns=1),
        'Missing independent C54 shared quad/u1-remainder review')
    ...
    require(comparison.get('candidate')=='C52-row7x3shared2' and comparison.get('job_id')=='1582134'
        ...
        and bool(comparison.get('stack_and_spill_review')),'Missing actual T parent comparison')
    parent_asm=ROOT/'.runs/conv/C52-row7x3shared2/sve-correctness-sep13t/raw/conv2d-sve.s'
    require(comparison.get('assembly_sha256')==hashlib.sha256(parent_asm.read_bytes()).hexdigest(),
        'Parent comparison must identify actual frozen T assembly')
...
def validate(base,version):
    ...
    for vl in (16,32,64):
        for threads in (1,4):
            ...
            one(r'^FULL_MATRIX_COUNTS core=3888 narrow=144 small=720 larger=32 quad_boundary=960$',guard,'AA matrix family mismatch')
            one(r'^DISPATCH_MATRIX_COUNTS core=972 quad_boundary=240$',dispatch,'AA dispatch family mismatch')
            require(all(value>0 for value in entries.values()),'AA legacy helper not entered')
            one(r'^PASS: 1212 dispatch cases; exact per-case rowseven entries and bitwise scalar reference$',dispatch,'AA dispatch1212 missing')
            one(r'^PASS: 432 direct fallback cases; kh1\.\.6, readonly/guard/canary and bitwise scalar reference$',dispatch,'AA direct432 missing')
            mask=(1<<threads)-1
            extra={}
    ...
    return dict(status='passed',complete=True,candidate=version,job_id=job['job_id'],scheduler=scheduler,exit_code=0,
        ...
        sanitizer='not_run',issues=[],note='Actual independent AA diagnostics only; parent T/C52 and older logs provide no C54 PASS. No speed or promotion claim.')
