"""One-shot C54 text preparation/checkpoint; no compiler, operator, SSH or job."""
...
with e.locked():
    ...
    # Scopes are lexical only; actual scheduling, live ranges and spills need C54 assembly.
    columns = [first.replace('const int column = ik;', 'const int column = ' + expression + ';')
               for expression in ('ik', 'ik + 1', 'ik + 2', 'ik + 3')]
    replacement = ('            /* Preserve each accumulator order: columns ik, ik+1, ik+2, ik+3. */\n'
                   '            for (; kw - ik >= 4; ik += 4) {\n'
                   + '\n'.join(columns) + '\n            }')
    ...
    assert result.count('for (; kw - ik >= 4; ik += 4)') == 1
    assert 'for (; kw - ik >= 2; ik += 2)' not in result
    assert replacement.count('svmul_f32_x(') == replacement.count('svadd_f32_x(') == 84
    assert replacement.count('svld1(') == 12 and replacement.count('svdup_n_f32(') == 28
    ...
    meta.update(source_parent=PARENT, source_parent_frozen_directory=str(FROZEN.relative_to(ROOT)),
        ...
        shared_kernel_columns_per_iteration=4, other_stage_kernel_columns_per_iteration=1,
        shared_remainder_columns_per_iteration=1, shared_remainder_max_columns=3,
        shared_paired_remainder_layer=False, formal_best_label='C6',
        source_scope='Only shared main loop factor2 to factor4; all other12 stages, original u1 remainder, dispatch, tails, fallback and other submission files unchanged.',
        source_hypothesis='Amortize shared loop/address control with four sequential complete column scopes; actual scheduling, spills, code size and speed unknown.',
        runtime_validation_status='Source prepared only; own C54 diagnostic required. Parent T PASS cannot cover the new factor4 main loop.',
        required_future_kernel_widths=[1, 2, 3, 4, 5, 7, 8, 15, 81],
    ...
    e.checkpoint(argparse.Namespace(problem='conv', version=VERSION,
        note='Source-only shared4 from frozen T/C52. Four complete original column bodies in ik/+1/+2/+3 order; safe kw-ik>=4, ik+=4 and unchanged u1 remainder0..3. Other12 stages/dispatch/tail/fallback/submission bytes unchanged. Await own source review and future C54 diagnostic; no correctness or speed claim.'))
    audit = dict(candidate=VERSION, parent=PARENT, parent_job_id='1582134',
        ...
        main_column_body_order=['ik', 'ik+1', 'ik+2', 'ik+3'],
        four_column_bodies_equal_original_modulo_column_declaration=True,
        scopes_are_lexical_only=True,
        statements_per_column=dict(coefficient_broadcasts=7, input_loads=3, multiplies=21, adds=21),
        statements_per_main_iteration=dict(coefficient_broadcasts=28, input_loads=12, multiplies=84, adds=84),
