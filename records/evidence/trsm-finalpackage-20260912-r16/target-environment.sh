...
kml251_confirmed_job=$(python3 - "$(dirname "${BASH_SOURCE[0]}")/assigned-job-id.txt" <<'PY'
import os
...
while not marker.is_file() and time.monotonic() < deadline:
    time.sleep(1)
job = marker.read_text().strip() if marker.is_file() and not marker.is_symlink() else ''
if not re.fullmatch(r'\d+', job):
    raise SystemExit('KML251_BLOCKED: confirmed scheduler job ID marker missing or invalid')
for key in ('TRSM_SCHEDULER_JOB_ID', 'SLURM_JOB_ID', 'LSB_JOBID', 'CCS_JOB_ID', 'JOB_ID'):
    if os.environ.get(key) and os.environ[key] != job:
        raise SystemExit('KML251_BLOCKED: scheduler identity differs from confirmed marker: ' + key)
print(job)
PY
) || return 2
export TRSM_SCHEDULER_JOB_ID="$kml251_confirmed_job" JOB_ID="$kml251_confirmed_job"
...
def cpus(text):
    ...
    for part in text.strip().split(','):
        if not part:continue  # Some NUMA nodes have memory but no CPUs.
...
if [[ -d /CLUSTER_USER_HOME/HPCKit/latest/modulefiles ]]; then
