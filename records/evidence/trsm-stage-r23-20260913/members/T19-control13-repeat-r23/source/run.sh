...
if type module >/dev/null 2>&1 && [[ -d /CLUSTER_USER_HOME/HPCKit/latest/modulefiles ]]; then
    module use /CLUSTER_USER_HOME/HPCKit/latest/modulefiles
...
# 默认使用官方 kblas。验证时可显式指定兼容的参考库路径，日志记录差异。
if [[ -n "${KBLAS_LIB:-}" ]]; then
    [[ -f "$KBLAS_LIB" ]] || { echo 'KBLAS_LIB 文件不存在'; exit 2; }
    export LD_LIBRARY_PATH="$(dirname "$KBLAS_LIB"):${LD_LIBRARY_PATH:-}"
    echo "Reference BLAS override: $KBLAS_LIB (与官方 KML 配置不同)"
    "$cc" "${flags[@]}" -Icompat bench_trsm.c trsm.c "$KBLAS_LIB" -o trsm_test -lm
else
    echo 'Reference BLAS: official kblas'
    "$cc" "${flags[@]}" bench_trsm.c trsm.c -o trsm_test -lm -lkblas
fi
runs="${TEST_RUNS:-1}"
[[ "$runs" =~ ^[1-9][0-9]*$ ]] || exit 2
cases=(
...
for i in "${!cases[@]}"; do
    ...
    numactl --cpunodebind="$node" --membind="$node" ./trsm_test "${dims[@]}" "$runs" | tee "$logdir/case-$i.log"
