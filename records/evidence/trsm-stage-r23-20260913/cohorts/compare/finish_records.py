#!/usr/bin/env python3
"""Register collected r23 logs without running task code or computing digests."""
...
with (HERE / 'finish-attempt.json').open('x') as handle:
    ...
    handle.write('\n')
spec = importlib.util.spec_from_file_location('r23_finish_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
names = ['T19-control13-repeat-r23', 'T25-smallpairload']
versions = ['T19-control13', 'T25-smallpairload']
if (list(config['members']) != names or config['round_order'] != [names[i:] + names[:i] for i in range(3)]
        ...
        or config['warmup_protocol'].get('protocol_id') != 'trsm-r23-one-official-warmup-v1'
    ...
    raise SystemExit('r23 protocol identity changed')
for index, (name, version) in enumerate(zip(names, versions)):
    if config['members'][name].get('version') != version or config['members'][name].get('repeat_existing') is not (index == 0):
        raise SystemExit('r23 requires one baseline repeat and one new T25 version')
...
for index, (name, version) in enumerate(zip(names, versions)):
    ...
    if index == 0:
        ...
        if prior.get('job_id') != '1582526' or meta.get('prior_job_id') != '1582526':
            raise SystemExit('r23 must preserve its declared new-baseline prior record')
        ...
        if (prepared_bytes[version] != helper.regular_bytes(run / 'experiment.json')
            ...
            raise SystemExit('The new T25 member already has changed or recorded experiment evidence')
...
comparisons = {}
for candidate in ('T25-smallpairload',):
