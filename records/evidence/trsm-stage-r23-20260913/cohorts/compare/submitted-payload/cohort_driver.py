...
def main():
    ...
    if (names != ['T19-control13-repeat-r23', 'T25-smallpairload']
            or order != [names, names[::-1], names]):
        raise RuntimeError('The r23 protocol requires A/B, B/A, A/B formal suites')
    for index, (name, version) in enumerate(zip(names, ('T19-control13', 'T25-smallpairload'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index == 0)):
            raise RuntimeError('The r23 pair must retain its repeat/new identities')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r23-one-official-warmup-v1'
