...
EXPECTED_NAMES = ['T19-control13-repeat-r23', 'T25-smallpairload']
EXPECTED_VERSIONS = ['T19-control13', 'T25-smallpairload']
PRIOR_JOB_ID = '1582526'
...
def record_helpers():
    ...
    spec = importlib.util.spec_from_file_location('r23_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
def prepare():
    if PREPARE_REVIEW_PENDING:
        raise RuntimeError('r23 preparation is blocked pending root independent review')
    ...
    if ([item['run_name'] for item in plan['members']] != EXPECTED_NAMES
        ...
        raise RuntimeError('The r23 protocol requires one unchanged baseline repeat and the new T25 member')
    ...
    if (declared['warmup_order'] != EXPECTED_NAMES or declared['warmup_suites_per_member'] != 1
        ...
        raise RuntimeError('The r23 predeclared warm-up/formal protocol changed')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing'):
            ...
            if prior.get('job_id') != PRIOR_JOB_ID:
                raise RuntimeError('The r23 repeat must follow the registered r22 baseline repeat job 1582526')
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r23 plan must preserve original implementation identity: ' + field)
        ...
        if item.get('repeat_existing'):
            ...
            if item['source_from'] != item['version'] or item['run_name'] != item['version']:
                raise RuntimeError('The new T25 member must use its prepared source directory')
        original = ROOT / '.runs/trsm' / item['source_from'] / 'source'
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r23-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after full small-panel8x16 preflight, to apply the '
                'startup-warming protocol declared before viewing r23 measurements.',
