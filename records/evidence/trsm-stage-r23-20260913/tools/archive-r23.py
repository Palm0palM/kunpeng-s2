#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Archive existing r23 evidence once; never execute or synthesize tests."""
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r23-20260913'
COHORTS = {
    'compare': ('optimization-20260913-r23-compare', ['T19-control13-repeat-r23', 'T25-smallpairload']),
}
...
def main():
    ...
    associations = {}
    previous = ROOT / 'records/evidence/trsm-stage-r22-20260913/records/latest'
    for version, member in zip(('T19-control13',), COHORTS['compare'][1][:1]):
        ...
        valid = (prior['version'] == version and prior['job_id'] == '1582526'
                 and json.loads(redact(json.dumps(prior))) == previous_public)
        if not valid:
            raise RuntimeError('Repeat prior differs from archived r22 record: ' + version)
        associations[version] = dict(valid=True, prior_job_id='1582526',
                                     prior_public_record=str((previous / (version + '.json')).relative_to(ROOT)))
    associated = all(x['valid'] for x in associations.values())
    add(ROOT / 'records/experiments/trsm/T25-smallpairload.json',
        Path('records/latest/T25-smallpairload.json'), required=True)
    for name in ('PREPARATION.md', 'VALIDATION-PLAN.md', 'STATIC-REVIEW.md'):
        add(RUNS / 'T25-smallpairload' / name, Path('members/T25-smallpairload') / name, required=True)
    for name in ('T19-control13-vs-T25-smallpairload.json',):
    ...
    add(Path(__file__), Path('tools/archive-r23.py'), required=True)
    if missing:
    ...
    manifest = dict(schema_version=1, stage='r23-20260913', problem='trsm',
    ...
    (DEST / 'README.md').write_text("""# TRSM r23 evidence

This redacted public derivative preserves the same-allocation comparison of
unchanged T19-control13 with new T25-smallpairload. Submitted inputs, source
...
r20 record from job1582526; historical samples are never pooled.
...
""")
    print('Archived', len(inventory), 'r23 text files; prior association valid, required missing=0')
