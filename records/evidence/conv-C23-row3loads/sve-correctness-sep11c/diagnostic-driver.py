...
if len(sys.argv) != 4 or sys.argv[1] not in ('C23-row3loads', 'C24-row4x4', 'C25-row3x6') or sys.argv[3] not in ('submit', 'status', 'fetch'):
    raise SystemExit('Usage: python3 .runs/conv/sep11c-checks/driver.py VERSION VERIFIED_CONFIG submit|status|fetch')
...
if action == 'submit':
    ...
    remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep11c-' + version + '-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    data = {'version': version, 'remote_dir': remote_dir, 'submitted': False}
    ...
    command = cluster.submit_command(cfg, remote_dir, 'conv-sep11c-' + version)
