...
import cluster
base=Path('.runs/conv/C14-ext4/sve-correctness')
...
if action=='submit':
    ...
    remote_dir=cfg['remote_root']+'/diagnostics/conv-ext4-correctness-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command=cluster.submit_command(cfg,remote_dir,'conv-ext4-correctness')
