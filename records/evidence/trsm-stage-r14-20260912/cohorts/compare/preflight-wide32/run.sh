...
set -euo pipefail
if [[ $# != 3 ]]; then
    echo 'usage: bash preflight-wide32/run.sh /absolute/candidate/source/ /absolute/outputdir expected_CT{32|64|128}' >&2
    exit 2
fi
...
output_dir=$2
expected_ct=$3
case "$expected_ct" in 32|64|128) ;; *) echo 'PREFLIGHT_BLOCKED expected_CT must be 32, 64, or 128' >&2; exit 2 ;; esac
...
test_flags=(-finstrument-functions "-DTRSM_TEST_EXPECTED_CT=$expected_ct" "-DTRSM_SOURCE=\"$output_dir/instrumented-trsm.c\"")
...
run_step verify-results python3 - "$output_dir" "$expected_ct" <<'PY'
...
expected_ct = int(sys.argv[2])
if expected_ct not in (32, 64, 128):
    raise SystemExit('PREFLIGHT_BLOCKED invalid parser expected_CT')
...
def check_dispatch(label, data, threads, wide, narrow=False, no_sve=False):
    tile_rows = re.findall(r'^TILE_CONFIG_PASS KB=(\d+) CT=(\d+)$', data, re.M)
    if (tile_rows != [('256', str(expected_ct))]
            or len(re.findall(r'^TILE_CONFIG_PASS\b', data, re.M)) != 1):
        blocked('missing, duplicate or wrong CT/KB configuration: ' + label)
...
if (len(micro_rows), whole, shared_fail, no_sve_cases, narrow_cases,
        argument_checked_cases) != (28, 28, 1, 1, 1, 56):
...
summary = dict(schema='trsm-wide32-preflight-v1', complete=True,
               expected_CT=expected_ct, KB=256, tile_config_processes=7,
