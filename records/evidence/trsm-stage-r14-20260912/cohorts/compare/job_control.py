...
def prepare():
    ...
    if [item['run_name'] for item in plan['members']] != ['T8-control12-repeat-r14', 'T10-lhistbarrier-repeat-r14', 'T17-lhistbudget4', 'T18-budgetwide']:
        raise RuntimeError('The predeclared r14 protocol requires exactly the planned four members')
        ...
        if item.get('repeat_existing'):
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r14 plan must preserve original implementation identity: ' + field)
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r14-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r14 measurements.',
