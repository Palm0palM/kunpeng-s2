#!/usr/bin/env python3
"""Register collected r14 logs without running task code or computing digests."""
...
names = ['T8-control12-repeat-r14', 'T10-lhistbarrier-repeat-r14', 'T17-lhistbudget4', 'T18-budgetwide']
...
for name in names:
    ...
    audit['members'][name] = dict(warmup_cases_pass=3, measured_cases_pass=count,
                                  general_preflight=general,
                                  warmup_excluded_from_comparison=True)
    audit['measured_official_pass_count'] += count
    audit['warmup_official_pass_count'] += 3
audit['wide_preflight'] = audit_wide(DEST / 'preflight-wide32-results' / 'T18-budgetwide')
if audit['measured_official_pass_count'] != 36 or audit['warmup_official_pass_count'] != 12:
    raise SystemExit('Expected 36 measured and 12 warm-up official case passes')
...
comparisons = {}
for candidate in ('T10-lhistbarrier', 'T17-lhistbudget4', 'T18-budgetwide'):
...
# These two comparisons isolate design changes, not promotion parents.
mechanism = {}
for base, candidate in (('T10-lhistbarrier', 'T17-lhistbudget4'),
                        ('T17-lhistbudget4', 'T18-budgetwide')):
...
medians = {name: json.loads((ROOT / 'records/experiments/trsm' / (name + '.json')).read_text())['total_median_ms']
           for name in ('T8-control12', 'T10-lhistbarrier', 'T17-lhistbudget4', 'T18-budgetwide')}
