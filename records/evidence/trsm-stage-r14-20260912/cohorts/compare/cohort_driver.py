...
def main():
    ...
    if (names != ['T8-control12-repeat-r14', 'T10-lhistbarrier-repeat-r14', 'T17-lhistbudget4', 'T18-budgetwide']
            or order != [names[i:] + names[:i] for i in range(3)]):
        raise RuntimeError('The predeclared r14 protocol requires formal A/B/C/D, B/C/D/A, C/D/A/B rounds')
    for index, (name, version) in enumerate(zip(names, ('T8-control12', 'T10-lhistbarrier', 'T17-lhistbudget4', 'T18-budgetwide'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index < 2)):
            raise RuntimeError('The r14 members must retain their declared versions and repeat flags')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r14-one-official-warmup-v1'
    ...
    with (ROOT / 'preflight-wide32.log').open('xb') as log:
        ...
        if code:
            raise RuntimeError('T18 wide32 preflight failed')
