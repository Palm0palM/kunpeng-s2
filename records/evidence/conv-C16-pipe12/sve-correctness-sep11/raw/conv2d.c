 ...
 * an independent output; twelve accumulators span the entire kernel. Stream
    ...
    for (; ow - i >= block; i += block) {
        ...
        for (int jk = 0; jk < kh; ++jk) {
            ...
            for (; ik < kw - 1; ik += 2) {
                 ...
                 * thirteenth unshifted vector could exceed the input row. */
                a11 = svadd_f32_x(pg, a11, svmul_f32_x(pg, svld1(pg, p + 11 * lanes + 1), k1));
            }
            for (; ik < kw; ++ik) {
