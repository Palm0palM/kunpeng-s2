...
if len(sys.argv) != 4 or sys.argv[1] not in ('C34-colmajor256', 'C35-prefetch2', 'C36-qmajor') or sys.argv[3] not in ('submit', 'status', 'fetch'):
    raise SystemExit('Usage: python3 .runs/conv/sep12e-checks/driver.py VERSION VERIFIED_CONFIG submit|status|fetch')
version, config, action = sys.argv[1:]
expected_full_cases, expected_smoke_cases, expected_probe_cases = {'C34-colmajor256': (21748,96,0), 'C35-prefetch2': (29668,168,18), 'C36-qmajor': (18052,96,0)}[version]
expected_checks = {'full_per_configuration': expected_full_cases, 'smoke_per_configuration': expected_smoke_cases, 'prefetch_probe_per_configuration': expected_probe_cases, 'configurations': 6, 'total': (expected_full_cases + expected_smoke_cases + expected_probe_cases) * 6}
...
if action == 'submit':
    ...
    remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12e-' + version + '-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command = cluster.submit_command(cfg, remote_dir, 'conv-sep12e-' + version)
