...
仅计算节点编译执行，配置config/conv-sep12.local.json。C34后C35再C36串行，唯一job ID，约60秒查询，所有diagnostic终止后根代理才可测性能。不本机编译测试，不用重置卡。
...
C35包已就绪；计划每配置29668 full+168 smoke+18独立prefetch分支proof，六配置179124；具体proof和验收条件随独立包保存。两者严格参考、只读输入/kernel、保护页/输出canary及NaN、worker实际VL保持原样。wrapper先检查38CPU单NUMA计算分配，禁止本机/登录节点执行。
...
C36标准guard每配置18052 full+96 smoke，共108888，沿C26原矩阵；kh>=4/kw>=2及满4VL可达共享双列路径。C35与C36在计算节点检查前记录非致命PMU元数据，不把失败或未支持当作可用。详见各候选COVERAGE/PREFETCH_PROOF记录。
