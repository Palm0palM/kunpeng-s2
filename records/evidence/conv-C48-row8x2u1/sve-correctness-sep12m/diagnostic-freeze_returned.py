"""Atomically freeze M evidence; local file copying/text inspection only.

Usage: python3 .runs/conv/sep12m-checks/freeze_returned.py VERSION [--failed]
...
EXPECTED={'C48-row8x2u1':dict(full=5360,dispatch=1080,direct=504,total=41664)}
...
def validate_archive_input(source,version,failed):
    ...
    # Preserve the mandatory original M byte-count manifest; compare existing
    ...
    if failed:
        ...
        require(result.get('total_cases')==expected['total'] and result.get('runner_cases',0)==0
            ...
            'Accepted per-path/total counts differ from M plan')
        ...
        require(assembly.get('review_complete') is True and assembly.get('production_uninstrumented') is True
            and assembly.get('stage_count')==15 and assembly.get('fma_count')==0
            ...
            and assembly.get('shape')==dict(rows_per_group=8,vectors_per_row=2,accumulators=16),
            'Actual fifteen-stage roweight and complete transition/dispatch review missing')
        require(all(row.get('dispatch_entries')==792 and row.get('direct_entries')==1260
            and row.get('dispatch_worker_mask')==row.get('direct_worker_mask')==(1<<row['threads'])-1
            and row.get('block_outputs_per_row')==row['sve_bytes']//2
            for row in configurations), 'Accepted roweight entries/masks/block widths mismatch')
...
def main():
    ...
    target=BASE.parent/args.version/'sve-correctness-sep12m'
    staging=target.with_name('.sve-correctness-sep12m.preparing')
