                               ...
                               float *restrict dst6, float *restrict dst7, int ow)
{
    ...
    for (; ow - i >= block; i += block) {
        ...
        svfloat32_t h0 = svdup_n_f32(0.0f);
        svfloat32_t h1 = svdup_n_f32(0.0f);
        ...
        /* Input row 6: start output 6 and advance earlier outputs. */
        {
            const float *row = base + (size_t)6 * stride + i;
            const float *ka = kernel + ((size_t)6) * (size_t)kw;
            const float *kb = kernel + ((size_t)5) * (size_t)kw;
            const float *kc = kernel + ((size_t)4) * (size_t)kw;
            const float *kd = kernel + ((size_t)3) * (size_t)kw;
            const float *ke = kernel + ((size_t)2) * (size_t)kw;
            const float *kf = kernel + ((size_t)1) * (size_t)kw;
            const float *kg = kernel;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ak = svdup_n_f32(ka[ik]);
                const svfloat32_t bk = svdup_n_f32(kb[ik]);
                const svfloat32_t ck = svdup_n_f32(kc[ik]);
        ...
        /* Shared input rows advance all eight outputs in strict kernel-column order. */
        for (int t = 7; t < kh; ++t) {
            const float *row = base + (size_t)t * stride + i;
            ...
            const float *krow_h = kernel + ((size_t)t - 7) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ak = svdup_n_f32(ka[ik]);
                const svfloat32_t bk = svdup_n_f32(kb[ik]);
                const svfloat32_t ck = svdup_n_f32(kc[ik]);
                ...
                const svfloat32_t fk = svdup_n_f32(kf[ik]);
                const svfloat32_t gk = svdup_n_f32(kg[ik]);
                const svfloat32_t hk = svdup_n_f32(krow_h[ik]);
                /* One source-level input window; GCC may reschedule. */
                {
                    const svfloat32_t v = svld1(pg, row + ik + 0 * lanes);
                    a0 = svadd_f32_x(pg, a0, svmul_f32_x(pg, v, ak));
                    ...
                    f0 = svadd_f32_x(pg, f0, svmul_f32_x(pg, v, fk));
                    g0 = svadd_f32_x(pg, g0, svmul_f32_x(pg, v, gk));
                    h0 = svadd_f32_x(pg, h0, svmul_f32_x(pg, v, hk));
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 1 * lanes);
                    a1 = svadd_f32_x(pg, a1, svmul_f32_x(pg, v, ak));
                    ...
                    f1 = svadd_f32_x(pg, f1, svmul_f32_x(pg, v, fk));
                    g1 = svadd_f32_x(pg, g1, svmul_f32_x(pg, v, gk));
                    h1 = svadd_f32_x(pg, h1, svmul_f32_x(pg, v, hk));
                }
            }
        }
        /* Trailing input row kh+0: finish output 1 and advance later outputs. */
        {
            ...
            const float *krow_h = kernel + ((size_t)kh - 7) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t bk = svdup_n_f32(kb[ik]);
                const svfloat32_t ck = svdup_n_f32(kc[ik]);
                const svfloat32_t dk = svdup_n_f32(kd[ik]);
        ...
        /* Trailing input row kh+1: finish output 2 and advance later outputs. */
        {
            ...
            const float *krow_h = kernel + ((size_t)kh - 6) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ck = svdup_n_f32(kc[ik]);
                const svfloat32_t dk = svdup_n_f32(kd[ik]);
                const svfloat32_t ek = svdup_n_f32(ke[ik]);
        ...
        /* Trailing input row kh+2: finish output 3 and advance later outputs. */
        {
            ...
            const float *krow_h = kernel + ((size_t)kh - 5) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t dk = svdup_n_f32(kd[ik]);
                const svfloat32_t ek = svdup_n_f32(ke[ik]);
                const svfloat32_t fk = svdup_n_f32(kf[ik]);
        ...
        /* Trailing input row kh+3: finish output 4 and advance later outputs. */
        {
            ...
            const float *krow_h = kernel + ((size_t)kh - 4) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ek = svdup_n_f32(ke[ik]);
                const svfloat32_t fk = svdup_n_f32(kf[ik]);
                const svfloat32_t gk = svdup_n_f32(kg[ik]);
        ...
        /* Trailing input row kh+4: finish output 5 and advance later outputs. */
        {
            ...
            const float *krow_h = kernel + ((size_t)kh - 3) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t fk = svdup_n_f32(kf[ik]);
                const svfloat32_t gk = svdup_n_f32(kg[ik]);
                const svfloat32_t hk = svdup_n_f32(krow_h[ik]);
        ...
        /* Trailing input row kh+5: finish output 6 and advance later outputs. */
        {
            ...
            const float *krow_h = kernel + ((size_t)kh - 2) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t gk = svdup_n_f32(kg[ik]);
                const svfloat32_t hk = svdup_n_f32(krow_h[ik]);
                /* One source-level input window; GCC may reschedule. */
        ...
        /* Trailing input row kh+6: finish output 7 and advance later outputs. */
        {
            const float *row = base + ((size_t)kh + 6) * stride + i;
            const float *krow_h = kernel + ((size_t)kh - 1) * (size_t)kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t hk = svdup_n_f32(krow_h[ik]);
                /* One source-level input window; GCC may reschedule. */
                {
        ...
        svst1(pg, dst7 + i + 0 * lanes, h0);
        svst1(pg, dst7 + i + 1 * lanes, h1);
    }
    if (i < ow) {
            ...
            CONVFLOAT *output)
{
    ...
    if (use_sve && kernelHeight >= 8 && oh >= 8) {
        const size_t output_rows = (size_t)oh;
        const size_t output_stride = (size_t)ow;
        const size_t groups = output_rows / 8 + (output_rows % 8 != 0);
#pragma omp parallel for schedule(static)
        for (size_t group = 0; group < groups; ++group) {
            const size_t first_row = group * 8;
