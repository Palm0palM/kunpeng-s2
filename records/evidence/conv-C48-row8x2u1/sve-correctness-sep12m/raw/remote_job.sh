...
[[ "$CANDIDATE" == C48-row8x2u1 && "$EXPECTED_ACC" == 2 ]]
[[ "$EXPECTED_FULL" == 5360 && "$EXPECTED_DISPATCH" == 1080 && "$EXPECTED_DIRECT" == 504 && "$EXPECTED_TOTAL" == 41664 ]]
[[ "$EXPECTED_DISPATCH_ROWEIGHT_ENTRIES" == 792 && "$EXPECTED_DIRECT_ROWEIGHT_ENTRIES" == 1260 ]]
printf 'CANDIDATE=%s\n' "$CANDIDATE"
date -u
hostname
compiler_check() {
