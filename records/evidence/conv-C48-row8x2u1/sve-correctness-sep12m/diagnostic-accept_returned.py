"""Accept M returned text only. No SSH, compilation, tests or operator execution.
...
ROOT=BASE.parents[2]
PLANS={'C48-row8x2u1':dict(full=5360,dispatch=1080,direct=504,total=41664,vectors=2,rows=8,helper='conv_sve_roweight',parent='C40-row6x3u1')}
...
def source_identity(base,job,version,require_returned=True):
    ...
    require(prepared.get('source_parent')==PLANS[version]['parent'] and prepared.get('rows_per_group')==8
        and prepared.get('vectors_per_row')==2 and prepared.get('accumulators')==16,'Prepared M source/shape mismatch')
...
def assembly_review(base,job,p,source_hash):
    ...
    for helper in helpers:
        ...
        require([s['stage'] for s in stages]==phase_ids,'Missing/reordered fifteen roweight phases')
        ranges=[]
        for stage in stages:
    ...
    require(review.get('shape')==dict(rows_per_group=8,vectors_per_row=2,accumulators=16),
        'Missing actual review association with C48 eight-row/two-vector source shape')
    require(review.get('whole_machine_code_identical_claimed') is False,
        'C40 is source background, not current C48 machine-code proof')
...
def validate(base,version):
    ...
    for vl in (16,32,64):
        for threads in (1,4):
            ...
            one(r'^FULL_MATRIX_COUNTS core=4320 narrow=144 small=864 larger=32$',guard,'M matrix family mismatch')
            require(all(value>0 for value in entries.values()),'M legacy helper not entered')
            one(r'^PASS: 1080 dispatch cases; exact per-case roweight entries and bitwise scalar reference$',dispatch,'M dispatch1080 missing')
            one(r'^PASS: 504 direct fallback cases; kh1\.\.7, readonly/guard/canary and bitwise scalar reference$',dispatch,'M direct504 missing')
            mask=(1<<threads)-1
            extra={}
            for phase,n in (('dispatch',792),('direct',1260)):
                values=tuple(map(int,one('^'+phase.upper()+r'_ROWEIGHT_ACTUAL_ENTRIES=(\d+) EXPECTED=(\d+) WORKER_MASK=(\d+) EXPECTED_MASK=(\d+)$',dispatch,'Missing roweight entry/mask')))
                require(values==(n,n,mask,mask),'Roweight per-phase entry/mask mismatch')
    ...
    return dict(status='passed',complete=True,candidate=version,job_id=job['job_id'],scheduler=scheduler,exit_code=0,
        ...
        sanitizer='not_run',issues=[],note='Actual independent M diagnostics only; old C40/C47 logs provide no C48 PASS. No speed or promotion claim.')
