"""Prepared M diagnostic transport for C48; explicit root --go only.
...
VERSIONS = ('C48-row8x2u1',)
SOURCE_NAMES = {'conv2d.c', 'check_conv_guard.c', 'check_sve_dispatch.c', 'candidate.env', 'remote_job.sh'}
SOURCE_PARENTS = {'C48-row8x2u1': 'C40-row6x3u1'}
COUNTS = {'C48-row8x2u1': dict(full_per_configuration=5360, dispatch_per_configuration=1080,
    direct_per_configuration=504, configurations=6, total=41664)}
...
# The frozen M wrapper builds no retained production object for separate transfer.
...
def serial_gate(cfg):
    """Bounded live read-only queries for known H/J/K, package, H/I/L/M work.

    A prepared package without job.json is ignored. An existing unresolved
    reservation blocks; no cancellation, automatic retry or teammate-job scan.
    This is not a global scheduler mutex: one coordinator supplies explicit --go.
    """
    identities = {'1579660': ['J performance'], '1579677': ['K confirmation'],
        '1579748': ['H performance (root-owned during M preparation)']}
    unresolved = []
    ignored_prepared = []
    job_paths = [ROOT / '.runs/conv/sep12h-checks' / name / 'job.json'
        for name in ('C43-padinput', 'C44-copyinput')]
    job_paths += [ROOT / '.runs/conv/sep12i-checks' / name / 'job.json'
        for name in ('C45-row4dup4', 'C46-row5x5asmfix')]
    job_paths += [ROOT / '.runs/conv/sep12l-checks/C47-row6x4u1/job.json']
    job_paths += [BASE / name / 'job.json' for name in VERSIONS]
    job_paths += [ROOT / '.runs/conv' / name / 'cluster.json'
        for name in ('C26-r14', 'C44-copyinput', 'C43-padinput', 'C26-r15',
    ...
    for path in job_paths:
        ...
        if not re.fullmatch(r'[0-9]+', str(job_id or '')):
            unresolved.append(dict(path=origin, reason='Existing reservation without a reconciled numeric job ID'))
            continue
        identities.setdefault(str(job_id), []).append(origin)
    known = {'sep12h': '1579748', 'sep12j': '1579660', 'sep12k': '1579677'}
    for round_name in ('sep12h', 'sep12i', 'sep12j', 'sep12k', 'sep12l', 'sep12m'):
        ...
        origin = str(path.relative_to(ROOT))
        if not re.fullmatch(r'[0-9]+', str(job_id or '')):
            if (round_name not in known and data.get('status') in ('prepared', 'planned')
                    and data.get('submit_attempted') is False and data.get('submitted') is False):
                ignored_prepared.append(origin)
                continue
            unresolved.append(dict(path=origin, reason='Campaign reservation lacks a reconciled performance job ID'))
            continue
        if round_name in known and str(job_id) != known[round_name]:
            unresolved.append(dict(path=origin, reason='Campaign differs from its known H/J/K job'))
        identities.setdefault(str(job_id), []).append(origin)
    ...
    return dict(checked_at=datetime.now(timezone.utc).isoformat(), jobs=evidence,
        unresolved_submissions=unresolved, ignored_unsubmitted_prepared_campaigns=ignored_prepared, clear=clear)
...
def frozen_source(base, version):
    ...
    require(extra_manifest.is_file() and not extra_manifest.is_symlink(),
        'Frozen M bytes/SHA source-manifest.json is required')
    ...
    require(prepared.get('expected_acc') == 2 and prepared.get('expected_checks') == counts,
        'M matrix schema mismatch')
    require(prepared.get('rows_per_group') == 8 and prepared.get('vectors_per_row') == 2
        and prepared.get('accumulators') == 16, 'C48 eight-row/two-vector shape mismatch')
    require(prepared.get('expected_roweight_entries') == dict(dispatch_per_configuration=792, direct_per_configuration=1260),
        'Roweight entry expectations mismatch')
    require(prepared.get('full_matrix_counts') == dict(core=4320, narrow=144, small=864, larger=32)
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root M submission authorization')
    ...
    if args.action == 'submit':
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12m-' + args.version + '-' + created
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep12m-' + args.version)
    ...
    print('Fetched immutable M diagnostic text/assembly. Fetch is not acceptance or performance.')
