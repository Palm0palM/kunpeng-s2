...
int main(void){
 double te=0;
 int td[][2]={{4095,9},{4096,8},{4097,9},{4355,65}};
 for(int d=0;d<4;++d){int m=td[d][0],n=td[d][1],la=m+3,lb=n+7;size_t nl=(size_t)m*la,nb=(size_t)m*lb;double *l=calloc(nl,sizeof(double)),*save=malloc(nl*sizeof(double)),*x=malloc(nb*sizeof(double)),*b=malloc(nb*sizeof(double));
 ...
 printf("TRSM packed-path: 4 cases PASS; known solution max error %.3e; padding and L unchanged\n",te);
 return 0;
}
