"""Prepared O diagnostic transport for C49; explicit root --go only.
...
SOURCE_PARENTS = {'C49-row4dup4fence': 'C45-row4dup4'}
COUNTS = {'C49-row4dup4fence': dict(full_per_configuration=20164,
    dispatch_per_configuration=96, direct_per_configuration=0,
    configurations=6, total=121560)}
...
# The O wrapper retains no production object for separate transfer.
...
def serial_gate(cfg):
    """Bounded current I/L/M/O/N CONV queries; no historical or teammate scan.

    Explicit --go is still required from the single compute coordinator.
    Unsubmitted prepared packages/campaigns are not running reservations.
    Unknown actual job reservations block; never retry/cancel them here.
    """
    identities = {'1579762': ['I C45 diagnostic'], '1579795': ['I C46 diagnostic']}
    unresolved = []
    ignored_prepared = []
    job_paths = [ROOT / '.runs/conv/sep12l-checks/C47-row6x4u1/job.json',
        ROOT / '.runs/conv/sep12m-checks/C48-row8x2u1/job.json',
        BASE / 'C49-row4dup4fence/job.json']
    ...
    if path.exists():
        ...
        origin = str(path.relative_to(ROOT))
        if re.fullmatch(r'[0-9]+', str(job_id or '')):
            identities.setdefault(str(job_id), []).append(origin)
        elif (data.get('status') in ('prepared', 'planned')
                and not data.get('submitted') and not data.get('submit_attempted')):
            ignored_prepared.append(origin)
        else:
            unresolved.append(dict(path=origin, reason='N reservation lacks reconciled performance job ID'))
    ...
    return dict(checked_at=datetime.now(timezone.utc).isoformat(), jobs=evidence,
        ...
        ignored_unsubmitted_prepared_campaigns=ignored_prepared, clear=clear)
...
def frozen_source(base, version):
    ...
    # O preserves both first-transport manifest formats.
    ...
    if extra_manifest.exists():
        ...
        require(False, 'O supplemental source-manifest.json is required')
    ...
    require(prepared.get('expected_acc') == 4
        and prepared.get('check_rowtriple') == prepared.get('check_rowquad') == 1,
        'C49 helper instrumentation schema mismatch')
    require(prepared.get('kh5_dynamic_coverage') is False,
        'Frozen O matrix has no kh5 dynamic coverage')
    return expected
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root O submission authorization')
    ...
    if args.action == 'submit':
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep12o-' + args.version + '-' + created
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep12o-' + args.version)
    ...
    print('Fetched immutable O diagnostic text/assembly. Fetch is not acceptance or performance.')
