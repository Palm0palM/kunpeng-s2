"""Accept O returned text only. No SSH, compilation, tests or operator execution.
...
ROOT=BASE.parents[2]
PLANS={'C49-row4dup4fence':dict(full=20164,dispatch=96,direct=0,total=121560,vectors=4,rows=4,helper='conv_sve_rowquad',parent='C45-row4dup4')}
...
def assembly_review(base,job,p,source_hash):
    ...
    for helper in helpers:
        ...
        require([s['stage'] for s in stages]==phase_ids,'Missing/reordered seven phases')
        ranges=[]
        for stage in stages:
    ...
    # Empty asm has no emitted instruction and may have no #APP marker.
    # Accepted immutable source + real successful compile proves acceptance;
    # schedule reconstruction is reviewed separately without invented PCs.
    fences=review.get('fence_review') or {}
    for key in ('review_complete','compiler_accepted','source_contract_reviewed',
                'empty_templates_emit_no_instruction','does_not_claim_hardware_fence'):
        require(fences.get(key) is True,'C49 fence review missing '+key)
    source=(raw/'conv2d.c').read_text()
    starts=list(re.finditer(r'__asm__\s+__volatile__\s*\(\s*""',source))
    require(len(starts)==fences.get('source_fence_count')==3,'Need exactly three original empty asm sites')
    expected='__asm__ __volatile__("" : '+', '.join('"+w"('+v+')' for v in ('ak4','bk4','ck4','dk4'))+' : '+', '.join('"w"('+row+str(n)+')' for row in 'abcd' for n in range(4))+' : "memory");'
    contract=re.sub(r'\s+','',expected)
    source_lines=[]
    for start in starts:
        end=source.find(');',start.start())
        require(end>=0 and re.sub(r'\s+','',source[start.start():end+2])==contract,'Source packed/acc/memory contract differs')
        source_lines.append(source[:start.start()].count('\n')+1)
    shared_ranges=[(s['line_start'],s['line_end']) for h in helpers for s in h['stages'] if s['stage']=='shared']
    observations=fences.get('observations') or []
    require(len(observations)==3 and [o.get('after_kernel_lane') for o in observations]==[0,1,2],'Need q0/q1/q2 observations')
    for i,item in enumerate(observations):
        require(item.get('source_asm_line')==source_lines[i],'Fence source location mismatch')
        require(item.get('mapping_status') in ('reconstructed','not_separately_identifiable'),'Invalid fence location certainty')
        require(item.get('instruction_address_claimed') is False,'Empty compiler-only asm has no asserted instruction PC')
        ranges=item.get('evidence_ranges') or []
        require(ranges,'Fence observation needs actual surrounding evidence')
        for span in ranges:
            a,z=span.get('line_start'),span.get('line_end')
            require(type(a) is int and type(z) is int and any(x<=a<=z<=y for x,y in shared_ranges),'Fence evidence outside real shared loop')
        for key in ('packed_value_notes','accumulator_readiness_notes','scheduling_notes','register_and_spill_notes'):
            require(bool(item.get(key)),'Missing actual fence dependency/liveness interpretation '+key)
    comparison=review.get('codegen_comparison') or {}
    require(comparison.get('review_complete') is True and type(comparison.get('distinct_from_prior_machine_arrangements')) is bool,'Need actual C45/C49 comparison, including a truthful false')
    require(comparison.get('prior_versions')==['C45-row4dup4'],'Only C45 is the direct same-shape control')
    prior_base=ROOT/'.runs/conv/C45-row4dup4/sve-correctness-sep12i'
    prior=read(prior_base/'validation.json')
    prior_review=prior.get('assembly') or {}
    require(prior.get('status')=='passed' and prior.get('complete') is True and prior.get('candidate')=='C45-row4dup4' and prior.get('job_id')=='1579762' and prior.get('source_hashes_verified') is True,'Missing actual frozen C45 evidence')
    identity={key:prior_review.get(key) for key in ('candidate','job_id','source_sha256','assembly_sha256')}
    require(comparison.get('prior_identity')==identity and identity['source_sha256']==prior['source_hashes']['conv2d.c'],'Wrong prior compiler artifact identity')
    prior_bytes=(prior_base/'raw/conv2d-sve.s').read_bytes()
    require(hashlib.sha256(prior_bytes).hexdigest()==identity['assembly_sha256'],'Prior assembly identity mismatch')
    prior_lines=prior_bytes.decode().splitlines()
    observed=[]
    for helper in prior_review.get('helpers',[]):
        for stage in helper.get('stages',[]):
            if stage.get('stage')=='shared':
                require(stage.get('kernel_columns_per_iteration')==4,'Prior work normalization mismatch')
                observed.append(dict(symbol=helper['symbol'],line_start=stage['line_start'],line_end=stage['line_end'],kernel_columns_per_iteration=4,
                    counts=counts(prior_lines,stage['line_start'],stage['line_end']),
                    vector_spill_loads=stage['vector_spill_loads'],vector_spill_stores=stage['vector_spill_stores']))
    require(observed and comparison.get('prior_shared')==observed,'Prior counts/ranges must match actual C45 frozen evidence')
    for key in ('normalization','notes','spill_and_schedule_difference_notes'):
        require(bool(comparison.get(key)),'Missing same-work actual codegen comparison '+key)
    # This gate is codegen evidence only. It never asserts speed or submits N.
    review['performance_codegen_eligible']=comparison['distinct_from_prior_machine_arrangements']
...
def validate(base,version):
    ...
    for vl in (16,32,64):
        for threads in (1,4):
            ...
            one(r'^PASS: 96'+ending,dispatch,'C49 smoke96 missing')
            require(all(entries[x]>0 for x in ('rowpair','rowtriple','rowquad')),'C49 required helper entry absent')
            extra=dict(entry_check_scope='Unchanged C32/I smoke entries; no per-case new-loop count or worker mask')
    ...
    return dict(status='passed',complete=True,candidate=version,job_id=job['job_id'],scheduler=scheduler,exit_code=0,
        ...
        allocation=dict(cpus=cpus,numa_node=numa),assembly=assembly,
        performance_codegen_eligible=assembly['performance_codegen_eligible'],kh5_dynamic_coverage=False,
        performance_measured=False,performance_requires_root_review=True,
        automatic_performance_submission=False,spill_is_correctness_failure=False,executed_locally=False,executed_remotely=True,
        sanitizer='not_run',issues=[],note='Actual independent O diagnostics only; old C45 logs provide no C49 PASS; codegen false is a complete valid outcome that must not enter N. No speed or promotion claim.')
