 ...
 * Full-vector tails reuse adjacent inputs for each ordered kernel pair.
                          ...
                          float *restrict dst, int ow)
{
    ...
    while (ow - i >= 4 * lanes) {
        ...
        for (int jk = 0; jk < kh; ++jk) {
            ...
            int ik = 0;
            for (; ik < kw - 1; ik += 2) {
                const svfloat32_t k0 = svdup_n_f32(kr[ik]);
                const svfloat32_t k1 = svdup_n_f32(kr[ik + 1]);
                const float *p = row + ik;
                const svfloat32_t v0 = svld1(pg, p);
                const svfloat32_t v1 = svld1(pg, p + lanes);
                ...
                /* A shifted last window stays within the valid input row. */
