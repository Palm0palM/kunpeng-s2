"""Atomically freeze T evidence; local file copying/text inspection only.

Usage: python3 .runs/conv/sep13t-checks/freeze_returned.py VERSION [--failed]
...
EXPECTED={'C52-row7x3shared2':dict(full=4784,dispatch=972,direct=432,total=37128)}
...
def validate_archive_input(source,version,failed):
    ...
    require(hashes['conv2d.c']==SOURCE_SHA256 and prepared.get('source_parent')=='C51-row7x3u1',
        'T must retain the original independent C52 source identity')
    # Preserve the mandatory original T byte-count manifest; compare existing
    ...
    if failed:
        ...
        require(result.get('total_cases')==expected['total'] and result.get('runner_cases',0)==0
            ...
            'Accepted per-path/total counts differ from T plan')
        ...
        require(assembly.get('candidate')==version and assembly.get('job_id')==job_id
            ...
            and assembly.get('shared_pair_and_remainder_reviewed') is True
            and assembly.get('shared_unroll')==dict(paired_columns=2,odd_remainder_columns=1,other_stage_columns=1)
            and assembly.get('arithmetic_region_count_per_helper')==14,
            'Accepted independent shared pair/remainder identity or review missing')
...
def main():
    ...
    target=BASE.parent/args.version/'sve-correctness-sep13t'
    staging=target.with_name('.sve-correctness-sep13t.preparing')
