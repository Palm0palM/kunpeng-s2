"""Accept T returned text only. No SSH, compilation, tests or operator execution.
...
ROOT=BASE.parents[2]
PLANS={'C52-row7x3shared2':dict(full=4784,dispatch=972,direct=432,total=37128,vectors=3,rows=7,helper='conv_sve_rowseven',parent='C51-row7x3u1')}
...
def source_identity(base,job,version,require_returned=True):
    ...
    require(hashes['conv2d.c']==SOURCE_SHA256,'T requires its own fixed C52 production source')
    ...
    require(prepared.get('source_parent')==PLANS[version]['parent'] and prepared.get('rows_per_group')==7
        and prepared.get('vectors_per_row')==3 and prepared.get('accumulators')==21,'Prepared T source/shape mismatch')
    require(prepared.get('expected_semantic_stage_count')==13
        and prepared.get('shared_kernel_columns_per_iteration')==2
        and prepared.get('shared_odd_remainder_columns_per_iteration')==1
        and prepared.get('other_stage_kernel_columns_per_iteration')==1,'Prepared shared pair/remainder contract mismatch')
...
def assembly_review(base,job,p,source_hash):
    ...
    for helper in helpers:
        ...
        for stage in stages:
            if stage['stage']!='shared':
                ...
                continue
            # Shared is one semantic phase with two separately reviewed real
            # arithmetic regions. Do not invent a loop backedge for an odd tail.
            for key in ('source_mapping','control_flow_notes','accumulator_order_review',
                        'pair_to_remainder_review','transition_spill_review'):
                require(bool(stage.get(key)),'Missing shared interpretation '+key)
            blocks=stage.get('blocks') or []
            require([block.get('block') for block in blocks]==['paired','odd_remainder'],
                'Shared needs distinct paired and odd-remainder evidence; changed lowering requires root review')
            for block,work in zip(blocks,(2,1)):
                kinds=('loop',) if work==2 else ('loop','straight_line')
                require(block.get('region_kind') in kinds,'Unreviewed shared control-flow form')
                require(bool(block.get('entry_exit_notes')),'Missing actual shared entry/exit and kw path review')
                range_review(block,lines,outer,ranges,work)
                actual=block['derived_counts']
                require(actual['fmul']==actual['fadd']==21*work,
                    'Shared region arithmetic does not match reviewed work; retain actual lowering and stop')
    ...
    require(review.get('shape')==dict(rows_per_group=7,vectors_per_row=3,accumulators=21),
        'Missing actual review association with C52 seven-row/three-vector source shape')
    require(review.get('whole_machine_code_identical_claimed') is False,
        'Q/C51 is source background, not current C52 machine-code proof')
    require(review.get('shared_pair_and_remainder_reviewed') is True
        and review.get('shared_unroll')==dict(paired_columns=2,odd_remainder_columns=1,other_stage_columns=1),
        'Missing independent C52 shared pair/remainder review')
    review.update(stage_count=len(phase_ids),fma_count=fma,transitions_reviewed=True,
        arithmetic_region_count_per_helper=14,
...
def validate(base,version):
    ...
    for vl in (16,32,64):
        for threads in (1,4):
            ...
            one(r'^FULL_MATRIX_COUNTS core=3888 narrow=144 small=720 larger=32$',guard,'T matrix family mismatch')
            require(all(value>0 for value in entries.values()),'T legacy helper not entered')
            one(r'^PASS: 972 dispatch cases; exact per-case rowseven entries and bitwise scalar reference$',dispatch,'T dispatch972 missing')
            one(r'^PASS: 432 direct fallback cases; kh1\.\.6, readonly/guard/canary and bitwise scalar reference$',dispatch,'T direct432 missing')
            mask=(1<<threads)-1
            extra={}
    ...
    return dict(status='passed',complete=True,candidate=version,job_id=job['job_id'],scheduler=scheduler,exit_code=0,
        ...
        sanitizer='not_run',issues=[],note='Actual independent T diagnostics only; parent Q/C51 and older logs provide no C52 PASS. No speed or promotion claim.')
