"""Prepared T diagnostic transport for C52; explicit root --go only.
...
SOURCE_PARENTS = {'C52-row7x3shared2': 'C51-row7x3u1'}
COUNTS = {'C52-row7x3shared2': dict(full_per_configuration=4784, dispatch_per_configuration=972,
...
# The frozen T wrapper builds no retained production object for separate transfer.
...
def serial_gate(cfg):
    """Bounded reviewed R, optional S, and present P/T jobs; no teammate scan.

    Prepared P/T packages without job.json do not represent active jobs.
    ...
    job_paths = [ROOT / '.runs/conv/sep13p-checks/C50-row4dupfencenomem/job.json',
                 BASE / 'C52-row7x3shared2/job.json']
    job_paths += [ROOT / '.runs/conv' / name / 'cluster.json'
                  for name in ('C26-r24', 'C51-r1', 'C26-r25')]
    ...
    # S is higher priority. Any actual reservation must be reconciled, even if
    # its campaign is still prepared or was interrupted before submission.
    path = ROOT / '.runs/conv/sep13s-campaign.json'
    if path.exists():
        origin = str(path.relative_to(ROOT))
        job_id = read_json(path).get('performance_job')
        if not re.fullmatch(r'[0-9]+', str(job_id or '')):
            unresolved.append(dict(path=origin, reason='Existing S campaign has no reconciled job ID'))
        else:
            identities.setdefault(str(job_id), []).append(origin)
    path = ROOT / '.runs/conv/sep13r-campaign.json'
    origin = str(path.relative_to(ROOT))
    if not path.exists():
        unresolved.append(dict(path=origin, reason='Required reviewed R campaign evidence is missing'))
    else:
        job_id = read_json(path).get('performance_job')
        if str(job_id) != '1581911':
            unresolved.append(dict(path=origin, reason='R campaign differs from reviewed completed job1581911'))
        else:
            identities.setdefault(str(job_id), []).append(origin)
...
def frozen_source(base, version):
    ...
    require(expected['conv2d.c'] == SOURCE_SHA256, 'T requires its own root-reviewed C52 source')
    source = base / 'source'
    ...
    require(extra_manifest.is_file() and not extra_manifest.is_symlink(),
        'Frozen T bytes/SHA source-manifest.json is required')
    ...
    require(prepared.get('expected_acc') == 3 and prepared.get('expected_checks') == counts,
        'T matrix schema mismatch')
    require(prepared.get('rows_per_group') == 7 and prepared.get('vectors_per_row') == 3
        and prepared.get('accumulators') == 21, 'C52 seven-row/three-vector shape mismatch')
    ...
    require(prepared.get('expected_semantic_stage_count') == 13
        ...
        'C52 shared pair/remainder expectations mismatch')
    return expected
...
def main():
    ...
    parser.add_argument('--go', action='store_true', help='Explicit one-shot root T submission authorization')
    ...
    if args.action == 'submit':
        ...
        created = stamp()
        remote_dir = cfg['remote_root'] + '/diagnostics/conv-sep13t-' + args.version + '-' + created
        ...
        command = cluster.submit_command(cfg, remote_dir, 'conv-sep13t-' + args.version)
    ...
    print('Fetched immutable T diagnostic text/assembly. Fetch is not acceptance or performance.')
