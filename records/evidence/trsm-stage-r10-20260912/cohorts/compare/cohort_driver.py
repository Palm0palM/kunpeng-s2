...
def main():
    ...
    if (names != ['T8-control12-repeat-r10', 'T13-sve4x32']
            or order != [names, list(reversed(names)), names]):
        raise RuntimeError('The predeclared r10 protocol requires formal A/B, B/A, A/B rounds')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r10-one-official-warmup-v1'
    ...
    with (ROOT / 'preflight-wide32.log').open('xb') as log:
        code = stream(['/bin/bash', str(ROOT / 'preflight-wide32/run.sh'),
                       str(ROOT / 'T13-sve4x32/source'),
                       str(ROOT / 'preflight-wide32-results/T13-sve4x32')], ROOT, env, [log])
        if code:
            raise RuntimeError('Wide-kernel and large-path supplemental preflight failed')
    completion = (ROOT / 'preflight-wide32-results/T13-sve4x32/completion.txt').read_text()
