...
def prepare():
    ...
    if [item['run_name'] for item in plan['members']] != ['T8-control12-repeat-r10', 'T13-sve4x32']:
        raise RuntimeError('The predeclared r10 protocol requires exactly the planned A/B members')
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r10-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r10 measurements.',
