#!/usr/bin/env python3
"""Create one public, redacted r10 evidence archive; never run tests or digests.
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r10-20260912'
COHORTS = {
    'compare': RUNS / 'optimization-20260912-r10-compare',
}
VERSION_BY_MEMBER = {
    'T8-control12-repeat-r10': 'T8-control12',
    'T13-sve4x32': 'T13-sve4x32',
}
MEMBERS = tuple(VERSION_BY_MEMBER)
REPEAT_ORIGINS = {
    'T8-control12-repeat-r10': 'T8-control12-repeat-r9',
}
...
def main():
    ...
    for member in MEMBERS:
        ...
        for name in sorted(MEMBER_FILES):
            add(folder / name, prefix / name,
                required=member == 'T13-sve4x32' and name in ('PREPARATION.md', 'VALIDATION-PLAN.md'))
        for name in SOURCE_FILES:
    ...
    add(Path(__file__).resolve(), 'tools/archive-r10.py', required=True)

    # The T8 repeat keeps its separate pre-r10 record. Associate it with the
    # public r9 record and run using the known r9 job ID (no byte identity claim).
    history_archive = ROOT / 'records/evidence/trsm-stage-r9-20260912'
    expected_history_job = '1579479'
    ...
    for member, prior_run in REPEAT_ORIGINS.items():
        ...
        prior_record_origins.append({
            ...
            'prior_run': prior_run,
            'prior_public_archive': '../trsm-stage-r9-20260912/',
            'prior_public_record': '../trsm-stage-r9-20260912/records/latest/' + version + '.json',
            'prior_run_cluster': '../trsm-stage-r9-20260912/members/' + prior_run + '/cluster.json',
            ...
            'note': 'Job ID/version association only. The r9 archive retains the prior T8 record and its run evidence; older T8 history is referenced there. Original T8 baseline evidence is retained in r6.',
        })
        if not job_id_match or not version_match:
            missing.append(member + ' prior-record.json matching its public r9 record and job 1579479; inspect the referenced r9 archive')
...
File owner/group identifiers redacted for publication.
    ...
    document = {
        'schema_version': 1, 'problem': 'trsm', 'stage': 'r10-20260912',
        ...
        'repeat_policy': 'The r10 T8 repeat retains a separate prior record associated by version and job 1579479 with the corresponding public r9 record. Older T8 history is referenced by r9; original T8 baseline evidence is retained in r6. Samples from separate jobs are not merged.',
    ...
    (DEST / 'README.md').write_text('''# TRSM r10 公开实验归档
...
T13 自身的 PREPARATION、VALIDATION-PLAN 及已有 STATIC-REVIEW 与本轮 STATIC-CONTROLLER-REVIEW 另行保留；这些是候选准备和静态审查材料，不能作为实际运行通过验证的证据。不从其他候选目录补写准备文档，也不假定 T8 重复 run 会重新撰写候选文档。
...
本轮 T8 的 prior-record.json 保存在 T8-control12-repeat-r10 member 下，关联 r9 公开档的 [T8 记录](../trsm-stage-r9-20260912/records/latest/T8-control12.json)及 [T8-control12-repeat-r9 运行](../trsm-stage-r9-20260912/members/T8-control12-repeat-r9/cluster.json)。原记录及对应 run 的作业 ID 应为 1579479；关联结果见 ARCHIVE.json 的 prior_record_origins。关联失败会明确列为缺失/待核对事项，不借版本名认定。更早 T8 repeat/history 由 [r9 公开档](../trsm-stage-r9-20260912/README.md)继续引用；[原始 T8 初测](../trsm-stage-r6-20260912/members/T8-control12/)实际保存在 r6 公开档。r10 不重建或覆盖旧运行。records/latest 保存归档时的 T8-control12 和 T13-sve4x32 台账，不能把不同作业的样本合并为一次比较。
...
''')
    print('Created public TRSM r10 archive:', DEST.relative_to(ROOT), 'text_files=', len(inventory))
