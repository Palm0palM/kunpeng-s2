...
def prepare():
    ...
    if [item['run_name'] for item in plan['members']] != ['T8-control12-repeat-r11', 'T13-sve4x32-repeat-r11']:
        raise RuntimeError('The predeclared r11 protocol requires exactly the planned A/B members')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing') is not True or item['source_from'] != item['version']:
            raise RuntimeError('Both r11 members must repeat their original implementation version')
        ...
        for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
            if field not in prior or item.get(field) != prior[field]:
                raise RuntimeError('The r11 plan must preserve original implementation identity: ' + field)
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r11-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r11 measurements.',
    ...
    config['round_order'] = [names, list(reversed(names)), names]
