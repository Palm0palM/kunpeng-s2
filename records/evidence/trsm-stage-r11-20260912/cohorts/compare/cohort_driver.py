...
def main():
    ...
    if (names != ['T8-control12-repeat-r11', 'T13-sve4x32-repeat-r11']
            or order != [names, list(reversed(names)), names]):
        raise RuntimeError('The predeclared r11 protocol requires formal A/B, B/A, A/B rounds')
    for name, version in zip(names, ('T8-control12', 'T13-sve4x32')):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not True):
            raise RuntimeError('Both r11 members must retain their original version as repeat runs')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r11-one-official-warmup-v1'
    ...
    # T13 changes a 32-column large-path kernel requiring four-panel and CT-boundary checks.
    with (ROOT / 'preflight-wide32.log').open('xb') as log:
        code = stream(['/bin/bash', str(ROOT / 'preflight-wide32/run.sh'),
                       str(ROOT / 'T13-sve4x32-repeat-r11/source'),
                       str(ROOT / 'preflight-wide32-results/T13-sve4x32-repeat-r11')], ROOT, env, [log])
        if code:
            raise RuntimeError('Wide-kernel and large-path supplemental preflight failed')
    completion = (ROOT / 'preflight-wide32-results/T13-sve4x32-repeat-r11/completion.txt').read_text()
