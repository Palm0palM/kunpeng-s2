#!/usr/bin/env python3
"""Create one public, redacted r11 evidence archive; never run tests or digests.
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r11-20260912'
COHORTS = {
    'compare': RUNS / 'optimization-20260912-r11-compare',
}
VERSION_BY_MEMBER = {
    'T8-control12-repeat-r11': 'T8-control12',
    'T13-sve4x32-repeat-r11': 'T13-sve4x32',
}
MEMBERS = tuple(VERSION_BY_MEMBER)
REPEAT_ORIGINS = {
    'T8-control12-repeat-r11': 'T8-control12-repeat-r10',
    'T13-sve4x32-repeat-r11': 'T13-sve4x32',
}
...
def main():
    ...
    for member in MEMBERS:
        ...
        for path in sorted(folder.glob('*NOTES*.md')):
            add(path, prefix / path.name)
        assembly_reviews(folder, prefix)
        if member == 'T13-sve4x32-repeat-r11':
    ...
    add(Path(__file__).resolve(), 'tools/archive-r11.py', required=True)

    # Both repeats keep separate pre-r11 records. Associate each with its own
    # public r10 record and run using the known r10 job ID (no byte identity claim).
    history_archive = ROOT / 'records/evidence/trsm-stage-r10-20260912'
    expected_history_job = '1579585'
    ...
    for member, prior_run in REPEAT_ORIGINS.items():
        ...
        prior_record_origins.append({
            ...
            'prior_run': prior_run,
            'prior_public_archive': '../trsm-stage-r10-20260912/',
            'prior_public_record': '../trsm-stage-r10-20260912/records/latest/' + version + '.json',
            'prior_run_cluster': '../trsm-stage-r10-20260912/members/' + prior_run + '/cluster.json',
            ...
            'note': 'Job ID/version association only. The r10 archive retains both prior records and their corresponding run evidence; older T8 history is referenced there. Original T8 baseline evidence is retained in r6.',
        })
        if not job_id_match or not version_match:
            missing.append(member + ' prior-record.json matching its public r10 record and job 1579585; inspect the referenced r10 archive')
...
File owner/group identifiers redacted for publication.
    ...
    document = {
        'schema_version': 1, 'problem': 'trsm', 'stage': 'r11-20260912',
        ...
        'repeat_policy': 'The r11 T8 and T13 repeats each retain a separate prior record associated by version and job 1579585 with the corresponding public r10 record. Older T8 history is referenced by r10; original T8 baseline evidence is retained in r6. Samples from separate jobs are not merged.',
    ...
    (DEST / 'README.md').write_text('''# TRSM r11 公开实验归档
...
cohorts/compare 保存本轮唯一 cohort 的控制脚本、计划、提交元数据、finish_records.py、通用预检、preflight-wide32 宽核预检、KML 探针、实际取回日志，以及 submitted-payload 中的上传源码/脚本快照。preflight-wide32 的 guard.py、run.sh、check-wide32.c 和 README.md 分别保留当前准备版与冻结提交版。参数观察生成脚本和严格结果 parser 均完整内嵌在 run.sh 的 Python heredoc 中；没有独立脚本文件。实际 preflight-wide32-results、目标汇编及已存在的 assembly review 从结果/diagnostics 或 assembly-review 目录另行收录。members 保留 T8-control12-repeat-r11 与 T13-sve4x32-repeat-r11 两个运行目录。失败时只归档已经取回的日志；缺失项和各运行的已登记/未验证状态列在 ARCHIVE.json。

宽核结果中的 instrumented-trsm.c 是仅供参数观察的预检副本，不能当作实际 benchmark 源码；trsm-wide4x32.s 则由原候选源码生成。实际已有的 summary.tsv、summary.json、completion.txt、commands.txt、guard.json，以及 guard、compiler、instrument-source、build-normal、build-no-sve、build-fail-shared、assembly、micro-t1、normal-t1、normal-t4、normal-t38、shared-fail-t4、no-sve-t4、narrow-vl-t4、verify-results 日志按原结果目录收录。检查程序二进制不归档，缺失的运行结果不生成。

T13 重复运行若没有独立 PREPARATION 或 VALIDATION-PLAN，相应文件从原 .runs/trsm/T13-sve4x32 目录收录到该 member 的 candidate-preparation 下，来源逐项记录于 candidate_preparation_origins 及文件清单；STATIC-REVIEW 仅在当前重复目录或原候选目录已有文件时收录，不补写不存在的审查报告。它们说明既有候选，不能作为 r11 运行通过验证的证据。本轮 STATIC-CONTROLLER-REVIEW 另行保留，不假定重复 run 会重新撰写候选文档。
...
本轮 T8 与 T13 的 prior-record.json 分别保存在 T8-control12-repeat-r11 与 T13-sve4x32-repeat-r11 member 下，关联 r10 公开档的 [T8 记录](../trsm-stage-r10-20260912/records/latest/T8-control12.json)与 [T13 记录](../trsm-stage-r10-20260912/records/latest/T13-sve4x32.json)，以及对应的 [T8-control12-repeat-r10 运行](../trsm-stage-r10-20260912/members/T8-control12-repeat-r10/cluster.json)和 [T13-sve4x32 运行](../trsm-stage-r10-20260912/members/T13-sve4x32/cluster.json)。两份原记录及对应 run 的作业 ID 都应为 1579585；逐份关联结果见 ARCHIVE.json 的 prior_record_origins。关联失败会明确列为缺失/待核对事项，不借版本名认定。更早 T8 repeat/history 由 [r10 公开档](../trsm-stage-r10-20260912/README.md)继续引用；[原始 T8 初测](../trsm-stage-r6-20260912/members/T8-control12/)实际保存在 r6 公开档。r11 不重建或覆盖旧运行。records/latest 保存归档时的 T8-control12 和 T13-sve4x32 台账，仅按本轮 job ID 关联已有声明，不预判重复比较或晋级结论，也不把不同作业的样本合并为一次比较。
...
''')
    print('Created public TRSM r11 archive:', DEST.relative_to(ROOT), 'text_files=', len(inventory))
