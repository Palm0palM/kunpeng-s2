#!/usr/bin/env python3
"""Register collected r12 logs without running task code or computing digests."""
...
names = ['T8-control12-repeat-r12', 'T13-sve4x32-repeat-r12', 'T15-svetile128', 'T16-svetile32']
...
audit['wide_preflight_completions'] = {}
audit['wide_preflight_summaries'] = {}
for name, expected_ct in zip(names[1:], (64, 128, 32)):
    folder = DEST / 'preflight-wide32-results' / name
    wide = (folder / 'completion.txt').read_text().strip()
    if wide != ('TRSM_WIDE32_PREFLIGHT_COMPLETE=1 MICRO_CASES=28 WHOLE_CASES=28 '
                'SHARED_FAIL_CASES=1 NO_SVE_CASES=1 NARROW_VL_CASES=1'):
        raise SystemExit('Wide/tile preflight completion missing: ' + name)
    wide_summary = json.loads((folder / 'summary.json').read_text())
    if (wide_summary.get('complete') is not True or wide_summary.get('micro_cases') != 28
            or wide_summary.get('whole_cases') != 28 or wide_summary.get('argument_checked_cases') != 56
            or wide_summary.get('kernel_argument_mismatches') != 0
            or wide_summary.get('original_candidate_modified') is not False
            or wide_summary.get('expected_CT') != expected_ct or wide_summary.get('KB') != 256
            or wide_summary.get('tile_config_processes') != 7):
        raise SystemExit('Wide/tile preflight argument evidence missing: ' + name)
    audit['wide_preflight_completions'][name] = wide
    audit['wide_preflight_summaries'][name] = wide_summary
...
comparisons = {}
for candidate in ('T13-sve4x32', 'T15-svetile128', 'T16-svetile32'):
...
medians = {name: json.loads((ROOT / 'records/experiments/trsm' / (name + '.json')).read_text())['total_median_ms']
           for name in ('T8-control12', 'T13-sve4x32', 'T15-svetile128', 'T16-svetile32')}
summary = dict(status='compared_pending_review', job_id=GROUP['job_id'], formal_pass_count=36,
               warmup_pass_count=12, baseline_version='T8-control12', best_implementation='T8-svepanel16',
