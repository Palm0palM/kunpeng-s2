...
def main():
    ...
    if (names != ['T8-control12-repeat-r12', 'T13-sve4x32-repeat-r12', 'T15-svetile128', 'T16-svetile32']
            or order != [names[i:] + names[:i] for i in range(3)]):
        raise RuntimeError('The predeclared r12 protocol requires formal A/B/C/D, B/C/D/A, C/D/A/B rounds')
    for index, (name, version) in enumerate(zip(names, ('T8-control12', 'T13-sve4x32', 'T15-svetile128', 'T16-svetile32'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index < 2)):
            raise RuntimeError('The r12 members must retain their declared versions and repeat flags')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r12-one-official-warmup-v1'
    ...
    # All three wide kernels share identical dimensions/fallback gates, with explicit CT identity.
    with (ROOT / 'preflight-wide32.log').open('xb') as log:
        for name, expected_ct in zip(names[1:], (64, 128, 32)):
            code = stream(['/bin/bash', str(ROOT / 'preflight-wide32/run.sh'),
                           str(ROOT / name / 'source'),
                           str(ROOT / 'preflight-wide32-results' / name), str(expected_ct)], ROOT, env, [log])
            if code:
                raise RuntimeError('Wide/tile supplemental preflight failed for ' + name)
            completion = (ROOT / 'preflight-wide32-results' / name / 'completion.txt').read_text()
            if 'TRSM_WIDE32_PREFLIGHT_COMPLETE=1' not in completion:
                raise RuntimeError('Wide supplemental completion missing for ' + name)
    handles = {}
    try:
        member_details = {}
        for name in names:
