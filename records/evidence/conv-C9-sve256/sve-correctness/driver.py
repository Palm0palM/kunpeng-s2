...
import cluster
base=Path('.runs/conv/C9-sve256/sve-correctness')
...
if action=='submit':
    ...
    remote_dir=cfg['remote_root']+'/diagnostics/conv-sve256-correctness-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command=cluster.submit_command(cfg,remote_dir,'conv-sve256-correctness')
