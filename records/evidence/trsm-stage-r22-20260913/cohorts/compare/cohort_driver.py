...
def main():
    ...
    if (names != ['T19-control13-repeat-r22', 'T24-smallbroadcast']
            or order != [names, names[::-1], names]):
        raise RuntimeError('The r22 protocol requires A/B, B/A, A/B formal suites')
    for index, (name, version) in enumerate(zip(names, ('T19-control13', 'T24-smallbroadcast'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index == 0)):
            raise RuntimeError('The r22 pair must retain its repeat/new identities')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r22-one-official-warmup-v1'
    ...
    with (ROOT / 'preflight-panel8x16.log').open('xb') as log:
        for name in names:
            ...
            if code:
                raise RuntimeError('Small-panel8x16 preflight failed for ' + name)
            panel_check = audit_panel8x16(ROOT / 'preflight-panel8x16-results' / name,
                                          ROOT / name / 'source/trsm.c')
            if panel_check['guard']['scheduler_job_id'] != job:
                raise RuntimeError('Dedicated preflight guard differs from confirmed cohort job')
    handles = {}
    try:
        member_details = {}
        for name in names:
