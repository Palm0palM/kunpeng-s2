#!/usr/bin/env python3
"""Register collected r22 logs without running task code or computing digests."""
...
with (HERE / 'finish-attempt.json').open('x') as handle:
    ...
    handle.write('\n')
spec = importlib.util.spec_from_file_location('r22_finish_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
names = ['T19-control13-repeat-r22', 'T24-smallbroadcast']
versions = ['T19-control13', 'T24-smallbroadcast']
if (list(config['members']) != names or config['round_order'] != [names[i:] + names[:i] for i in range(3)]
        ...
        or config['warmup_protocol'].get('protocol_id') != 'trsm-r22-one-official-warmup-v1'
    ...
    raise SystemExit('r22 protocol identity changed')
for index, (name, version) in enumerate(zip(names, versions)):
    if config['members'][name].get('version') != version or config['members'][name].get('repeat_existing') is not (index == 0):
        raise SystemExit('r22 requires one baseline repeat and one new T24 version')
...
for index, (name, version) in enumerate(zip(names, versions)):
    ...
    if index == 0:
        ...
        if prior.get('job_id') != '1582392' or meta.get('prior_job_id') != '1582392':
            raise SystemExit('r22 must preserve its declared new-baseline prior record')
        ...
        if (prepared_bytes[version] != helper.regular_bytes(run / 'experiment.json')
            ...
            raise SystemExit('The new T24 member already has changed or recorded experiment evidence')
...
audit['panel8x16_preflight'] = {}
for name in names:
    checked = audit_panel8x16(DEST / 'preflight-panel8x16-results' / name,
                              HERE / 'payload' / name / 'source/trsm.c')
    if checked['guard']['scheduler_job_id'] != GROUP['job_id']:
        raise SystemExit('Dedicated preflight guard differs from collected cohort job')
    audit['panel8x16_preflight'][name] = checked
...
for index, (name, version) in enumerate(zip(names, versions)):
    expected_record = prior_bytes[version] if index == 0 else prepared_bytes[version]
...
comparisons = {}
for candidate in ('T24-smallbroadcast',):
...
summary = dict(status='compared_pending_root_review', job_id=GROUP['job_id'], formal_pass_count=18,
               warmup_pass_count=6, baseline_version='T19-control13', best_implementation='T19-panel8x16budget',
               comparisons=comparisons, medians_ms={key: value['total_median_ms'] for key, value in recorded.items()},
...
with (HERE / 'finish-complete.json').open('x') as handle:
    json.dump(dict(job_id=GROUP['job_id'], status='compared_pending_root_review', formal_pass_count=18,
                   warmup_pass_count=6, repeat_versions=versions[:1], new_versions=versions[1:], promoted=False), handle, indent=2)
    handle.write('\n')
