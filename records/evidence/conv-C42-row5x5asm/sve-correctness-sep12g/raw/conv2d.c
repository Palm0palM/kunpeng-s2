...
#if CONV_CAN_DISPATCH_SVE
/* Five outputs share input row t; output r consumes kernel row t-r.
 * Each output keeps increasing kernel row/column order and separate mul/add. */
__attribute__((target("arch=armv8-a+sve"), noinline))
static void conv_sve_rowquint(const float *restrict base, size_t stride,
                              const float *restrict kernel, int kh, int kw,
                              float *restrict dst0, float *restrict dst1,
                              float *restrict dst2, float *restrict dst3,
                              float *restrict dst4, int ow)
{
    if (kh < 5) {
        conv_sve_rowquad(base, stride, kernel, kh, kw, dst0, dst1, dst2, dst3, ow);
        conv_sve_prefix(base + (size_t)4 * stride, stride, kernel, kh, kw, dst4, ow);
        return;
    }
    const int lanes = (int)svcntw();
    const int block = 5 * lanes;
    const svbool_t pg = svptrue_b32();
    int i = 0;
    for (; ow - i >= block; i += block) {
        svfloat32_t a0 = svdup_n_f32(0.0f);
        ...
        svfloat32_t c2 = svdup_n_f32(0.0f);
        svfloat32_t c3 = svdup_n_f32(0.0f);
        svfloat32_t c4 = svdup_n_f32(0.0f);
        svfloat32_t d0 = svdup_n_f32(0.0f);
        svfloat32_t d1 = svdup_n_f32(0.0f);
        svfloat32_t d2 = svdup_n_f32(0.0f);
        svfloat32_t d3 = svdup_n_f32(0.0f);
        svfloat32_t d4 = svdup_n_f32(0.0f);
        svfloat32_t e0 = svdup_n_f32(0.0f);
        svfloat32_t e1 = svdup_n_f32(0.0f);
        svfloat32_t e2 = svdup_n_f32(0.0f);
        svfloat32_t e3 = svdup_n_f32(0.0f);
        svfloat32_t e4 = svdup_n_f32(0.0f);
        /* Input row 0: start output 0 after advancing earlier outputs. */
        {
            const float *row = base + i;
            const float *ka = kernel;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ak = svdup_n_f32(ka[ik]);
                /* Stream one input window in source; actual scheduling is compiler-owned. */
                {
                ...
                {
                    const svfloat32_t v = svld1(pg, row + ik + 3 * lanes);
                    a3 = svadd_f32_x(pg, a3, svmul_f32_x(pg, v, ak));
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 4 * lanes);
                    a4 = svadd_f32_x(pg, a4, svmul_f32_x(pg, v, ak));
                }
            }
        }
        /* Input row 1: start output 1 after advancing earlier outputs. */
        {
            const float *row = base + (size_t)1 * stride + i;
            const float *ka = kernel + ((size_t)1) * kw;
            const float *kb = kernel;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ak = svdup_n_f32(ka[ik]);
                const svfloat32_t bk = svdup_n_f32(kb[ik]);
                ...
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 3 * lanes);
                    a3 = svadd_f32_x(pg, a3, svmul_f32_x(pg, v, ak));
                    b3 = svadd_f32_x(pg, b3, svmul_f32_x(pg, v, bk));
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 4 * lanes);
                    a4 = svadd_f32_x(pg, a4, svmul_f32_x(pg, v, ak));
                    b4 = svadd_f32_x(pg, b4, svmul_f32_x(pg, v, bk));
                }
            }
        }
        /* Input row 2: start output 2 after advancing earlier outputs. */
        {
            const float *row = base + (size_t)2 * stride + i;
            const float *ka = kernel + ((size_t)2) * kw;
            const float *kb = kernel + ((size_t)1) * kw;
            const float *kc = kernel;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ak = svdup_n_f32(ka[ik]);
                const svfloat32_t bk = svdup_n_f32(kb[ik]);
                ...
                }
                {
                    ...
                    a3 = svadd_f32_x(pg, a3, svmul_f32_x(pg, v, ak));
                    b3 = svadd_f32_x(pg, b3, svmul_f32_x(pg, v, bk));
                    c3 = svadd_f32_x(pg, c3, svmul_f32_x(pg, v, ck));
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 4 * lanes);
                    a4 = svadd_f32_x(pg, a4, svmul_f32_x(pg, v, ak));
                    b4 = svadd_f32_x(pg, b4, svmul_f32_x(pg, v, bk));
                    c4 = svadd_f32_x(pg, c4, svmul_f32_x(pg, v, ck));
                }
            }
        }
        /* Input row 3: start output 3 after advancing earlier outputs. */
        {
            const float *row = base + (size_t)3 * stride + i;
            const float *ka = kernel + ((size_t)3) * kw;
            const float *kb = kernel + ((size_t)2) * kw;
            const float *kc = kernel + ((size_t)1) * kw;
            ...
            for (int ik = 0; ik < kw; ++ik) {
                ...
                }
                {
                    ...
                    b4 = svadd_f32_x(pg, b4, svmul_f32_x(pg, v, bk));
                    c4 = svadd_f32_x(pg, c4, svmul_f32_x(pg, v, ck));
                    d4 = svadd_f32_x(pg, d4, svmul_f32_x(pg, v, dk));
                }
            }
        }
        /* Shared interior input rows advance all five outputs. */
        for (int t = 4; t < kh; ++t) {
            const float *row = base + (size_t)t * stride + i;
            const float *ka = kernel + ((size_t)t) * kw;
            const float *kb = kernel + ((size_t)(t - 1)) * kw;
            const float *kc = kernel + ((size_t)(t - 2)) * kw;
            const float *kd = kernel + ((size_t)(t - 3)) * kw;
            const float *ke = kernel + ((size_t)(t - 4)) * kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ak = svdup_n_f32(ka[ik]);
                const svfloat32_t bk = svdup_n_f32(kb[ik]);
                const svfloat32_t ck = svdup_n_f32(kc[ik]);
                ...
                /* Stream one input window in source; actual scheduling is compiler-owned. */
                {
                    ...
                    __asm__ __volatile__(
                        "fmul %Z[t].s, %Z[v].s, %Z[ka].s\n\t"
                        "fadd %Z[a].s, %Z[a].s, %Z[t].s\n\t"
                        "fmul %Z[t].s, %Z[v].s, %Z[kb].s\n\t"
                        "fadd %Z[b].s, %Z[b].s, %Z[t].s\n\t"
                        "fmul %Z[t].s, %Z[v].s, %Z[kc].s\n\t"
                        "fadd %Z[c].s, %Z[c].s, %Z[t].s\n\t"
                        "fmul %Z[t].s, %Z[v].s, %Z[kd].s\n\t"
                        "fadd %Z[d].s, %Z[d].s, %Z[t].s\n\t"
                        "fmul %Z[t].s, %Z[v].s, %Z[ke].s\n\t"
                        "fadd %Z[e].s, %Z[e].s, %Z[t].s\n\t"
                        : [a] "+&w"(a0), [b] "+&w"(b0),
                          [c] "+&w"(c0), [d] "+&w"(d0),
                          [e] "+&w"(e0), [t] "=&w"(tmp)
                        : [v] "w"(v), [ka] "w"(ak), [kb] "w"(bk),
                          [kc] "w"(ck), [kd] "w"(dk), [ke] "w"(ek)
                        : "memory");
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 1 * lanes);
        ...
        /* Trailing input row kh+0: finish output 1 and advance later outputs. */
        {
            ...
            const float *kc = kernel + ((size_t)(kh - 2)) * kw;
            const float *kd = kernel + ((size_t)(kh - 3)) * kw;
            const float *ke = kernel + ((size_t)(kh - 4)) * kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t bk = svdup_n_f32(kb[ik]);
                const svfloat32_t ck = svdup_n_f32(kc[ik]);
                const svfloat32_t dk = svdup_n_f32(kd[ik]);
                ...
                }
                {
                    ...
                    c3 = svadd_f32_x(pg, c3, svmul_f32_x(pg, v, ck));
                    d3 = svadd_f32_x(pg, d3, svmul_f32_x(pg, v, dk));
                    e3 = svadd_f32_x(pg, e3, svmul_f32_x(pg, v, ek));
                }
                {
                    const svfloat32_t v = svld1(pg, row + ik + 4 * lanes);
                    b4 = svadd_f32_x(pg, b4, svmul_f32_x(pg, v, bk));
                    c4 = svadd_f32_x(pg, c4, svmul_f32_x(pg, v, ck));
                    d4 = svadd_f32_x(pg, d4, svmul_f32_x(pg, v, dk));
                    e4 = svadd_f32_x(pg, e4, svmul_f32_x(pg, v, ek));
                }
            }
        }
        /* Trailing input row kh+1: finish output 2 and advance later outputs. */
        {
            ...
            const float *kc = kernel + ((size_t)(kh - 1)) * kw;
            const float *kd = kernel + ((size_t)(kh - 2)) * kw;
            const float *ke = kernel + ((size_t)(kh - 3)) * kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ck = svdup_n_f32(kc[ik]);
                const svfloat32_t dk = svdup_n_f32(kd[ik]);
                const svfloat32_t ek = svdup_n_f32(ke[ik]);
        ...
        /* Trailing input row kh+3: finish output 4 and advance later outputs. */
        {
            ...
            const float *ke = kernel + ((size_t)(kh - 1)) * kw;
            for (int ik = 0; ik < kw; ++ik) {
                const svfloat32_t ek = svdup_n_f32(ke[ik]);
                /* Stream one input window in source; actual scheduling is compiler-owned. */
                {
        ...
        svst1(pg, dst2 + i + 2 * lanes, c2);
        svst1(pg, dst2 + i + 3 * lanes, c3);
        svst1(pg, dst2 + i + 4 * lanes, c4);
        svst1(pg, dst3 + i + 0 * lanes, d0);
        svst1(pg, dst3 + i + 1 * lanes, d1);
        svst1(pg, dst3 + i + 2 * lanes, d2);
        svst1(pg, dst3 + i + 3 * lanes, d3);
        svst1(pg, dst3 + i + 4 * lanes, d4);
        svst1(pg, dst4 + i + 0 * lanes, e0);
        svst1(pg, dst4 + i + 1 * lanes, e1);
        svst1(pg, dst4 + i + 2 * lanes, e2);
        svst1(pg, dst4 + i + 3 * lanes, e3);
        svst1(pg, dst4 + i + 4 * lanes, e4);
    }
    if (i < ow) {
        conv_sve_rowquad(base + i, stride, kernel, kh, kw,
                         dst0 + i, dst1 + i, dst2 + i, dst3 + i, ow - i);
        conv_sve_prefix(base + (size_t)4 * stride + i, stride, kernel, kh, kw, dst4 + i, ow - i);
    }
}
#endif
            ...
            CONVFLOAT *output)
{
    ...
    if (use_sve && kernelHeight >= 5 && oh >= 5) {
        const size_t output_rows = (size_t)oh;
        const size_t output_stride = (size_t)ow;
        const size_t groups = output_rows / 5 + (output_rows % 5 != 0);
#pragma omp parallel for schedule(static)
        for (size_t group = 0; group < groups; ++group) {
            const size_t row = group * 5;
            const size_t remaining = output_rows - row;
            const float *base = input + row * stride;
            float *dst = output + row * output_stride;
            if (remaining >= 5) {
                conv_sve_rowquint(base, stride, kernel, kernelHeight, kernelWidth,
                ...
                conv_sve_rowtriple(base, stride, kernel, kernelHeight, kernelWidth,
                                   dst, dst + output_stride, dst + 2 * output_stride, ow);
            } else if (remaining == 2) {
                conv_sve_rowpair(base, stride, kernel, kernelHeight, kernelWidth,
                                 dst, dst + output_stride, ow);
            } else {
                conv_sve_prefix(base, stride, kernel, kernelHeight, kernelWidth, dst, ow);
            }
        }
        return;
    }
