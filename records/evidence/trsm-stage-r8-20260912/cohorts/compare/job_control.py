...
def prepare():
    ...
    if [item['run_name'] for item in plan['members']] != ['T8-control12-repeat-r8', 'T11-sve8x16']:
        raise RuntimeError('The predeclared r8 protocol requires exactly the planned A/B members')
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r8-one-official-warmup-v1', declared_at=group['created_at'],
        ...
        purpose='One identical full official suite per member after general preflight, to apply the '
                'startup-warming protocol declared before viewing r8 measurements.',
    ...
    config['round_order'] = [names.copy(), list(reversed(names)), names.copy()]
    ...
    for directory in ('preflight', 'preflight-wide', 'reference'):
