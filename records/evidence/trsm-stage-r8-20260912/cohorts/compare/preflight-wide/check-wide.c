...
static int micro_check(void) {
    ...
    for (int packed = 0; packed <= 1; ++packed) {
        ...
        int ldc = 23 + (int)pi * 4, ldx = packed ? 8 : 31 + (int)pi * 2;
        size_t nl = (size_t)8 * lda + 2 * GUARD;
        size_t nx = (packed ? 2u * DIRECT_KB * 8 : (size_t)DIRECT_KB * ldx) + 2 * GUARD;
        size_t nc = (size_t)8 * ldc + 2 * GUARD;
        ...
        size_t second = packed ? (size_t)DIRECT_KB * 8 : 8;
        double *x1 = x0 + second;
        for (int r = 0; r < 8; ++r) for (int k = 0; k < count; ++k)
            l[(size_t)r * lda + k] = (2 * ((r * 7 + k * 11) % 29) - 27) / 101.0;
        for (int k = 0; k < count; ++k) for (int j = 0; j < 8; ++j) {
            x0[(size_t)k * ldx + j] = (2 * ((k * 13 + j * 17) % 47) - 45) / 71.0;
            x1[(size_t)k * ldx + j] = (2 * ((k * 19 + j * 23) % 53) - 51) / 79.0;
        }
        for (int r = 0; r < 8; ++r) for (int j = 0; j < 16; ++j)
        ...
        for (int r = 0; r < 8; ++r) for (int j = 0; j < 16; ++j) {
            double sum = 0;
            size_t panel_origin = GUARD + (j >= 8 ? second : 0);
        ...
        reset_counters();
        update8x16_sve(count, l, lda, x0, x1, ldx, c, ldc);
        unsigned long calls = __atomic_load_n(&wide_entries, __ATOMIC_RELAXED);
        int bad = calls != 1 || memcmp(cs, want, nc * sizeof(double))
...
static unsigned long expected_entries(int m, int n) {
    ...
    /* Count complete output 8-row groups below each solved 256-row block.
     * Since CT=64 is divisible by 8 and 16, tile boundaries do not discard
     * any complete 8-row or 16-column group. This does not replay call loops. */
    for (int end = DIRECT_KB; end < m; end += DIRECT_KB)
        total += (unsigned long)((m - end) / 8) * (unsigned long)(n / 16);
    return total;
}
...
static int case_check(int m, int n) {
    ...
    int bad = calls != expected, shared_fail = 0;
...
#pragma omp parallel reduction(min:min_vl) reduction(max:max_vl) reduction(&:widths_ok) reduction(+:workers)
    ...
    if (case_check(4105, 25)) return 1;
    if (!strcmp(mode, "full") && case_check(4111, 31)) return 1;
