#!/usr/bin/env python3
"""Register collected r8 logs without running task code or computing digests."""
...
names = ['T8-control12-repeat-r8', 'T11-sve8x16']
...
wide = (DEST / 'preflight-wide-results/T11-sve8x16/completion.txt').read_text().strip()
if wide != ('TRSM_WIDE_PREFLIGHT_COMPLETE=1 MICRO_CASES=28 WHOLE_CASES=8 '
...
argv = ['python3', '.runs/trsm/nohash-tools/records-kml.py', 'compare', 'T8-control12', 'T11-sve8x16']
result = subprocess.run(argv, cwd=ROOT, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
(HERE / 'T8-control12-vs-T11-sve8x16.json').write_bytes(result.stdout)
commands['executed_argv'].append(argv)
commands['exit_codes'].append(result.returncode)
(HERE / 'record-compare-commands.json').write_text(json.dumps(commands, indent=2) + '\n')
print(result.stdout.decode(), end='')
...
medians = {name: json.loads((ROOT / 'records/experiments/trsm' / (name + '.json')).read_text())['total_median_ms']
           for name in ('T8-control12', 'T11-sve8x16')}
summary = dict(status='compared_pending_review', job_id=GROUP['job_id'], formal_pass_count=18,
               ...
               comparison=comparison, medians_ms=medians, reference='KML25.1/GCC12; not specified KML25.2',
               promoted=False)
