#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Archive existing r21 evidence once; never execute or synthesize tests."""
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r21-20260913'
COHORTS = {
    'compare': ('optimization-20260913-r21-compare', ['T19-control13-repeat-r21', 'T23-smallunroll2']),
}
...
def main():
    ...
    associations = {}
    previous = ROOT / 'records/evidence/trsm-stage-r20-20260913/records/latest'
    for version, member in zip(('T19-control13',), COHORTS['compare'][1][:1]):
        ...
        valid = (prior['version'] == version and prior['job_id'] == '1582239'
                 and json.loads(redact(json.dumps(prior))) == previous_public)
        if not valid:
            raise RuntimeError('Repeat prior differs from archived r20 record: ' + version)
        associations[version] = dict(valid=True, prior_job_id='1582239',
                                     prior_public_record=str((previous / (version + '.json')).relative_to(ROOT)))
    associated = all(x['valid'] for x in associations.values())
    add(ROOT / 'records/experiments/trsm/T23-smallunroll2.json',
        Path('records/latest/T23-smallunroll2.json'), required=True)
    for name in ('PREPARATION.md', 'VALIDATION-PLAN.md', 'STATIC-REVIEW.md'):
        add(RUNS / 'T23-smallunroll2' / name, Path('members/T23-smallunroll2') / name, required=True)
    for name in ('T19-control13-vs-T23-smallunroll2.json',):
    ...
    add(Path(__file__), Path('tools/archive-r21.py'), required=True)
    if missing:
    ...
    manifest = dict(schema_version=1, stage='r21-20260913', problem='trsm',
    ...
    (DEST / 'README.md').write_text("""# TRSM r21 evidence

This redacted public derivative preserves the same-allocation comparison of
unchanged T19-control13 with new T23-smallunroll2. Submitted inputs, source
...
r20 record from job1582239; historical samples are never pooled.
...
""")
    print('Archived', len(inventory), 'r21 text files; prior association valid, required missing=0')
