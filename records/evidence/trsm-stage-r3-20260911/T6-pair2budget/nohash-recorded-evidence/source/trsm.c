...
/* Candidate tuning budget: grouped RHS storage plus ROWS L rows must fit
 * within 256 KiB. This is a policy budget, not a hardware cache-size claim. */
enum { PANEL_GROUP_SIZE = 2 };
static void solve_panel_grouped(int m,int n,const double *L,int lda,double *B,int ldb)
{
    if(m<=0 || n<=0) return;
#pragma omp parallel
    {
        /* Group adjacent panels only when at least two groups per actual
         * worker remain. Each panel revisits the same current L row block. */
        const int panel_count=(n-1)/RHS+1;
        const int panels_per_group=panel_count/omp_get_num_threads()>=2*PANEL_GROUP_SIZE?PANEL_GROUP_SIZE:1;
...
void l_trsm(int m,int n,const double *L,int lda,double *B,int ldb)
{
    ...
    else if((size_t)m*(PANEL_GROUP_SIZE*RHS+ROWS)*sizeof(double)<=256u*1024u)
        solve_panel_grouped(m,n,L,lda,B,ldb);
    else
