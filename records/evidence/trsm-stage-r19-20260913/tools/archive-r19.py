#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Archive existing r19 evidence once; never execute or synthesize tests."""
...
RUNS = ROOT / '.runs/trsm'
DEST = ROOT / 'records/evidence/trsm-stage-r19-20260913'
COHORTS = {
    'compare': ('optimization-20260913-r19-compare', ['T19-control13-repeat-r19', 'T20-wideunroll2-repeat-r19', 'T21-unrollfence']),
}
...
def main():
    ...
    associations = {}
    previous = ROOT / 'records/evidence/trsm-stage-r18-20260913/records/latest'
    for version, member in zip(('T19-control13', 'T20-wideunroll2'), COHORTS['compare'][1]):
        ...
        valid = (prior['version'] == version and prior['job_id'] == '1582004'
                 and json.loads(redact(json.dumps(prior))) == previous_public)
        if not valid:
            raise RuntimeError('Repeat prior differs from archived r18 record: ' + version)
        associations[version] = dict(valid=True, prior_job_id='1582004',
                                     prior_public_record=str((previous / (version + '.json')).relative_to(ROOT)))
    associated = all(x['valid'] for x in associations.values())
    add(ROOT / 'records/experiments/trsm/T21-unrollfence.json',
        Path('records/latest/T21-unrollfence.json'), required=True)
    for name in ('PREPARATION.md', 'VALIDATION-PLAN.md', 'STATIC-REVIEW.md'):
        add(RUNS / 'T21-unrollfence' / name, Path('members/T21-unrollfence') / name, required=True)
    for name in ('T19-control13-vs-T21-unrollfence.json', 'T20-wideunroll2-vs-T21-unrollfence.json'):
    ...
    add(Path(__file__), Path('tools/archive-r19.py'), required=True)
    if missing:
    ...
    manifest = dict(schema_version=1, stage='r19-20260913', problem='trsm',
    ...
    (DEST / 'README.md').write_text("""# TRSM r19 evidence

This redacted public derivative preserves the same-allocation comparison of
unchanged T19-control13 and T20-wideunroll2 with new T21-unrollfence. Submitted
...
their archived r18 records from job1582004; historical samples are never pooled.
...
T19 is the promotion parent; T20-to-T21 is marked mechanism-only. Original T20
preparation is stored as historical, while current T21 preparation is retained.
...
""")
    print('Archived', len(inventory), 'r19 text files; prior association valid, required missing=0')
