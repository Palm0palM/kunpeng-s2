...
def main():
    ...
    if (names != ['T19-control13-repeat-r19', 'T20-wideunroll2-repeat-r19', 'T21-unrollfence']
            or order != [names[i:] + names[:i] for i in range(3)]):
        raise RuntimeError('The r19 protocol requires ABC/BCA/CAB formal suites')
    for index, (name, version) in enumerate(zip(names, ('T19-control13', 'T20-wideunroll2', 'T21-unrollfence'))):
        if (config['members'][name].get('version') != version
                or config['members'][name].get('repeat_existing') is not (index < 2)):
            raise RuntimeError('The r19 three-member cohort must retain its repeat/new identities')
    protocol = config.get('warmup_protocol', {})
    if (config.get('warmup_order') != names or config.get('warmup_suites_per_member') != 1
            or protocol.get('protocol_id') != 'trsm-r19-one-official-warmup-v1'
