#!/usr/bin/env python3
"""Register collected r19 logs without running task code or computing digests."""
...
with (HERE / 'finish-attempt.json').open('x') as handle:
    ...
    handle.write('\n')
spec = importlib.util.spec_from_file_location('r19_finish_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
names = ['T19-control13-repeat-r19', 'T20-wideunroll2-repeat-r19', 'T21-unrollfence']
versions = ['T19-control13', 'T20-wideunroll2', 'T21-unrollfence']
if (list(config['members']) != names or config['round_order'] != [names[i:] + names[:i] for i in range(3)]
        ...
        or config['warmup_protocol'].get('protocol_id') != 'trsm-r19-one-official-warmup-v1'
    ...
    raise SystemExit('r19 protocol identity changed')
for index, (name, version) in enumerate(zip(names, versions)):
    if config['members'][name].get('version') != version or config['members'][name].get('repeat_existing') is not (index < 2):
        raise SystemExit('r19 requires two repeats and one new T21 version')
...
for index, (name, version) in enumerate(zip(names, versions)):
    ...
    if index < 2:
        ...
        if prior.get('job_id') != '1582004' or meta.get('prior_job_id') != '1582004':
            raise SystemExit('r19 must preserve its declared r18 prior records')
        ...
        if (prepared_bytes[version] != helper.regular_bytes(run / 'experiment.json')
            ...
            raise SystemExit('The new T21 member already has changed or recorded experiment evidence')
...
comparisons = {}
for candidate in ('T20-wideunroll2', 'T21-unrollfence'):
...
mechanism = {}
base, candidate = 'T20-wideunroll2', 'T21-unrollfence'
