...
EXPECTED_NAMES = ['T19-control13-repeat-r19', 'T20-wideunroll2-repeat-r19', 'T21-unrollfence']
EXPECTED_VERSIONS = ['T19-control13', 'T20-wideunroll2', 'T21-unrollfence']
PRIOR_JOB_ID = '1582004'
...
def record_helpers():
    ...
    spec = importlib.util.spec_from_file_location('r19_nohash_records', ROOT / '.runs/trsm/nohash-tools/records-kml.py')
...
def prepare():
    if PREPARE_REVIEW_PENDING:
        raise RuntimeError('r19 preparation is blocked pending root independent review')
    ...
    if ([item['run_name'] for item in plan['members']] != EXPECTED_NAMES
            ...
            or [item.get('parent') for item in plan['members']] != [None, 'T19-control13', 'T19-control13']
            or any(item.get('repeat_existing', False) is not (index < 2)
                   or (item.get('prior_job_id') != PRIOR_JOB_ID if index < 2 else item.get('prior_job_id') is not None)
                   for index, item in enumerate(plan['members']))):
        raise RuntimeError('The r19 protocol requires two unchanged repeats and the new T21 member')
    ...
    if (declared['warmup_order'] != EXPECTED_NAMES or declared['warmup_suites_per_member'] != 1
            ...
            or declared['formal_case_count'] != 27 or declared['warmup_case_count'] != 9
            or declared['sample_policy'] != 'Keep all three formal suites; no slow-sample exclusion'):
        raise RuntimeError('The r19 predeclared warm-up/formal protocol changed')
    ...
    for item in plan['members']:
        ...
        if item.get('repeat_existing'):
            ...
            if prior.get('job_id') != PRIOR_JOB_ID:
                raise RuntimeError('The r19 repeat must follow the registered r18 comparison job 1582004')
            ...
            for field in ('version', 'parent', 'strategy', 'source_implementation_id'):
                if field not in prior or item.get(field) != prior[field]:
                    raise RuntimeError('The r19 plan must preserve original implementation identity: ' + field)
        ...
        if item.get('repeat_existing'):
            ...
            if item['source_from'] != item['version'] or item['run_name'] != item['version']:
                raise RuntimeError('The new T21 member must use its prepared source directory')
        original = ROOT / '.runs/trsm' / item['source_from'] / 'source'
    ...
    config['warmup_protocol'] = dict(
        protocol_id='trsm-r19-one-official-warmup-v1', declared_at=group['created_at'],
        declared_before_new_performance_data=True,
        purpose='One identical full official suite per member after full CT64 wide32 preflight, to apply the '
                'startup-warming protocol declared before viewing r19 measurements.',
    ...
    for directory in ('preflight-wide32', 'reference'):
