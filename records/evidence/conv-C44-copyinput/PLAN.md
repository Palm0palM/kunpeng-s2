# C44-copyinput：原行距输入拷贝对照

状态仅prepared，未编译、测试、SSH、提交作业或推送，不使用重置卡。parent/source_parent均为C26-row4loads（正式C6来源）。本版本是reference-only，禁止晋级或作为最佳提交包；metadata标记reference_only=true/promotion_allowed=false，这只是预先约束，公共晋级工具未修改。
...
本版本实际copy_stride=stride。目标padded_stride为ceil(inputWidth/16)后置最低位为1、再乘16，例如64→80、65→80、80→80（不启用）、96→112、128→144。padding不初始化、不可被读入浮点运算；helpers的逻辑输出宽度始终原ow，kernel及原input只读。
