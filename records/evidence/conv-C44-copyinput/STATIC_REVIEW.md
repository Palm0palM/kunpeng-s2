...
- height×padded_stride先检查除法上界，再检查×sizeof(float)；row_bytes=stride×sizeof(float)另行先检查。分配字节<=536870912。复制行r满足0<=r<height，actual copy_stride=stride<=padded_stride，故r×copy_stride+stride不超过已核实分配元素数；源r×stride+stride也不超过原input元素数及size_t上界。copy_row最大INT_MAX-1后增至INT_MAX退出，不会再增溢出。
...
本次准备conv2d.c SHA256：`8618bd699656a750a378bd008a863178ab738093a783dfda9619ec035945fb6d`。prepared状态及源身份由checkpoint保存；无任何PASS或性能结论。
