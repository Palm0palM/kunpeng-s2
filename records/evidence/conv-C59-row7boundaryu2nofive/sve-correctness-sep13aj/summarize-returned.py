"""Read original AJ logs and write a lifecycle summary; no network/operator import."""
...
check(j['job_id']=='1589289' and j['version']=='C59-row7boundaryu2nofive','Original AJ identity')
...
for v in (16,32,64):
    for t in (1,4):
        ...
        check(legacy==dict(PREFIX=10850,TAIL=10850,ROWPAIR=234,ROWTRIPLE=2746,ROWQUAD=1778),f'Legacy entries {v}/{t}')
...
check(len(cpus)==len(set(cpus))==38 and numa=='node3' and cpus==list(range(114,152)),'Actual affinity')
check('114-151' in scheduler and 'aarch64' in scheduler,'Scheduler affinity/architecture')
...
outer=[]
for label in ('sep13aj-agent-status-001','sep13aj-agent-fetch-001'):
...
lines += ['',f'实际 GCC {compiler}，三条完整 gcc argv 来自 probe.log 的 xtrace，strict/generic/关闭FP融合；38CPU、24576MiB、单packed NUMA、1800秒，允许CPU {cpus[0]}–{cpus[-1]}，{numa}。六配置实际线程1/4，close/cores绑定。','', '五份运输源的返回字节、远端manifest、原件、prepared/job与本地清单一致。未运行sanitizer或官方runner；汇编原件已返回，目标spill分析由独立审查者处理。此摘要不执行接受/归档，不填性能或提速。','', '原始日志在 raw/；完整逐配置、source关联、资源及两次外层argv/UTC/输出路径见 AJ_SUMMARY.json。主额度检查35%，未使用reset；此子任务完成后停止。']
with (B/'AJ_SUMMARY.md').open('x') as f:f.write('\n'.join(lines)+'\n')
