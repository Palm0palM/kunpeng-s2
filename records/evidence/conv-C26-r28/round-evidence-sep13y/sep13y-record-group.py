"""Record only the saved Y group; preserve all 36 original measurements.
...
spec = importlib.util.spec_from_file_location('sep13y_submission_contract', Path(__file__).with_name('sep13y-submit-performance.py'))
...
def verify_plan(plan):
    assert plan['round'] == 'sep13y' and plan['status'] in ('performance_running', 'performance_complete')
    ...
    assert plan['diagnostic_expected_checks'] == {'C52-r1': 37128}
    ...
    assert plan['prior_job'] == '1582256' and plan['prior_campaign'] == '.runs/conv/sep13w-campaign.json'
    ...
    assert plan['candidate_source_hashes']['C52-r1'] == records[contract.SOURCE_VERSION]['source_hashes']
...
def main():
    ...
    for name in names:
        ...
        with (run/('status-driver-sep13y-'+stamp+'.log')).open('x') as output, contextlib.redirect_stdout(output):
    ...
    for name in names:
        ...
        with (run/('fetch-driver-sep13y-'+stamp+'.log')).open('x') as output, contextlib.redirect_stdout(output):
