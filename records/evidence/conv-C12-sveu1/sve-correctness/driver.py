...
import cluster
base=Path('.runs/conv/C12-sveu1/sve-correctness')
...
if action=='submit':
    ...
    remote_dir=cfg['remote_root']+'/diagnostics/conv-sveu1-correctness-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    ...
    command=cluster.submit_command(cfg,remote_dir,'conv-sveu1-correctness')
