...
参考作业运行于 COMPUTE_NODE_1、q_kunpeng、单 NUMA 5，CPU 190–227，38 线程；GCC 10.3.1，`-O3 -fno-fast-math -ffp-contract=off -mcpu=generic -fopenmp`，`CONV_BLOCK=32`、`CONV_KERNEL_UNROLL=2`。每组正式计时一次，另有一次预热。校验容差为 `1e-5`。
...
最终作业 1484056 为 SUCCEEDED，运行于 COMPUTE_NODE_1、q_kunpeng、单 NUMA 2，CPU 76–113，38 线程。GCC 10.3.1，`-O3 -mcpu=generic -ffp-contract=off -fopenmp`，未启用 fast-math。每组耗时为三次计时的均值，包含算子内部打包、分配和计算；benchmark 在计时外进行的数据准备不计入算子时间。
...
原 ZIP 的身份留档如下；首次导入阶段仅保留源码和文本证据。当前按协作要求将每题最佳提交包保存于 `题目/result/版本/题目.zip`，见[提交清单](../SUBMISSIONS.md)；下表继续保留原始历史包身份，不代表当前最佳包：
