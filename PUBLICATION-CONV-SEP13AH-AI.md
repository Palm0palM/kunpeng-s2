...
`records/conv-sep13ah-ai-publication.json` 为每个导出源文件和生成的候选补丁分别登记 original_sha256 与 public_sha256。前者是脱敏前内容的 SHA，后者才用于公开副本完整性校验。补丁的 original_sha256 指生成但尚未脱敏的补丁内容；其源码身份由实验 source_hashes 核对。生成的中文表和本说明没有对应原始文件。占位符映射及认证配置不公开。
...
本轮仅三个版本 C26-r36 / C58-row7boundaryu2 / C26-r37：AH1583350自身44,328项通过，AI1583408原36项通过，C58仅初筛资格true。两端C6门槛通过尚不等于独立确认通过；没有晋级或新提交包，C6保留。
