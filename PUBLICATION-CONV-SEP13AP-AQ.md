...
`records/conv-publication-sep13ap-aq.json` 为每个导出源文件和生成的候选补丁分别登记 original_sha256 与 public_sha256。前者是脱敏前内容的 SHA，后者才用于公开副本完整性校验。补丁的 original_sha256 指生成但尚未脱敏的补丁内容；其源码身份由实验 source_hashes 核对。生成的中文表和本说明没有对应原始文件。占位符映射及认证配置不公开。
