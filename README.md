...
CONV C52 共享两列展开的 W 初筛通过，随后 Y 独立确认36/36正确、合计耗时减少 **3.51%**；结束C6对照第一例的61.24ms慢样本使波动达到 **18.95%**，确认未通过，该轮保留C6。C53权重游标专项37128/37128通过，但实际地址指令未消除，未测性能。所有慢样本及未执行的包工具均留档。[C52独立确认](docs/CONV_SEP13Y.md) · [C53专项](docs/CONV_SEP13X.md) · [此前W初筛](docs/CONV_SEP13W.md)。

CONV C54四列展开已完成44328项专项和48/48性能样本：总中位706.99ms，比两端C6慢约56%，不晋级。全部慢样本、优化策略与实际汇编栈访问保留，该轮保留C6。[专项诊断](docs/CONV_SEP13AA.md) · [全部性能样本](docs/CONV_SEP13AB.md)。
...
CONV 单列循环与编译调优：同资源15套件60/60 PASS；C37/C39比C6慢1.58%/1.62%，C38拆分构建仅作对照，无确认收益。另有152,688项专项及原runner的8个用例通过，该轮保留C6。[完整记录](docs/CONV_SEP12F.md)。

CONV 六行/五行分块：同资源12套件48/48 PASS，专项共53,520项通过。C40比C6快1.78%，仍低于前后对照的波动门槛；C41慢4.22%，C42因GCC10不接受汇编操作数格式而编译失败、零项数值执行。该轮保留C6，完整样本与2,666个quad热点样本见[本轮记录](docs/CONV_SEP12G.md)。
...
CONV 输入复制与行距对照完成：434,352项专项检查及12套件48/48性能用例全部通过，但C43调整行距比C6慢4.10%，C44只复制输入慢4.29%；C43/C44差异也未达到行距收益门槛。全部样本和退化策略已留档，该轮保留C6。[完整记录](docs/CONV_SEP12H.md)。


## CONV AE/AF：列末约束未带来提速

C56完成AE1583102的44328项专项和AF1583230的48/48性能样本，最大误差0、所有退出为0。总中位467.46ms，比结束C6的452.33ms慢3.34%，两端初筛false；C55-r1仍仅作父源码参考。减少全helper栈访问没有带来实测提速，全部慢样本、策略、创建快照和原命令/退出保留；不确认、不晋级，该轮保留C6，Y/S原判保持。[专项诊断](docs/CONV_SEP13AE.md) · [全部性能样本](docs/CONV_SEP13AF.md)。


## CONV AG：显式 FMA 未通过官方精度

C57 基于 C52 将 shared 两列及单列余项改为显式 FMA；AG 原作业1583276的官方四例×三套 **12/12 FAIL**，误差超过原容差1e-5，失败发生在计时前，没有性能耗时。全部失败样本、退出和首次本地查询权限失败保留；不晋级，继续交付 C6。[完整报告](docs/CONV_SEP13AG.md) · [失败记录](docs/CONV_SEP13AG_RECORDS.md)。


## CONV AH/AI：C58初筛通过，等待独立确认

C58通过44328项正确性检查及36/36性能样本。合计437.56ms，比前后C6的452.41/452.42ms快约3.28%；第一例退化0.78%，其余三例改善。全部样本保留，初筛通过尚未晋级，当前最佳包仍C6。[正确性与展开证据](docs/CONV_SEP13AH.md) · [全部性能样本](docs/CONV_SEP13AI.md)。

CONV C59 撤掉一处边界展开提示：AJ44328项通过，AM36/36 PASS，合计437.94ms，相对结束C7控制437.66ms减少-0.0640%。未通过初筛，保留C7。目标spill确实减少，但晋级仍以完整测量门槛为准。[本轮记录](docs/CONV_SEP13AJ_AM.md)。

CONV C60 相邻列输入复用：AN44328项通过，AO36/36 PASS，合计458.47ms，相对结束C7控制437.55ms耗时增加4.7812%。未通过初筛，保留C7。目标输入加载确实减少，但晋级仍以完整测量门槛为准。[本轮记录](docs/CONV_SEP13AN_AO.md)。

CONV C61 逐行共享两列系数调度：AP44328项通过，AQ36/36 PASS，合计437.31ms，相对结束C7控制437.56ms耗时减少0.0571%。未通过初筛，保留C7。实际主循环指令数保持123，性能结论以完整测量门槛为准。[本轮记录](docs/CONV_SEP13AP_AQ.md)。

CONV C62 七行双向量/共享四列联合tile：AR44328项通过，AS36/36 PASS，合计538.98ms，相对结束C7控制437.62ms耗时增加23.1616%。未通过初筛，保留C7。实际四列循环仍有17读17写Z栈访问；变化后的tile原始指令数不代表加速。[本轮记录](docs/CONV_SEP13AR_AS.md)。

CONV C63 四列共享逐行权重调度：AT44328项通过，AU36/36 PASS，合计463.66ms，相对结束C7控制437.58ms耗时增加5.9601%。未通过初筛，保留C7。实际四列循环与全helper仅1读1写Z栈访问，速度判断仍以两端完整测量门槛为准。[本轮记录](docs/CONV_SEP13AT_AU.md)。
